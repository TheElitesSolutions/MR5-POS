 ...
 */
export function areAddonsEqual(existingAddons, selectedAddons) {
 ...
 */
export async function scaleAddonsForQuantityChange(orderItemId, quantityChange, currentOrderId) {
    try {
        // Call IPC to scale addon quantities
        const addonResult = await window.electronAPI.ipc.invoke('addon:scaleAddonQuantities', {
            orderItemId,
 ...
 */
export function shouldPrintAsInitialOrder(newItemsCount, updatesCount, totalOrderItems) {
 ...
 */
export function prepareKitchenPrintData(changesSummary, totalOrderItems) {
    // Filter changes
    ...
    const filteredChangesSummary = changesSummary.filter(c => c.changeType !== 'REMOVE' && (c.changeType !== 'UPDATE' || c.netChange > 0));
    // Determine ticket type
    const isInitialOrder = shouldPrintAsInitialOrder(newItems.length, updates.length, totalOrderItems);
    return {
        newItems,
        updates,
