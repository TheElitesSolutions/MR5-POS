import { OrderItem, AddonSelection, OrderItemAddon } from '@/types';

/**
 * Responsive Add-on Display Utilities
 *
 * Functions to optimize add-on display across different screen sizes:
 * - Mobile: Compact, touch-friendly display
 * - Tablet: Balanced information density
 * - Desktop: Full details with enhanced interactions
 */
 ...
 */
export function getResponsiveConfig(): ResponsiveDisplayConfig {
 ...
 */
export function getAddonItemsPerRow(config: ResponsiveDisplayConfig): number {
  if (config.isMobile) return 1;
  if (config.isTablet) return 2;
  return 3; // Desktop
}
 ...
 */
export function getAddonCardSize(
  config: ResponsiveDisplayConfig
): 'sm' | 'md' | 'lg' {
  if (config.isMobile) return 'sm';
  if (config.isTablet) return 'md';
  return 'lg';
}
 ...
 */
export function shouldShowAddonDescriptions(
  config: ResponsiveDisplayConfig
): boolean {
...
export function getInitialExpandedGroups(
  ...
  if (config.isMobile) return 1; // Only expand first group
  if (config.isTablet) return 2;
 ...
 */
export function getVirtualizationThreshold(
  config: ResponsiveDisplayConfig
): number {
  if (config.isMobile) return 10; // Lower threshold for mobile performance
  if (config.isTablet) return 20;
 ...
 */
export function getVirtualListHeight(config: ResponsiveDisplayConfig): number {
  if (config.isMobile) return 300; // Smaller height for mobile
  if (config.isTablet) return 400;
 ...
 */
export function getTouchTargetSize(config: ResponsiveDisplayConfig): number {
 ...
 */
export function createMobileAddonSummary(
  addons: (AddonSelection | OrderItemAddon)[],
  maxItems: number = 2
): string {
...
export function shouldExpandByDefault(
  ...
  if (priority === 'medium') return index < maxExpanded;
...
export function getResponsiveSpacing(
  config: ResponsiveDisplayConfig,
  element: 'container' | 'card' | 'button' | 'text'
): string {
  const spacing = {
    container: {
  ...
  if (config.isMobile) return spacing[element].mobile;
  if (config.isTablet) return spacing[element].tablet;
 ...
 */
export function getAddonGridClasses(config: ResponsiveDisplayConfig): string {
  if (config.isMobile) {
...
export function shouldEnableSearch(
  ...
  if (config.isMobile) return itemCount > 5;
  if (config.isTablet) return itemCount > 8;
 ...
 */
export function getKeyboardNavigationConfig(config: ResponsiveDisplayConfig) {
  return {
...
export function createResponsiveObserver(
  callback: (config: ResponsiveDisplayConfig) => void,
 ...
 */
export function optimizeAddonsForMobile(
  addons: (AddonSelection | OrderItemAddon)[],
  config: ResponsiveDisplayConfig
): (AddonSelection | OrderItemAddon)[] {
  if (!config.isMobile) return addons;
 ...
 */
export function getAddonModalSize(
  config: ResponsiveDisplayConfig
): 'sm' | 'md' | 'lg' | 'xl' {
  if (config.isMobile) return 'sm'; // Smaller modal on mobile
  if (config.isTablet) return 'md';
 ...
 */
export function shouldLoadAddonImages(
  config: ResponsiveDisplayConfig
): boolean {
...
/**
 * Gets performance-optimized settings for addon components
 */
export interface AddonPerformanceConfig {
