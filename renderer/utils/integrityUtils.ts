 ...
 */
export async function isMenuItemUsedInActiveOrders(
  menuItemId: string
): Promise<boolean> {
  try {
    ...
    return activeOrders.some((order: any) =>
      order.items.some((item: any) => item.menuItemId === menuItemId)
    );
  } catch (error) {
    console.error(
      'Failed to check if menu item is used in active orders:',
      error
    );
 ...
 */
export async function isMenuItemInCombos(menuItemId: string): Promise<boolean> {
  try {
    ...
    return combos.some((combo: any) => combo.items.includes(menuItemId));
  } catch (error) {
 ...
 */
export async function hasMenuItemSalesData(
  menuItemId: string
): Promise<boolean> {
  try {
 ...
 */
export async function checkMenuItemDeletionIntegrity(
  menuItemId: string,
  menuItemName: string
