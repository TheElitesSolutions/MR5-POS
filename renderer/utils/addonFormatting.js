 ...
 */
export const groupAddonsByCategory = (addons) => {
    ...
    return Array.from(groupMap.entries())
        ...
        const totalPrice = groupAddons.reduce((sum, addon) => sum + addon.totalPrice, 0);
        const totalQuantity = groupAddons.reduce((sum, addon) => sum + addon.quantity, 0);
        return {
            groupName,
            addons: groupAddons.sort((a, b) => a.addonName.localeCompare(b.addonName)),
            totalPrice,
            totalQuantity,
        };
    })
 ...
 */
export const formatOrderItemWithAddons = (orderItem) => {
    const addons = orderItem.addons || [];
    const formattedAddons = groupAddonsByCategory(addons);
    const totalAddonPrice = addons.reduce((sum, addon) => sum + addon.totalPrice, 0);
    const totalAddonCount = addons.reduce((sum, addon) => sum + addon.quantity, 0);
    return {
        ...orderItem,
        formattedAddons,
        addonSummary: {
 ...
 */
export const getAddonSummaryText = (addons, format = 'short') => {
    if (!addons || addons.length === 0) {
        return '';
    }
    if (format === 'short') {
    ...
    return grouped
        .map(group => `${group.groupName}: ${group.addons
        .map(addon => `${addon.addonName}${addon.quantity > 1 ? ` x${addon.quantity}` : ''}`)
        .join(', ')}`)
        .join('; ');
};
/**
 ...
 */
export const getAddonAccessibilityText = (addons) => {
    if (!addons || addons.length === 0) {
        return 'No add-ons';
    }
    const totalCount = addons.reduce((sum, addon) => sum + addon.quantity, 0);
    ...
    const addonList = addons
        .map(addon => `${addon.addonName}${addon.quantity > 1 ? ` quantity ${addon.quantity}` : ''} at ${addon.unitPrice.toFixed(2)} dollars each`)
        .join(', ');
...
export const calculateOrderSummary = (orderItems) => {
    ...
    orderItems.forEach(item => {
        ...
        // Add-on costs
        if (item.addons) {
            const itemAddonTotal = item.addons.reduce((sum, addon) => sum + addon.totalPrice, 0);
            const itemAddonCount = item.addons.reduce((sum, addon) => sum + addon.quantity, 0);
 ...
 */
export const getMobileAddonSummary = (addons) => {
    if (!addons || addons.length === 0) {
        return '';
    }
    if (addons.length === 1) {
 ...
 */
export const getHighlightAddon = (addons) => {
    if (!addons || addons.length === 0) {
        return null;
    }
    return addons.reduce((highest, current) => current.totalPrice > highest.totalPrice ? current : highest);
};
/**
 * Validates that addon prices match expected totals
 */
export const validateAddonPricing = (addons) => {
    const errors = [];
    let isValid = true;
    addons.forEach((addon, index) => {
        ...
        if (Math.abs(expectedTotal - actualTotal) > 0.01) {
            ...
            errors.push(`Addon ${index + 1} (${addon.addonName}): Expected total ${expectedTotal.toFixed(2)}, got ${actualTotal.toFixed(2)}`);
            isValid = false;
        }
        if (addon.quantity <= 0) {
            errors.push(`Addon ${index + 1} (${addon.addonName}): Invalid quantity ${addon.quantity}`);
            isValid = false;
        }
        if (addon.unitPrice < 0) {
            errors.push(`Addon ${index + 1} (${addon.addonName}): Invalid unit price ${addon.unitPrice}`);
            isValid = false;
        }
    });
 ...
 */
export const sortAddonsForDisplay = (addons) => {
 ...
 */
export const formatAddonSelection = (addon) => {
