 ...
 */
export function getMenuItemPrice(menuItem) {
    ...
    const matchingDefault = DEFAULT_PRICES.find(def => menuItem.name && def.pattern.test(menuItem.name));
 ...
 */
export function getOrderItemUnitPrice(orderItem) {
    ...
    const matchingDefault = DEFAULT_PRICES.find(def => itemName && def.pattern.test(itemName));
    // Return matching price or fallback
    return matchingDefault ? matchingDefault.price : FALLBACK_PRICE;
}
/**
 ...
 */
export function getOrderItemTotalPrice(orderItem) {
    if (!orderItem)
        return 0;
 ...
 */
export function formatPrice(price, { showCurrency = true, decimals = 2 } = {}) {
