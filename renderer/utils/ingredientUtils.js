 ...
 */
export function calculateIngredientCosts(ingredients, stockItems) {
    let totalCost = 0;
 ...
 */
export function convertIngredientsFromApi(ingredientIds, stockItems, defaultQuantity = 1) {
 ...
 */
export function convertIngredientsForApi(ingredients) {
...
const ingredientNameCache = new Map();
/**
 ...
 */
export function getIngredientNameById(ingredientId, stockItems, allowUnknown = false) {
    ...
    if (!stockItems || stockItems.length === 0) {
        if (process.env.NODE_ENV === 'development') {
            console.log('getIngredientNameById: Stock items not loaded yet for:', ingredientId);
        }
    ...
    if (!ingredientId || typeof ingredientId !== 'string') {
        if (process.env.NODE_ENV === 'development') {
            console.warn('getIngredientNameById: Invalid ingredient ID:', ingredientId);
        }
 ...
 */
export function clearIngredientNameCache() {
 ...
 */
export function getIngredientNameSafe(ingredientIdOrName, stockItems, fallback = '') {
    // 🔥 CRITICAL FIX: Handle non-string inputs (Ingredient objects, null, undefined)
    if (!ingredientIdOrName) {
        if (process.env.NODE_ENV === 'development') {
            console.warn('getIngredientNameSafe: Input is null/undefined:', ingredientIdOrName);
        }
        return fallback;
    }
    ...
    if (typeof ingredientIdOrName === 'object' && ingredientIdOrName.name) {
        if (process.env.NODE_ENV === 'development') {
            console.log('getIngredientNameSafe: Received Ingredient object:', ingredientIdOrName);
        }
    ...
    if (typeof ingredientIdOrName === 'object' && ingredientIdOrName.id) {
        const resultById = getIngredientNameById(ingredientIdOrName.id, stockItems, false);
        if (resultById) {
            return resultById;
        }
    ...
    if (stockItems && stockItems.length > 0) {
        const matchingItem = stockItems.find(item => item.itemName?.toLowerCase() === stringInput.toLowerCase());
        if (matchingItem) {
            if (process.env.NODE_ENV === 'development') {
    ...
    if (stringInput && stringInput.trim()) {
        if (process.env.NODE_ENV === 'development') {
            console.log('getIngredientNameSafe: Using ingredient name as-is:', stringInput);
        }
        return stringInput;
    }
    return fallback;
 ...
 */
export function getIngredientNamesByIds(ingredientIds, stockItems, allowUnknown = false) {
    if (!Array.isArray(ingredientIds))
        return [];
