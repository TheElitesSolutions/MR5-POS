...
interface UseAddonSearchProps {
  addons: Addon[];
  searchDelay?: number;
  minSearchLength?: number;
}

interface UseAddonSearchReturn {
  searchQuery: string;
...
export const useAddonSearch = ({
  ...
  searchDelay = 300,
  minSearchLength = 2,
}: UseAddonSearchProps): UseAddonSearchReturn => {
  ...
  const scoreAddon = useCallback(
    (addon: Addon, query: string): number => {
      const normalizedQuery = query.toLowerCase();
