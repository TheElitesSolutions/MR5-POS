...
export const useTouchGestures = (element, options = {}) => {
    const { onSwipeLeft, onSwipeRight, onSwipeUp, onSwipeDown, onTap, onDoubleTap, onLongPress, onPinch, swipeThreshold = 50, longPressDelay = 500, doubleTapDelay = 300, enabled = true, } = options;
    ...
    const handleTouchMove = useCallback((event) => {
        ...
        if (touchState.longPressTimer) {
            const moveDistance = Math.sqrt(Math.pow(touchState.endX - touchState.startX, 2) +
...
export const useSwipeToSelect = (element, onSelect, onDeselect, isSelected) => {
...
export const useSwipeQuantity = (element, onIncrease, onDecrease) => {
    return useTouchGestures(element, {
