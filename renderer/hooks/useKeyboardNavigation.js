 ...
 */
export const useKeyboardNavigation = ({ enabled = true, onNavigate, onSelect, onEscape, onSearch, itemCount = 0, currentIndex = -1, allowWrap = true, }) => {
    ...
    // Screen reader announcements
    const announceNavigation = useCallback((index, total) => {
    ...
    const focusItem = useCallback((index) => {
        const element = document.querySelector(`[data-addon-index="${index}"]`);
        if (element) {
            element.focus();
...
export const useAddonGroupFocus = (groupId, addonCount) => {
    const [focusedAddonIndex, setFocusedAddonIndex] = useState(-1);
    const focusAddon = useCallback((index) => {
        const addonElement = document.querySelector(`[data-group-id="${groupId}"] [data-addon-index="${index}"]`);
        if (addonElement) {
            addonElement.focus();
            setFocusedAddonIndex(index);
        }
    }, [groupId]);
    const navigateWithinGroup = useCallback((direction) => {
