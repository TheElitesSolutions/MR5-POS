...
export interface SimpleOrderChange {
  itemId: string;
  name: string;
  menuItemId: string;
  ...
  netChange: number;
  changeType: 'NEW' | 'UPDATE' | 'REMOVE';
}

export interface SimpleOrderTracking {
  newItems: Array<{
    id: string;
    name: string;
    menuItemId: string;
...
export function useSimpleOrderTracking(orderId?: string) {
  ...
  const trackNewItem = useCallback(
    (itemId: string, name: string, menuItemId: string, quantity: number) => {
      if (!orderId) return;
      ...
      const cleanRemovedItems = current.removedItems.filter(
        item => item.id !== itemId
      );
  ...
  const trackQuantityChange = useCallback(
    (
      ...
      if (isNewItem) {
        ...
        const updatedNewItems = current.newItems.map(item =>
  ...
  // Track item removal
  const trackItemRemoval = useCallback(
    (itemId: string, name: string, menuItemId: string, quantity: number) => {
      if (!orderId) return;
      ...
      if (wasNewItem) {
        ...
        const cleanNewItems = current.newItems.filter(
          item => item.id !== itemId
        );
  ...
  const getKitchenChanges = useCallback(() => {
    if (!tracking) return { hasChanges: false, changesSummary: [] as SimpleOrderChange[] };
    ...
    return {
      ...
      updatedItemsCount: Object.values(tracking.netChanges).filter(
