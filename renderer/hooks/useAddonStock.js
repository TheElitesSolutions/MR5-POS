 ...
 */
export const useAddonStock = (options = {}) => {
    const { enableRealTimeUpdates = true, updateInterval = 15000, // 15 seconds
    showToastWarnings = true, autoCheckOnSelectionChange = true, } = options;
    const { toast } = useToast();
    // State management
    const [stockStatuses, setStockStatuses] = useState(new Map());
    const [bulkStockResult, setBulkStockResult] = useState(null);
    const [alternatives, setAlternatives] = useState(new Map());
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const [subscribedAddonIds, setSubscribedAddonIds] = useState([]);
    // Get stock status for a single add-on
    const getStockStatus = useCallback(async (addon) => {
        try {
            setError(null);
            ...
            const errorMessage = err instanceof Error ? err.message : 'Failed to get stock status';
    ...
    const checkBulkStock = useCallback(async (selections) => {
        try {
            setIsLoading(true);
            setError(null);
            const request = {
            ...
            if (showToastWarnings) {
                if (result.overallStatus === 'errors' &&
                    ...
                    const highPriorityWarnings = result.warnings.filter(w => w.severity === 'high');
    ...
    const checkStock = useCallback(async (addonId, quantity, existingSelections) => {
        try {
            setError(null);
            ...
            if (result.alternatives.length > 0) {
                setAlternatives(prev => new Map(prev).set(addonId, result.alternatives));
            }
            return result;
        }
        catch (err) {
    ...
    const getAlternatives = useCallback(async (addon, quantity) => {
        try {
            setError(null);
            const alternativeResults = await addonStockService.getAlternatives(addon, quantity);
            // Cache the alternatives
            setAlternatives(prev => new Map(prev).set(addon.id, alternativeResults));
    ...
    // Validation helpers
    const canSelectQuantity = useCallback((addon, quantity) => {
    ...
    const getMaxQuantity = useCallback((addon) => {
        const status = stockStatuses.get(addon.id);
        return status ? status.maxQuantity : 999; // Default max if no status available
    }, [stockStatuses]);
    // Computed state
    const hasStockWarnings = useMemo(() => {
        return Array.from(stockStatuses.values()).some(status => status.level === 'low' || status.level === 'critical');
    }, [stockStatuses]);
    const hasStockErrors = useMemo(() => {
        return Array.from(stockStatuses.values()).some(status => status.level === 'out_of_stock');
    }, [stockStatuses]);
    // Cache management
    ...
    useEffect(() => {
        ...
        const subscribe = async () => {
            const unsubscribeFn = await addonStockService.subscribeToStockUpdates(subscribedAddonIds, (updates) => {
    ...
    const subscribeToAddon = useCallback((addonId) => {
    ...
    const unsubscribeFromAddon = useCallback((addonId) => {
 ...
 */
export const useAddonStockStatus = (addon) => {
    ...
    return {
        ...
        error,
        refreshStatus,
        canSelect: (quantity) => status?.canSelect(quantity) || false,
