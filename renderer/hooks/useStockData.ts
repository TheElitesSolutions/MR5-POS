...
import { StockQueryParams } from '../services/domain/StockService';
...
export interface UseStockItemsOptions extends StockQueryParams {
...
export interface UseStockItemsResult {
  // Data
...
export interface UseSingleStockItemResult {
  // Data
...
export interface UseLowStockAlertsResult {
  // Data
...
export interface UseIngredientAvailabilityResult {
  // Data
 ...
 */
export function useStockItemById(
  id: string | null,
  enabled = true
): UseSingleStockItemResult {
  const stockService = getStockService();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Failed to fetch stock item';
      setError(errorMessage);
 ...
 */
export function useStockCategories(enabled = true): UseCategoriesResult {
  const stockService = getStockService();
 ...
 */
export function useLowStockAlerts(enabled = true): UseLowStockAlertsResult {
  const stockService = getStockService();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Failed to fetch low stock alerts';
      setError(errorMessage);
...
export function useIngredientAvailability(
  ...
  enabled = true
): UseIngredientAvailabilityResult {
  const stockService = getStockService();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const availability = await stockService.checkIngredientAvailability(
        stableParams.ingredientIds,
        stableParams.requiredQuantities
      );
      ...
      setError(errorMessage);
      console.error(
        'useIngredientAvailability: Failed to check availability:',
        err
      );
    } finally {
      setIsLoading(false);
...
export function useAllStockData(enabled = true) {
  const {
    ...
    error: itemsError,
  } = useStockItems({ enabled });
  const {
    lowStockItems,
    isLoading: alertsLoading,
    error: alertsError,
  } = useLowStockAlerts(enabled);
  const {
    categories,
    isLoading: categoriesLoading,
    error: categoriesError,
  } = useStockCategories(enabled);
