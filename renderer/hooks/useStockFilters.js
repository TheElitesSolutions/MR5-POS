...
export const useStockFilters = (stockItems) => {
    ...
    const filteredItems = useMemo(() => {
        ...
        if (filters.searchTerm) {
            ...
            items = items.filter(item => (item.name ?? item.itemName ?? '').toLowerCase().includes(term) ||
        ...
        if (filters.stockLevelFilter !== 'all') {
            items = items.filter(item => getStockLevel(item) === filters.stockLevelFilter);
        }
        // Value range filter
    ...
    const stats = useMemo(() => {
        ...
        const realItems = stockItems.filter(item => item.supplier !== 'System-Generated Category');
        ...
        const categories = Array.from(new Set(stockItems.map(item => item.category || 'Uncategorized'))).sort();
        const categoryStats = categories.map(category => {
            // Only count real items for category stats
            const categoryItems = realItems.filter(item => (item.category || 'Uncategorized') === category);
