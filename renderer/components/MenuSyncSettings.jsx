...
export function MenuSyncSettings() {
    ...
    const [lastSyncStatus, setLastSyncStatus] = useState('pending');
    ...
    const loadSyncStatus = async () => {
        ...
        try {
            ...
            if (response.success && response.data) {
                ...
                setLastSyncTime(syncStatus.lastSyncTime ? new Date(syncStatus.lastSyncTime) : null);
    ...
    if (!isConfigured) {
        return (<div className='rounded-lg border border-yellow-300 bg-yellow-50 p-6'>
    ...
    return (<div className='space-y-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm'>
      <div className='flex items-center justify-between'>
        ...
        <button onClick={handleManualSync} disabled={isSyncing} className={`flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium text-white transition-colors ${isSyncing
      ...
      {/* Status Message */}
      {message && (<div className={`rounded-lg p-4 ${message.type === 'success'
                ? 'bg-green-50 text-green-800'
                : message.type === 'error'
                    ? 'bg-red-50 text-red-800'
                    : 'bg-blue-50 text-blue-800'}`}>
          <div className='flex items-center gap-2'>
            {message.type === 'success' ? (<Check className='h-5 w-5'/>) : message.type === 'error' ? (<AlertCircle className='h-5 w-5'/>) : (<RefreshCw className='h-5 w-5 animate-spin'/>)}
      ...
      <div className='space-y-3 rounded-lg border border-gray-200 bg-gray-50 p-4'>
        <div className='flex items-center justify-between'>
          ...
          <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-medium ${lastSyncStatus === 'success'
        ...
        {lastSyncError && (<p className='text-xs text-red-600'>{lastSyncError}</p>)}
      </div>
      ...
      <div className='space-y-4'>
        <div className='flex items-center justify-between'>
          ...
          <label className='relative inline-flex cursor-pointer items-center'>
            <input type='checkbox' checked={autoSyncEnabled} onChange={handleToggleAutoSync} className='peer sr-only'/>
            <div className="peer h-6 w-11 rounded-full bg-gray-200 after:absolute after:left-[2px] after:top-[2px] after:h-5 after:w-5 after:rounded-full after:border after:border-gray-300 after:bg-white after:transition-all after:content-[''] peer-checked:bg-blue-600 peer-checked:after:translate-x-full peer-checked:after:border-white peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300"></div>
          </label>
        </div>

        {/* Sync Interval Selector */}
        {autoSyncEnabled && (<div className='space-y-2'>
            <label className='text-sm font-medium text-gray-700 dark:text-white'>
              Sync Interval
            </label>
            <select value={syncInterval} onChange={e => handleIntervalChange(Number(e.target.value))} className='w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500'>
