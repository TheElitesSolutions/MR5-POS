...
export default function UpdateSettings() {
    ...
    const { status, checkForUpdates, downloadUpdate, installUpdate, cancelUpdate, skipVersion, } = useUpdater();
    ...
    useEffect(() => {
        const fetchVersion = async () => {
            if (typeof window !== 'undefined' && window.electron) {
                try {
                    const systemInfo = await window.electron.ipcRenderer.invoke('mr5pos:system:get-info');
    ...
    return (<div className='space-y-6'>
      ...
      <div className='flex items-center justify-between'>
        ...
        <Button onClick={handleCheckForUpdates} disabled={status.checking || status.downloading} variant='outline' size='sm'>
          {status.checking ? (<>
      ...
      {/* Error Message */}
      {status.error && (<div className='rounded-lg border border-red-200 bg-red-50 p-4'>
          <div className='flex items-start space-x-3'>
      ...
      {/* Update Available */}
      {status.available && !status.downloaded && (<div className='rounded-lg border border-blue-200 bg-blue-50 p-4'>
          <div className='flex items-start space-x-3'>
            ...
            <div className='flex-1'>
              ...
              {status.updateInfo?.releaseNotes && (<p className='mt-2 text-sm text-blue-700'>
      ...
      {/* Download Progress */}
      {status.downloading && status.progress && (<div className='space-y-3'>
      ...
      {/* Update Downloaded */}
      {status.downloaded && (<div className='rounded-lg border border-green-200 bg-green-50 p-4'>
          <div className='flex items-start space-x-3'>
      ...
      {!status.checking &&
            ...
            !status.downloading &&
            !status.downloaded &&
            !status.error && (<div className='rounded-lg border border-gray-200 bg-gray-50 p-4'>
            <div className='flex items-start space-x-3'>
