'use client';
import { AlertCircle, AlertTriangle, Bluetooth, CheckCircle, HelpCircle, Loader2, Printer, RefreshCw, Settings, Usb, Wifi, XCircle, Zap, } from 'lucide-react';
...
export default function PrinterSettings() {
    ...
    const [selectedPrinterType, setSelectedPrinterType] = useState('receipt');
    const fetchPrinters = async (showLoading = true) => {
        ...
        try {
            ...
            if (typeof window !== 'undefined' && window.electronAPI) {
                ...
                const printersWithStatus = printersData.map((printer) => ({
                    ...printer,
                ...
                appLogger.debug('✅ Printers loaded successfully:', printersWithStatus.length);
            }
            else {
    ...
    const directTestPrint = async (printerName) => {
        ...
        try {
            ...
            if (typeof window !== 'undefined' && window.electronAPI) {
                ...
                if (testResult.success && testResult.data?.success) {
                    ...
                    setPrinters(prev => prev.map(p => p.name === printerName
                        ? {
                            ...p,
                            connectionStatus: 'connected',
                            lastSeen: new Date().toLocaleTimeString(),
    ...
    const ultimateThermalTestPrint = async (printerName) => {
        setTestingPrinter(printerName);
        try {
            appLogger.debug(`🚀 Testing ultimate thermal printing (direct USB) for printer: ${printerName}`);
            if (typeof window !== 'undefined' && window.electronAPI) {
                // Use "ultimate_thermal" test type
                const testResult = await PrinterAPI.testPrint(printerName, 'ultimate_thermal');
    ...
    const hybridThermalTestPrint = async (printerName) => {
        setTestingPrinter(printerName);
        try {
            appLogger.debug(`🔧 Testing hybrid thermal printing for printer: ${printerName}`);
            if (typeof window !== 'undefined' && window.electronAPI) {
                // Use "hybrid_thermal" test type
                const testResult = await PrinterAPI.testPrint(printerName, 'hybrid_thermal');
    ...
    const thermalLibraryTestPrint = async (printerName) => {
        setTestingPrinter(printerName);
        try {
            appLogger.debug(`🔧 Testing with thermal library for printer: ${printerName}`);
            if (typeof window !== 'undefined' && window.electronAPI) {
                // Use "thermal_library" test type for electron-pos-printer
                const testResult = await PrinterAPI.testPrint(printerName, 'thermal_library');
    ...
    const testPrint = async (printerName) => {
        ...
        try {
            ...
            if (typeof window !== 'undefined' && window.electronAPI) {
                ...
                const printerType = selectedPrinterType || printerInfo?.type?.toLowerCase() || 'receipt';
                ...
                if (testResult.success && testResult.data?.success) {
                    ...
                    if (errorMessage.includes('Pending Deletion')) {
                        troubleshootingSteps.push('', '⚠️ PRINTER STATUS: "Pending Deletion"', '', 'This usually means there are stuck print jobs. The system is attempting to automatically fix this issue.', '', 'If the automatic fix fails, try these manual steps:', '1. Open Windows Services (services.msc)', '2. Find "Print Spooler" service', '3. Right-click and select "Stop"', '4. Navigate to C:\\Windows\\System32\\spool\\PRINTERS', '5. Delete all files in this folder', '6. Go back to Services and Start "Print Spooler"', '7. Retry the test print', '', 'Alternative: Restart Windows to clear the printer status');
                    }
                    // Add specific guidance based on printer type and error
                    if (printerName.includes('RONGTA') || printerName.includes('80mm')) {
                        troubleshootingSteps.push('', '🖨️ RONGTA/Thermal Printer Specific Checks:', '1. Verify RONGTA drivers are installed from manufacturer', '2. Check USB connection and try a different USB port', '3. Ensure printer is in ESC/POS mode (not label mode)', '4. Try power cycling: Turn off → wait 10 seconds → turn on', '5. Check if printer appears in Windows Device Manager', '6. Verify paper is loaded correctly and not jammed');
                    }
                    troubleshootingSteps.push('', '🔧 General Printer Checks:', '1. Is the printer powered on and showing ready status?', '2. Are there any error lights or display messages?', '3. Is the printer set as online (not paused/offline)?', '4. Try printing a Windows test page from printer properties', '5. Check Windows Print Spooler service is running', '', '📋 Advanced Troubleshooting:', '1. Update or reinstall printer drivers', '2. Try setting printer as default printer', '3. Check for Windows updates', '4. Restart the application and try again');
                    setTroubleshootingContent({
    ...
    const getConnectionIcon = (_connectionStatus, connectionType) => {
    ...
    const getSystemStatusIndicator = () => {
        switch (systemStatus) {
            ...
            {lastRefresh && (<span className='ml-2 text-xs text-muted-foreground'>
    ...
    const getPrinterTypeTag = (type) => {
        switch (type?.toLowerCase()) {
            case 'thermal':
                return <Badge className='bg-blue-500 hover:bg-blue-600'>Thermal</Badge>;
            case 'kitchen':
                return (<Badge className='bg-orange-500 hover:bg-orange-600'>Kitchen</Badge>);
            case 'bar':
                return <Badge className='bg-purple-500 hover:bg-purple-600'>Bar</Badge>;
            case 'document':
                return (<Badge className='bg-gray-500 hover:bg-gray-600'>Document</Badge>);
            default:
    ...
    return (<div className='space-y-6'>
      ...
      <Card>
        <CardHeader className='pb-3'>
          <div className='flex items-center justify-between'>
            ...
            <Button variant='outline' size='sm' onClick={() => fetchPrinters()} disabled={isLoading || isRefreshing}>
        ...
        <CardContent>
          ...
          <div className='mt-3 flex items-center rounded-md border border-amber-200 bg-amber-50 p-2'>
            ...
            <Button variant='link' className='ml-auto h-auto p-0 text-xs text-amber-700' onClick={showThermalPrinterHelp}>
              Setup Guide
            </Button>
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (<div className='flex items-center justify-center py-8'>
            ...
            </div>) : printers.length === 0 ? (<div className='py-8 text-center text-muted-foreground'>
              <Printer className='mx-auto mb-4 h-12 w-12 opacity-50'/>
              <p>No printers found</p>
              <Button variant='outline' onClick={() => fetchPrinters()} className='mt-4'>
                Scan Again
              </Button>
            </div>) : (<div className='space-y-4'>
              {printers.map((printer, index) => (<div key={printer.deviceId || index} className={`rounded-lg border p-4 transition-colors hover:bg-muted/50 ${selectedPrinter === printer.name
                  ...
                  <div className='flex items-start justify-between gap-4'>
                    ...
                    <div className='flex min-w-0 flex-1 items-start gap-3'>
                      <div className='mt-1 flex-shrink-0'>
                        {getConnectionIcon(printer.connectionStatus, printer.connectionType)}
                      </div>
                      <div className='min-w-0 flex-1'>
                        <div className='flex items-center gap-2'>
                          <h3 className='truncate font-medium' title={printer.name}>
                            {printer.name}
                          </h3>
                          {getPrinterTypeTag(printer.type)}
                        </div>
                        <p className='line-clamp-2 break-words text-sm text-muted-foreground' title={printer.description}>
                          {printer.description}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className='flex flex-shrink-0 items-center gap-3'>
                      {getStatusBadge(testingPrinter === printer.name
                    ? 'testing'
                    : printer.connectionStatus)}
                      <div className='flex gap-2'>
                        <Button variant={selectedPrinter === printer.name
                    ? 'default'
                    : 'outline'} size='sm' onClick={() => setSelectedPrinter(printer.name)} className='whitespace-nowrap'>
                          {selectedPrinter === printer.name ? (<CheckCircle className='mr-2 h-4 w-4'/>) : (<Settings className='mr-2 h-4 w-4'/>)}
                          {selectedPrinter === printer.name
                    ? 'Selected'
                    : 'Select'}
                        </Button>
                        <Button variant='outline' size='sm' onClick={() => ultimateThermalTestPrint(printer.name)} disabled={testingPrinter === printer.name} className='whitespace-nowrap border-purple-300 bg-gradient-to-r from-purple-50 to-pink-50 font-bold text-purple-800 shadow-md hover:from-purple-100 hover:to-pink-100' title='🚀 ULTIMATE SOLUTION: Direct USB hardware control bypassing Windows drivers completely - the most advanced thermal printing solution'>
                          {testingPrinter === printer.name ? (<Loader2 className='mr-2 h-4 w-4 animate-spin'/>) : (<Zap className='mr-2 h-4 w-4 text-purple-600'/>)}
                          Test Print
                        </Button>
                      </div>
                    </div>
                  ...
                  <div className='mt-3 flex flex-wrap items-center gap-2'>
                    ...
                    {printer.isDefault && (<span className='whitespace-nowrap rounded bg-blue-100 px-2 py-1 text-xs text-blue-800'>
                        Default
                      </span>)}
                    {selectedPrinter === printer.name && (<span className='whitespace-nowrap rounded bg-green-100 px-2 py-1 text-xs text-green-800'>
                        Selected for POS
                      </span>)}
                    {printer.lastSeen && (<span className='whitespace-nowrap text-xs text-muted-foreground'>
      ...
      <Card>
        ...
        <CardContent className='space-y-4'>
          <div className='grid grid-cols-1 gap-4'>
            <div>
              ...
              <Select value={selectedPrinter} onValueChange={setSelectedPrinter}>
                ...
                <SelectContent>
                  {printers.map((printer, index) => (<SelectItem key={index} value={printer.name}>
                      {printer.name}
                    </SelectItem>))}
                </SelectContent>
              </Select>
              {!selectedPrinter && (<p className='mt-1 text-xs text-red-500'>
            ...
            <div className='grid grid-cols-2 gap-4'>
              <div>
                ...
                <Select value={selectedPrinterType} onValueChange={setSelectedPrinterType}>
                  <SelectTrigger>
          ...
          <div className='flex justify-between'>
            ...
            <Button disabled={!selectedPrinter} onClick={() => selectedPrinter && ultimateThermalTestPrint(selectedPrinter)} className='border-purple-300 bg-gradient-to-r from-purple-50 to-pink-50 font-bold text-purple-800 shadow-md hover:from-purple-100 hover:to-pink-100'>
      ...
      <Dialog open={troubleshootingOpen} onOpenChange={setTroubleshootingOpen}>
        <DialogContent className='max-w-2xl' aria-describedby='troubleshooting-description'>
          <DialogHeader>
            <DialogTitle>{troubleshootingContent.title}</DialogTitle>
            <DialogDescription id='troubleshooting-description' className={troubleshootingContent.errorMessage ? 'text-red-500' : ''}>
          ...
          <Tabs defaultValue='steps'>
            ...
            <TabsContent value='steps' className='mt-2'>
              <ScrollArea className='h-[300px] rounded-md border p-4'>
                <div className='space-y-2 whitespace-pre-wrap font-mono text-sm'>
                  {troubleshootingContent.steps.map((step, index) => (<div key={index}>{step}</div>))}
                </div>
              </ScrollArea>
            </TabsContent>
            ...
            troubleshootingContent.suggestions.length > 0 ? (troubleshootingContent.suggestions.map((suggestion, index) => (<div key={index} className='whitespace-pre-wrap font-mono text-sm'>
                          {suggestion}
