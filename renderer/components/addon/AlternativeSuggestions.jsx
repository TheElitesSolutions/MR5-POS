...
import { Users, DollarSign, Tag, Star, CheckCircle, Lightbulb, RefreshCw, } from 'lucide-react';
 ...
 */
const AlternativeCard = ({ alternative, requestedQuantity, onSelect, isSelected = false, disabled = false, }) => {
    ...
    return (<Card className={cn('cursor-pointer border-2 transition-all duration-200 hover:shadow-md', isSelected
            ? 'border-primary bg-primary/5'
            : 'border-border hover:border-primary/50', disabled && 'cursor-not-allowed opacity-50')} onClick={() => !disabled && onSelect(addon, requestedQuantity)} role='button' tabIndex={disabled ? -1 : 0} onKeyDown={e => {
        ...
        <div className='space-y-3'>
          ...
          <div className='flex items-start justify-between'>
            <div className='flex-1'>
              ...
              {addon.description && (<p className='mt-0.5 line-clamp-2 text-xs text-muted-foreground'>
                  {addon.description}
                </p>)}
            </div>
          ...
          <div className='flex items-center gap-2'>
            ...
            {/* Stock status */}
            <MemoizedEnhancedStockIndicator addon={addon} required={requestedQuantity} variant='compact' size='sm'/>
          </div>
          ...
          <div className='flex items-center justify-between text-sm'>
            <div className='flex items-center gap-2'>
              ...
              {isPriceDifferent && (<Badge variant='outline' className={cn('text-xs', priceDifference > 0
          ...
          {/* Selection indicator */}
          {isSelected && (<div className='flex items-center justify-center rounded-md bg-primary/10 py-2'>
 ...
 */
export const AlternativeSuggestions = ({ originalAddon, requestedQuantity, onAlternativeSelect, onClose, className, trigger, autoOpen = false, maxSuggestions = 5, }) => {
    ...
    const handleSelect = useCallback((addon, quantity) => {
    ...
    const defaultTrigger = (<Button variant='outline' size='sm' className='h-8'>
    ...
    return (<Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className={cn('max-h-[80vh] max-w-2xl overflow-hidden', className)}>
        <DialogHeader className='pb-2'>
          <div className='flex items-start justify-between'>
            ...
            <Button variant='ghost' size='sm' onClick={loadAlternatives} disabled={isLoading} className='ml-2'>
        ...
        <ScrollArea className='flex-1'>
          <div className='space-y-4 pr-4'>
            {/* Loading state */}
            {isLoading && alternatives.length === 0 && (<div className='flex items-center justify-center py-8'>
                <RefreshCw className='mr-2 h-6 w-6 animate-spin'/>
                <span>Finding alternatives...</span>
              </div>)}

            {/* No alternatives found */}
            {!isLoading && alternatives.length === 0 && (<div className='py-8 text-center text-muted-foreground'>
            ...
            {Object.entries(groupedAlternatives).map(([reason, groupAlts], groupIndex) => (<div key={reason} className='space-y-2'>
                  ...
                  <div className='grid gap-2'>
                    {groupAlts.map((alternative, index) => (<AlternativeCard key={`${reason}-${index}`} alternative={alternative} requestedQuantity={requestedQuantity} onSelect={handleSelect} isSelected={selectedAlternative?.id === alternative.addon.id} disabled={!alternative.stockStatus.canSelect(requestedQuantity)}/>))}
                  </div>
                </div>))}
          </div>
        </ScrollArea>
        ...
        <div className='flex items-center justify-between border-t pt-4'>
          ...
          <div className='flex gap-2'>
            ...
            {selectedAlternative && (<Button onClick={handleClose} className='bg-primary'>
...
export const MemoizedAlternativeSuggestions = React.memo(AlternativeSuggestions);
