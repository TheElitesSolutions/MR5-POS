...
import { Plus, Minus, AlertTriangle, Ban, Lightbulb } from 'lucide-react';
 ...
 */
export const StockAwareAddonSelector = ({ addon, isSelected, quantity, maxQuantity: propMaxQuantity = 10, onSelectionChange, onDeselect, onAlternativeSelect, disabled = false, showDescription = true, showPrice = true, showAlternatives = true, enableStockChecking = true, enableSwipeGestures = true, size = 'md', className, }) => {
    const cardRef = useRef(null);
    const [showStockDetails, setShowStockDetails] = useState(false);
    // Stock status integration
    const { status: stockStatus, isLoading: stockLoading, canSelect, maxQuantity: stockMaxQuantity, isAvailable, level: stockLevel, warningMessage, } = useAddonStockStatus(enableStockChecking ? addon : null);
    ...
    const canSelectQuantity = useCallback((requestedQuantity) => {
    ...
    const canIncreaseQuantity = useMemo(() => {
        ...
        return (newQuantity <= effectiveMaxQuantity && canSelectQuantity(newQuantity));
    }, [
        isSelected,
        canBeSelected,
        quantity,
    ...
    const handleAlternativeSelect = useCallback((alternativeAddon, alternativeQuantity) => {
    ...
    return (<Card ref={cardRef} className={cn('border-2 transition-all duration-200', isSelected
            ? 'border-primary bg-primary/5 shadow-md'
            : 'border-border hover:border-primary/50 hover:shadow-sm', !canBeSelected && 'cursor-not-allowed opacity-60', enableSwipeGestures && 'touch-manipulation', className)}>
      <CardContent className={cn(config.card, 'space-y-3')}>
        {/* Header with selection and stock status */}
        <div className='flex items-start justify-between'>
          <div className='flex flex-1 items-start gap-3'>
            <Checkbox checked={isSelected} onCheckedChange={handleSelectionToggle} disabled={!canBeSelected} className='mt-1 min-h-[44px] min-w-[44px] touch-manipulation' // Touch optimized
     aria-describedby={`addon-${addon.id}-description`}/>
            ...
            <div className='flex-1 space-y-1'>
              <div className='flex items-center gap-2'>
                ...
                {stockLevel === 'low' || stockLevel === 'critical' ? (<TooltipProvider>
                    <Tooltip>
                ...
                {stockLevel === 'out_of_stock' && (<TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
              ...
              {/* Description */}
              {showDescription && addon.description && (<p className='line-clamp-2 text-xs text-muted-foreground'>
                  {addon.description}
                </p>)}

              {/* Price display */}
              {showPrice && (<div className='flex items-center gap-2'>
                  <Badge variant='outline' className={config.badge}>
                    {formatPrice(addon.price)} each
                  </Badge>
                  {isSelected && quantity > 1 && (<Badge variant='secondary' className={config.badge}>
          ...
          {/* Stock indicator */}
          {enableStockChecking && (<MemoizedEnhancedStockIndicator addon={addon} required={quantity} variant='compact' size='sm' showRefresh={false}/>)}
        </div>

        {/* Stock warning alert */}
        {enableStockChecking && stockLevel === 'critical' && isSelected && (<Alert variant='destructive' className='py-2'>
        ...
        {enableStockChecking && stockLevel === 'out_of_stock' && (<Alert className='py-2'>
            <Ban className='h-4 w-4'/>
            <AlertDescription className='text-xs'>
              <div className='flex items-center justify-between'>
                <span>This item is currently out of stock</span>
                {showAlternatives && (<MemoizedAlternativeSuggestions originalAddon={addon} requestedQuantity={quantity || 1} onAlternativeSelect={handleAlternativeSelect} trigger={<Button variant='ghost' size='sm' className='h-6 p-1 text-xs'>
                        <Lightbulb className='mr-1 h-3 w-3'/>
                        Alternatives
                      </Button>} maxSuggestions={3}/>)}
              </div>
            </AlertDescription>
          </Alert>)}

        {/* Quantity controls (only shown when selected) */}
        {isSelected && canBeSelected && (<div className='flex items-center justify-between'>
            <div className='flex items-center gap-2'>
              <Button variant='outline' size='sm' onClick={() => handleQuantityChange(-1)} disabled={!canDecreaseQuantity} className={cn('min-h-[44px] min-w-[44px]', config.button)} // Touch optimized
         aria-label='Decrease quantity'>
              ...
              <Button variant='outline' size='sm' onClick={() => handleQuantityChange(1)} disabled={!canIncreaseQuantity} className={cn('min-h-[44px] min-w-[44px]', config.button)} // Touch optimized
         aria-label='Increase quantity'>
            ...
            {/* Price impact */}
            {showPrice && (<PriceImpactIndicator basePrice={0} addonTotal={totalPrice} finalPrice={totalPrice} variant='compact' size='sm'/>)}
          </div>)}
...
export const MemoizedStockAwareAddonSelector = React.memo(StockAwareAddonSelector, (prevProps, nextProps) => {
