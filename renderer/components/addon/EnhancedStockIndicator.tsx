...
import { StockStatus } from '@/services/addonStockService';
...
interface EnhancedStockIndicatorProps {
  ...
  className?: string;
  onAlternativeClick?: (addonId: string) => void;
  onRefresh?: () => void;
}
 ...
 */
export const EnhancedStockIndicator: React.FC<EnhancedStockIndicatorProps> = ({
  addon,
  required = 1,
  showQuantity = false,
  showIcon = true,
  ...
  const getDisplayText = (): string => {
    ...
    if (variant === 'compact') {
      if (level === 'out_of_stock') return 'N/A';
      if (showQuantity) return `${status?.currentStock || 0}`;
      return config.text;
    }
  ...
  const getAriaLabel = (): string => {
    if (!status) return 'Stock information not available';
  ...
  if (isLoading && !status) {
    return (
      <Badge
        variant='outline'
        className={cn('flex items-center', sizes.badge, sizes.gap, className)}
        role='status'
        aria-label='Loading stock information'
      >
        <RefreshCw
  ...
  if (error) {
    return (
      <Badge
        ...
        className={cn('flex items-center', sizes.badge, sizes.gap, className)}
        role='status'
        aria-label='Error loading stock information'
      >
        ...
        {showRefresh && (
          <Button
            ...
            className={cn(sizes.button, 'ml-1 p-0')}
            aria-label='Refresh stock information'
          >
  ...
  if (!addon.inventory) {
    ...
    return (
      <Badge
        ...
        className={cn('flex items-center', sizes.badge, sizes.gap, className)}
        role='status'
        aria-label='Always available - no stock tracking'
      >
  ...
  const indicatorContent = (
    <Badge
      ...
      className={cn(
        'flex cursor-default items-center font-medium transition-all duration-200',
        config.className,
        sizes.badge,
        sizes.gap,
        variant === 'detailed' && 'cursor-pointer hover:opacity-80',
        className
      )}
      role='status'
      aria-label={getAriaLabel()}
      onClick={
        variant === 'detailed' ? () => setShowDetails(!showDetails) : undefined
      }
    >
      {showIcon && (
        <IconComponent
          className={cn(sizes.icon, isLoading && 'animate-spin')}
          aria-hidden='true'
        />
      )}
      ...
      {showRefresh && !isLoading && (
        <Button
          ...
          size='sm'
          onClick={handleRefresh}
          className={cn(sizes.button, 'ml-1 p-0 hover:bg-transparent')}
          aria-label='Refresh stock information'
        >
          <RefreshCw className='h-2.5 w-2.5' />
        </Button>
      ...
      <Collapsible open={showDetails} onOpenChange={setShowDetails}>
        <CollapsibleContent>
          <div className='space-y-2 rounded-lg border bg-card p-3 text-sm'>
            ...
            {showAlternatives && level === 'out_of_stock' && (
              <div className='space-y-2'>
                <p className='text-xs font-medium'>Alternatives available:</p>
                <Button
                  variant='outline'
                  size='sm'
                  onClick={() => onAlternativeClick?.(addon.id)}
...
export const MemoizedEnhancedStockIndicator = React.memo(
