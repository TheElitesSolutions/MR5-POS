...
import { AlertCircle, CheckCircle, Info, ChevronDown, ChevronUp, AlertTriangle, Target, TrendingUp, } from 'lucide-react';
/**
 * ValidationProgress - Shows progress towards group selection requirements
 */
const ValidationProgress = ({ group, selections, className, }) => {
    const groupSelections = selections.filter(s => s.addon.addonGroupId === group.id);
    const currentCount = groupSelections.reduce((sum, sel) => sum + sel.quantity, 0);
    const minRequired = group.minSelections;
    const maxAllowed = group.maxSelections || Infinity;
    // Calculate progress percentage
    const progress = minRequired > 0 ? Math.min((currentCount / minRequired) * 100, 100) : 100;
    // Determine status
    ...
    return (<div className={cn('space-y-2', className)}>
      ...
      <div className='flex items-center justify-between'>
        <div className='flex items-center gap-2'>
          <StatusIcon className={cn('h-4 w-4', statusColor === 'success' && 'text-green-600', statusColor === 'warning' && 'text-yellow-600', statusColor === 'destructive' && 'text-red-600', statusColor === 'secondary' && 'text-muted-foreground')}/>
          <span className='text-sm font-medium'>{group.name}</span>
        </div>

        <Badge variant='outline' className={cn('text-xs', statusColor === 'success' &&
            'border-green-200 bg-green-50 text-green-700', statusColor === 'warning' &&
            'border-yellow-200 bg-yellow-50 text-yellow-700', statusColor === 'destructive' &&
      ...
      {minRequired > 0 && (<div className='space-y-1'>
          <Progress value={progress} className={cn('h-2', isOverLimit && '[&>div]:bg-red-500', isComplete && '[&>div]:bg-green-500', isPartial && '[&>div]:bg-yellow-500')}/>
 ...
 */
export const AddonValidationFeedback = ({ validationErrors, groups, allSelections, showProgress = true, showSuggestions = true, className, }) => {
    const [showDetails, setShowDetails] = useState(false);
    // Group errors by type and severity
    const errorsByType = useMemo(() => {
        const grouped = {
            critical: validationErrors.filter(e => e.severity === 'error' &&
                (e.type === 'MIN_SELECTION' || e.type === 'OUT_OF_STOCK')),
            warnings: validationErrors.filter(e => e.severity === 'warning' || e.type === 'MAX_SELECTION'),
            info: validationErrors.filter(e => e.severity !== 'error' && e.type !== 'MAX_SELECTION'),
        };
        return grouped;
    }, [validationErrors]);
    ...
    const completionStatus = useMemo(() => {
        ...
        const completedGroups = requiredGroups.filter(group => {
            const groupSelections = allSelections.filter(s => s.addon.addonGroupId === group.id);
            const totalQuantity = groupSelections.reduce((sum, sel) => sum + sel.quantity, 0);
            return totalQuantity >= group.minSelections;
        });
        return {
            total: requiredGroups.length,
            completed: completedGroups.length,
            percentage: requiredGroups.length > 0
    ...
    const suggestions = useMemo(() => {
        ...
        if (incompleteRequiredGroups.length > 0) {
            tips.push(`Complete ${incompleteRequiredGroups.length} required ${incompleteRequiredGroups.length === 1 ? 'group' : 'groups'} first`);
        }
        ...
        if (overLimitGroups.length > 0) {
            tips.push(`Remove excess items from ${overLimitGroups.length} ${overLimitGroups.length === 1 ? 'group' : 'groups'}`);
        }
        return tips;
    ...
    return (<div className={cn('space-y-4', className)}>
      ...
      {showProgress && completionStatus.total > 0 && (<div className='rounded-lg border bg-card p-4'>
      ...
      {/* Critical errors */}
      {errorsByType.critical.length > 0 && (<Alert variant='destructive'>
          <AlertCircle className='h-4 w-4'/>
          <AlertDescription>
            <div className='space-y-2'>
              <div className='font-medium'>Required selections missing</div>
              <ul className='list-inside list-disc space-y-1 text-sm'>
                {errorsByType.critical.map((error, index) => (<li key={index}>{error.message}</li>))}
              </ul>
            </div>
          </AlertDescription>
        </Alert>)}
      ...
      {/* Warnings */}
      {errorsByType.warnings.length > 0 && (<Alert>
          <AlertTriangle className='h-4 w-4'/>
          <AlertDescription>
            <div className='space-y-2'>
              <div className='font-medium'>Selection limits exceeded</div>
              <ul className='list-inside list-disc space-y-1 text-sm'>
                {errorsByType.warnings.map((error, index) => (<li key={index}>{error.message}</li>))}
              </ul>
            </div>
          </AlertDescription>
        </Alert>)}
      ...
      {showProgress && groups.length > 0 && (<Collapsible open={showDetails} onOpenChange={setShowDetails}>
          ...
          <CollapsibleContent className='mt-3 space-y-3'>
            {groups.map(group => (<ValidationProgress key={group.id} group={group} selections={allSelections}/>))}
          </CollapsibleContent>
        </Collapsible>)}
...
export const MemoizedAddonValidationFeedback = React.memo(AddonValidationFeedback);
