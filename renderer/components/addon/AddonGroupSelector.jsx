...
import { ChevronDown, ChevronUp, AlertCircle, CheckCircle, Star, } from 'lucide-react';
 ...
 */
export const AddonGroupSelector = ({ group, addons, selections, stockLevels, onSelectionChange, showGroupDescription = true, collapsible = true, defaultExpanded = true, className, }) => {
      ...
      <div className='flex flex-1 items-start gap-3'>
        <div className='flex-1'>
          ...
          <div className='mt-2 flex items-center gap-2 text-xs text-muted-foreground'>
            ...
            {sortedAddons.length > 0 && (<span>{sortedAddons.length} options</span>)}
          </div>
        </div>
          ...
          <CollapsibleContent>
            <CardContent className='pt-0'>
              <div className='space-y-3'>
                ...
                return (<MemoizedAddonSelector key={addon.id} addon={addon} isSelected={!!selection} quantity={selection?.quantity || 0} currentStock={currentStock} onSelectionChange={handleAddonSelection} onDeselect={handleAddonDeselect} showDescription={true} showPrice={true} size='md'/>);
            })}
              </div>
            </CardContent>
          </CollapsibleContent>
...
export const MemoizedAddonGroupSelector = React.memo(AddonGroupSelector, (prevProps, nextProps) => {
    // ✅ FIX: Deep compare selections to detect quantity changes
    const selectionsEqual = prevProps.selections.length === nextProps.selections.length &&
        prevProps.selections.every((prevSel, idx) => {
            const nextSel = nextProps.selections[idx];
            return (prevSel.addonId === nextSel?.addonId &&
