...
import { ChevronDown, ChevronUp, AlertCircle, CheckCircle, Star, Ban, AlertTriangle, Info, RefreshCw, } from 'lucide-react';
 ...
 */
export const StockAwareAddonGroupSelector = ({ group, addons, selections, onSelectionChange, onAlternativeSelect, showGroupDescription = true, collapsible = true, defaultExpanded = true, enableSearch = true, enableVirtualization = true, enableStockChecking = true, enableTouchGestures = true, showValidationFeedback = true, virtualizationThreshold = 20, className, }) => {
    const [isExpanded, setIsExpanded] = useState(defaultExpanded);
    const [showStockDetails, setShowStockDetails] = useState(false);
    const containerRef = useRef(null);
    // Stock checking integration
    const { checkBulkStock, bulkStockResult, hasStockWarnings, hasStockErrors, isLoading: stockLoading, refreshStockData, } = useAddonStock({
    ...
    // ✅ FIX: Remove selections from deps to prevent stale state during rapid updates
    const handleAlternativeSelect = useCallback((originalAddon, alternativeAddon, quantity) => {
        // Always work with fresh selections prop (not captured in closure)
      ...
      <div className='flex flex-1 items-start gap-3'>
        <div className='flex-1'>
          ...
          <div className='flex flex-wrap items-center gap-2'>
            ...
            {enableStockChecking && groupStockStatus.hasIssues && (<Badge variant='outline' className={cn('text-xs', groupStockStatus.status === 'errors'
                ? 'border-red-200 bg-red-50 text-red-700'
                : 'border-yellow-200 bg-yellow-50 text-yellow-700')}>
                {groupStockStatus.status === 'errors' ? (<Ban className='mr-1 h-3 w-3'/>) : (<AlertTriangle className='mr-1 h-3 w-3'/>)}
                Stock Issues
              </Badge>)}

            {/* Stock refresh button */}
            {enableStockChecking && (<Button variant='ghost' size='sm' onClick={refreshStockData} disabled={stockLoading} className='h-6 w-6 p-0' title='Refresh stock data'>
                <RefreshCw className={cn('h-3 w-3', stockLoading && 'animate-spin')}/>
              </Button>)}
          </div>
          ...
          <div className='mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground'>
            ...
            {enableStockChecking && groupStockStatus.hasIssues && (<Button variant='ghost' size='sm' onClick={() => setShowStockDetails(!showStockDetails)} className='h-auto p-0 text-xs underline hover:no-underline'>
    ...
    const renderAddonList = () => {
        if (enableVirtualization &&
            ...
            return (<VirtualAddonList addons={displayAddons} selections={selections} stockLevels={new Map()} // Would be populated from stock service
             onSelectionChange={handleAddonSelection} onDeselect={handleAddonDeselect} itemHeight={120} containerHeight={400} emptyMessage='No add-ons found' isLoading={isSearching || stockLoading}/>);
        }
        ...
        {displayAddons.map((addon, index) => {
              ...
              <MemoizedStockAwareAddonSelector addon={addon} isSelected={!!selection} quantity={selection?.quantity || 0} onSelectionChange={handleAddonSelection} onDeselect={handleAddonDeselect} onAlternativeSelect={handleAlternativeSelect} showDescription={true} showPrice={true} showAlternatives={true} enableStockChecking={enableStockChecking} enableSwipeGestures={enableTouchGestures} size='md'/>
            </div>);
            })}
      </div>);
    };
        ...
        <Collapsible open={isExpanded} onOpenChange={setIsExpanded}>
          ...
          <CollapsibleContent>
            <CardContent className='space-y-4 pt-0'>
              {/* Stock details */}
              {enableStockChecking && showStockDetails && bulkStockResult && (<div className='space-y-2 rounded-lg bg-muted/50 p-3'>
                  <div className='flex items-center gap-2 text-sm font-medium'>
                    <Info className='h-4 w-4'/>
                    Stock Status Details
                  </div>

                  {bulkStockResult.warnings.map((warning, index) => (<Alert key={index} className='py-2'>
                  ...
                  {bulkStockResult.blockedSelections.length > 0 && (<Alert variant='destructive' className='py-2'>
...
export const MemoizedStockAwareAddonGroupSelector = React.memo(StockAwareAddonGroupSelector, (prevProps, nextProps) => {
