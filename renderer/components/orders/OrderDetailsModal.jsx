...
const OrderDetailsModal = ({ order, isOpen, onClose, onUpdateStatus = null, onPrintReceipt = null, showActions = true, }) => {
      ...
      <DialogContent className='max-h-[90dvh] max-w-2xl overflow-y-auto'>
        ...
        <div className='space-y-4'>
          ...
          <div className='flex justify-end'>
            <Button variant='outline' size='sm' onClick={() => setPrintPreviewMode(!printPreviewMode)} className='flex items-center space-x-2'>
          ...
          {printPreviewMode ? (<div className='mt-4'>
              <InvoicePreview order={order}/>
            </div>) : (<div className='rounded-lg border border-gray-300 bg-white font-mono text-black shadow-inner' style={{
                maxWidth: '80mm',
                margin: '0 auto',
                fontSize: '12px',
              ...
              <div className='text-center' style={{ marginBottom: '8px' }}>
                ...
                <div style={{ marginBottom: '0px' }}>
                  <img src='/logo.png' alt='Restaurant Logo' style={{
                width: '150px',
                height: '60px',
                margin: '0 auto',
                display: 'block',
              ...
              {(order.type === 'DELIVERY' ||
                (order.type === 'TAKEOUT' &&
                    (order.customerName ||
                        order.customerPhone ||
                        order.deliveryAddress))) && (<div style={{ marginTop: '10px' }}>
                  ...
                  {order.customerName && (<div style={{ fontSize: '14px', marginBottom: '5px' }}>
                      <strong>Name</strong> {order.customerName}
                    </div>)}

                  {order.customerPhone && (<div style={{ fontSize: '14px', marginBottom: '5px' }}>
                  ...
                  {order.type === 'DELIVERY' && (<>
                      ...
                      {order.deliveryInstructions && (<div style={{
                            fontSize: '13px',
                            marginBottom: '10px',
                            fontStyle: 'italic',
                        }}>
              ...
              {/* Items */}
              {!order.items || order.items.length === 0 ? (<div style={{
                    textAlign: 'center',
                    fontSize: '13px',
                    padding: '20px 0',
                    fontStyle: 'italic',
                ...
                const unitPrice = item.unitPrice || (item.totalPrice || 0) / qty || 0;
          ...
          {showActions && (<>
              ...
              <div className='flex items-center space-x-3'>
                {onPrintReceipt && (<Button variant='outline' onClick={() => onPrintReceipt(order.id)} className='flex items-center space-x-2'>
                    <Printer className='h-4 w-4'/>
                    <span>Print Invoice</span>
                  </Button>)}

                {onUpdateStatus && order.status.toLowerCase() === 'pending' && (<>
                    <Button variant='default' onClick={() => onUpdateStatus &&
                    onUpdateStatus(order.id, OrderStatus.COMPLETED)} className='flex items-center space-x-2'>
                      <CheckCircle className='h-4 w-4'/>
                      <span>Mark Complete</span>
                    </Button>

                    <Button variant='destructive' onClick={() => onUpdateStatus &&
                    onUpdateStatus(order.id, OrderStatus.CANCELLED)} className='flex items-center space-x-2'>
