...
import { CheckCircle, XCircle, Users, UtensilsCrossed, Clock, DollarSign, } from "lucide-react";
const RecentActivity = () => {
    ...
    return (<Card>
      ...
      <CardContent>
        <div className="space-y-4">
          {data.recentActivity.map((activity) => (<div key={activity.id} className="flex items-start space-x-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                {activity.metadata && (<div className="flex items-center space-x-4 text-xs text-gray-500">
                    {activity.metadata.amount && (<div className="flex items-center space-x-1">
                        <DollarSign className="w-3 h-3"/>
                        <span>${activity.metadata.amount}</span>
                      </div>)}
                    {activity.metadata.tableNumber && (<div className="flex items-center space-x-1">
                        <Users className="w-3 h-3"/>
                        <span>Table {activity.metadata.tableNumber}</span>
                      </div>)}
                    {activity.metadata.orderId && (<span>#{activity.metadata.orderId}</span>)}
                  </div>)}
              </div>
            </div>))}
        </div>
