...
import { DollarSign, TrendingUp, TrendingDown, Trash2, AlertTriangle, PieChart, } from "lucide-react";
const ProfitWasteMetrics = () => {
    ...
    const totalWasteValue = wasteMetrics.reduce((sum, waste) => sum + waste.value, 0);
    const profitMarginColor = netProfitData.profitMargin >= 30
        ? "text-green-600"
    ...
    return (<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Badge variant="outline" className={`${profitMarginColor} border-current`}>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="space-y-3">
                ...
                <Progress value={(netProfitData.breakdown.foodCosts /
            ...
            100} className="h-2"/>
              ...
              <div className="space-y-3">
                ...
                <Progress value={(netProfitData.breakdown.laborCosts /
            netProfitData.totalRevenue) *
            100} className="h-2"/>
              </div>
              ...
              <div className="space-y-3">
                ...
                <Progress value={(netProfitData.breakdown.overheadCosts /
            netProfitData.totalRevenue) *
            100} className="h-2"/>
              </div>
            </div>
            ...
            <div className="flex items-center justify-center space-x-2 pt-4 border-t">
              {netProfitData.netProfit > 0 ? (<TrendingUp className="w-4 h-4 text-green-600"/>) : (<TrendingDown className="w-4 h-4 text-red-600"/>)}
              <span className={`text-sm font-medium ${netProfitData.netProfit > 0
            ? "text-green-600"
            : "text-red-600"}`}>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {wasteMetrics.length === 0 ? (<div className="text-center py-8">
                <Trash2 className="w-12 h-12 text-gray-400 mx-auto mb-4"/>
                <p className="text-gray-500">No waste recorded</p>
              </div>) : (<>
                {wasteMetrics.map((waste) => (<div key={waste.id} className="flex items-start space-x-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                ...
                100).toFixed(1)}
                  ...
                  <Progress value={(totalWasteValue / netProfitData.totalRevenue) * 100} className="h-2 mt-2"/>
