...
const TopMenuItems = () => {
  ...
  const getProfitMarginColor = (margin: number) => {
    ...
    if (margin >= 50) return "text-yellow-600 bg-yellow-50";
      ...
      <CardContent>
        <div className="space-y-6">
          {data.topMenuItems.map((item, index) => (
            <div
              key={item.id}
              className="space-y-4 p-4 rounded-lg border hover:shadow-sm transition-shadow"
            >
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span
                    className={`text-xs font-medium ${
                      getProfitMarginColor(item.profitMargin).split(" ")[0]
                    }`}
                  >
              ...
              {index < data.topMenuItems.length - 1 && (
