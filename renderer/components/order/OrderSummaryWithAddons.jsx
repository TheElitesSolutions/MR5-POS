...
import { ChevronDown, ChevronUp, Tag, Package, Calculator, Receipt, TrendingUp, Star, Percent, } from 'lucide-react';
import { PriceImpactIndicator } from '@/components/addon/PriceImpactIndicator';
import { calculateOrderSummary, formatPrice, getMobileAddonSummary, } from '@/utils/addonFormatting';
/**
 ...
 */
export const OrderSummaryWithAddons = ({ orderItems, showTax = false, taxRate = 0.1, // 10% default
showDiscount = false, discount = 0, discountType = 'percentage', showService = false, serviceCharge = 0, showDetails = true, compactMode = false, className, onProceedToPayment, onEditOrder, paymentButtonText = 'Proceed to Payment', allowEditing = true, }) => {
    ...
    const addonBreakdown = useMemo(() => {
        ...
        return Array.from(addonGroups.values()).sort((a, b) => b.totalPrice - a.totalPrice);
    }, [orderItems]);
    ...
    return (<Card className={cn('w-full', className)}>
      <CardHeader className='pb-3'>
        <div className='flex items-center justify-between'>
          ...
          <div className='flex items-center gap-2'>
            {orderSummary.hasAddons && (<Badge variant='secondary' className='text-xs'>
      ...
      <CardContent className='space-y-4'>
        ...
        <div className='py-4 text-center'>
          <PriceImpactIndicator basePrice={orderSummary.subtotal} addonTotal={orderSummary.addonTotal} finalPrice={orderSummary.finalTotal} showBreakdown={true} variant='detailed' size='lg' className='justify-center'/>
        </div>

        {/* Order Breakdown */}
        {showDetails && (<Collapsible open={showBreakdown} onOpenChange={setShowBreakdown}>
            ...
            <CollapsibleContent className='mt-3 space-y-3'>
              ...
              <div className='space-y-2'>
                ...
                {/* Add-ons Breakdown */}
                {orderSummary.hasAddons && (<div className='space-y-2'>
                    <div className='flex items-center justify-between text-sm'>
                      <div className='flex items-center gap-2'>
                        <Tag className='h-4 w-4 text-green-600'/>
                        <span>Add-ons ({orderSummary.addonCount})</span>
                        <Button variant='ghost' size='sm' onClick={() => setShowAddonDetails(!showAddonDetails)} className='h-auto p-0 text-xs hover:bg-transparent'>
                    ...
                    {showAddonDetails && addonBreakdown.length > 0 && (<div className='ml-6 space-y-1 border-l pl-3 text-xs text-muted-foreground'>
                        {addonBreakdown.map((addon, index) => (<div key={index} className='flex justify-between'>
                            <span>
              ...
              {/* Advanced Pricing */}
              {(showDiscount || showService || showTax) && (<div className='space-y-2 border-t pt-3'>
                  {/* Discount */}
                  {showDiscount && discount > 0 && (<div className='flex items-center justify-between text-sm text-green-600'>
                      <div className='flex items-center gap-2'>
                  ...
                  {/* Service Charge */}
                  {showService && serviceCharge > 0 && (<div className='flex items-center justify-between text-sm'>
                      <div className='flex items-center gap-2'>
                  ...
                  {/* Tax */}
                  {showTax && taxRate > 0 && (<div className='flex items-center justify-between text-sm'>
                      <div className='flex items-center gap-2'>
        ...
        <div className='border-t pt-4'>
          <div className='flex items-center justify-between text-xl font-bold'>
            <span>Total</span>
            <span>
              {formatPrice(showTax || showService || showDiscount
          ...
          {/* Savings Indicator */}
          {showDiscount && advancedPricing.discountAmount > 0 && (<div className='mt-2 text-center'>
              <Badge variant='secondary' className='border-green-200 bg-green-50 text-green-600'>
        ...
        <div className='space-y-2 pt-4'>
          {onProceedToPayment && (<Button onClick={onProceedToPayment} className='w-full bg-gradient-to-r from-blue-500 to-purple-600 text-white shadow-lg hover:from-blue-600 hover:to-purple-700' size='lg'>
              <Receipt className='mr-2 h-4 w-4'/>
              {paymentButtonText}
            </Button>)}

          {allowEditing && onEditOrder && (<Button onClick={onEditOrder} variant='outline' className='w-full' size='lg'>
              Edit Order
            </Button>)}
        </div>

        {/* Mobile Add-on Summary */}
        {orderSummary.hasAddons && compactMode && (<div className='md:hidden'>
            <div className='text-xs text-muted-foreground'>
              {getMobileAddonSummary(orderItems.flatMap(item => item.addons || []))}
            </div>
          </div>)}
...
export const MemoizedOrderSummaryWithAddons = React.memo(OrderSummaryWithAddons, (prevProps, nextProps) => {
