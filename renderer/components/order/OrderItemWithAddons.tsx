...
interface OrderItemWithAddonsProps {
  orderItem: OrderItem;
  onQuantityChange?: (itemId: string, newQuantity: number) => void;
  onRemoveItem?: (itemId: string) => void;
  onEditAddons?: (itemId: string) => void;
  showActions?: boolean;
  showAddonsExpanded?: boolean;
  compact?: boolean;
  className?: string;
  disabled?: boolean;
}
 ...
 */
export const OrderItemWithAddons: React.FC<OrderItemWithAddonsProps> = ({
  orderItem,
  onQuantityChange,
  onRemoveItem,
  onEditAddons,
  showActions = true,
  showAddonsExpanded = false,
  compact = false,
  className,
  disabled = false,
}) => {
  ...
  const formattedItem = useMemo(
    () => formatOrderItemWithAddons(orderItem),
    [orderItem]
  );
  ...
  const addonTotal =
    formattedItem.addonSummary.totalAddonPrice * orderItem.quantity;
  ...
  const sortedAddons = useMemo(
    () => sortAddonsForDisplay(orderItem.addons || []),
    [orderItem.addons]
  );
  ...
  const handleQuantityChange = (delta: number) => {
    if (!onQuantityChange || disabled) return;
            ...
            <div className='flex flex-col items-end gap-2'>
              ...
              <div className='flex items-center gap-1 rounded-md bg-muted'>
                <Button
                  ...
                  disabled={disabled || orderItem.quantity <= 1}
              ...
              <div className='flex items-center gap-1'>
                {onEditAddons && (
                  <Button
                    variant='outline'
                    size='sm'
                    onClick={() => onEditAddons(orderItem.id)}
                    disabled={disabled}
                    className='h-8 px-2'
                  >
                ...
                <Button
                  ...
                  onClick={() => onRemoveItem?.(orderItem.id)}
                  disabled={disabled}
                  className='h-8 border-destructive/20 px-2 text-destructive'
                >
                  <Trash2 className='h-3 w-3' />
                </Button>
              </div>
            ...
            <Collapsible open={addonsExpanded} onOpenChange={setAddonsExpanded}>
              ...
              <CollapsibleContent>
                <div className='mt-3 space-y-2 border-l-2 border-primary/20 pl-6'>
                  {formattedItem.formattedAddons.map((group, groupIndex) => (
                    <div key={groupIndex} className='space-y-1'>
                      {formattedItem.formattedAddons.length > 1 && (
                  ...
                  {formattedItem.formattedAddons.length > 1 && (
                    <div className='mt-2 border-t border-muted pt-2'>
                      <div className='flex items-center justify-between text-sm font-medium'>
                        ...
                        <span>
                          {formatPrice(
                            formattedItem.addonSummary.totalAddonPrice
                          )}
                        </span>
                      </div>
                    </div>
...
export const MemoizedOrderItemWithAddons = React.memo(
  OrderItemWithAddons,
  (prevProps, nextProps) => {
    return (
      prevProps.orderItem.id === nextProps.orderItem.id &&
