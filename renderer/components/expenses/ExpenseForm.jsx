...
const ExpenseForm = memo(({ isOpen, onClose, expenseId }) => {
    ...
    const loadingExpenseRef = useRef(null);
    const [activeTab, setActiveTab] = useState('regular');
    // Data from new service layer - only load when needed (lazy loading)
    const shouldLoadStockData = isOpen && activeTab === 'stock';
    const { stockItems, isLoading: stockLoading, error: stockError, refresh: refreshStock, } = useStockItems({ enabled: shouldLoadStockData }); // Only load when modal is open AND on stock tab
    const { categories, isLoading: categoriesLoading, error: categoriesError, refetch: refreshCategories, } = useStockCategories(shouldLoadStockData); // Only load when modal is open AND on stock tab
    const [expenseStockItems, setExpenseStockItems] = useState([
        {
            id: undefined,
            itemName: '',
            category: '',
    ...
    useEffect(() => {
        if (process.env.NODE_ENV === 'development') {
            appLogger.debug('DEBUG: expenseStockItems state changed to:', expenseStockItems);
            if (expenseStockItems.length > 0 && expenseStockItems[0]?.itemName) {
                appLogger.debug('DEBUG: expenseStockItems has real data, first item:', expenseStockItems[0]);
            }
            else {
                appLogger.debug('DEBUG: expenseStockItems is empty or default template');
            }
        }
    }, [expenseStockItems]);
    // Calculate total amount for stock purchases (memoized)
    const calculateTotalAmount = useCallback(() => {
        return expenseStockItems.reduce((total, item) => total + item.quantity * item.costPerUnit, 0);
    }, [expenseStockItems]);
    ...
    useEffect(() => {
        if (process.env.NODE_ENV === 'development') {
            ...
            appLogger.debug('DEBUG: expenseStockItems.length:', expenseStockItems.length);
            appLogger.debug('DEBUG: expenseStockItems[0]?.itemName:', expenseStockItems[0]?.itemName);
        }
        if (expenseId &&
            activeTab === 'stock' &&
            ...
            if (process.env.NODE_ENV === 'development') {
                ...
                appLogger.debug('DEBUG: expenseStockItems for form:', JSON.stringify(expenseStockItems, null, 2));
                appLogger.debug('DEBUG: Current form values before reset:', stockForm.getValues());
            }
            const totalAmount = expenseStockItems.reduce((sum, item) => sum + item.quantity * item.costPerUnit, 0);
            if (process.env.NODE_ENV === 'development') {
            ...
            if (process.env.NODE_ENV === 'development') {
                appLogger.debug('DEBUG: Resetting form with:', JSON.stringify(resetData, null, 2));
            }
            stockForm.reset(resetData);
            if (process.env.NODE_ENV === 'development') {
                appLogger.debug('DEBUG: Form values after reset:', stockForm.getValues());
            }
        }
        else if (process.env.NODE_ENV === 'development') {
            appLogger.debug('DEBUG: Conditions not met for form update');
            if (!expenseId)
                appLogger.debug('  - No expenseId');
            if (activeTab !== 'stock')
                appLogger.debug('  - Not on stock tab');
    ...
    const updateStockItemAtIndex = (index, field, value) => {
        ...
        const newTotal = updated.reduce((total, item) => total + item.quantity * item.costPerUnit, 0);
        stockForm.setValue('totalAmount', newTotal);
    };
    // Handle adding a new category
    const handleAddNewCategory = async (itemIndex) => {
        ...
        try {
            ...
            appLogger.debug('Creating category via service layer:', newCategoryName.trim());
    ...
    const handleAddNewItem = (itemIndex) => {
        if (!newItemName.trim())
            return;
        ...
        if (expenseId &&
            ...
            const loadExpense = async () => {
                try {
                    ...
                    appLogger.debug('DEBUG: Raw expense data:', JSON.stringify(expenseData, null, 2));
                    appLogger.debug('DEBUG: Expense category:', expenseData?.category);
                    appLogger.debug('DEBUG: ExpenseCategory.INVENTORY:', ExpenseCategory.INVENTORY);
                    appLogger.debug('DEBUG: Category match:', expenseData?.category === ExpenseCategory.INVENTORY);
                    appLogger.debug('DEBUG: All expense fields:', Object.keys(expenseData || {}));
                    appLogger.debug('DEBUG: Description field:', expenseData?.description);
                    ...
                    if (expenseData) {
                        ...
                        if (expenseData.category === ExpenseCategory.INVENTORY) {
                            ...
                            appLogger.debug('DEBUG: Loading stock purchase expense, switching to stock tab');
                            ...
                            appLogger.debug('DEBUG: Description content:', expenseData.description);
                            appLogger.debug('DEBUG: Description type:', typeof expenseData.description);
                            appLogger.debug('DEBUG: Starts with Added?', expenseData.description?.startsWith('Added '));
                            appLogger.debug('DEBUG: Description length:', expenseData.description?.length);
                            appLogger.debug('DEBUG: Title content:', expenseData.title);
                            appLogger.debug('DEBUG: Notes content:', expenseData.notes);
                            appLogger.debug('DEBUG: Notes starts with Added?', expenseData.notes?.startsWith('Added '));
                            // Check description, notes, and title fields for item data
                            const itemDataSource = expenseData.description?.startsWith('Added ')
                                ? expenseData.description
                                : expenseData.notes?.startsWith('Added ')
                                    ? expenseData.notes
                                    : expenseData.title?.startsWith('Added ')
                                        ? expenseData.title
                                        : null;
                            const sourceField = itemDataSource === expenseData.description
                                ? 'description'
                                : itemDataSource === expenseData.notes
                                    ? 'notes'
                            ...
                            if (itemDataSource) {
                                try {
                                    ...
                                    appLogger.debug('DEBUG: Parsing item data:', {
                                        sourceField: itemDataSource === expenseData.description
                                            ? 'description'
                                            : 'title',
                                    ...
                                    // Parse the items
                                    appLogger.debug('DEBUG: Processing item strings:', itemStrings);
                                    for (const itemString of itemStrings) {
                                        appLogger.debug('DEBUG: Processing item string:', itemString);
                                        // Parse pattern like "5 kg of Flour"
                                        const match = itemString.match(/([\d.]+)\s+([\w]+)\s+of\s+(.+)/);
                                    ...
                                    appLogger.debug('DEBUG: Current expenseStockItems before update:', expenseStockItems);
                                    appLogger.debug('=== STATE UPDATE AUDIT ===');
                                    appLogger.debug('DEBUG: Total recovered items:', recoveredItems.length);
                                    appLogger.debug('DEBUG: Recovered items details:', JSON.stringify(recoveredItems, null, 2));
                                    if (recoveredItems.length > 0) {
                                        appLogger.debug('DEBUG: About to update state with recovered items');
                                        appLogger.debug('DEBUG: Current expenseStockItems before setState:', expenseStockItems);
                                        setExpenseStockItems(recoveredItems);
                                        appLogger.debug('DEBUG: setExpenseStockItems called successfully');
                                        // Force immediate debug of what we just set
                                        setTimeout(() => {
                                            appLogger.debug('DEBUG: State after setTimeout:', expenseStockItems);
                                        }, 100);
                                    }
                                    else {
                                        appLogger.debug('DEBUG: No items recovered - this means parsing failed');
                                        appLogger.debug('DEBUG: Creating default stock item for manual entry');
                                    }
                                }
                                catch (parseError) {
                                    appLogger.error('Error parsing stock items from description:', parseError);
                                }
                            }
                            else {
                                appLogger.debug('=== NO PARSEABLE DATA FOUND ===');
                                appLogger.debug('DEBUG: No item data found in description or title');
                                appLogger.debug('DEBUG: This might be a legacy expense or different format');
                                appLogger.debug('DEBUG: Available fields:', Object.keys(expenseData));
                                // Try to manually create a basic item for editing if this looks like a stock purchase
                                if (expenseData.category === ExpenseCategory.INVENTORY) {
                                    appLogger.debug('DEBUG: Creating empty stock item for manual editing');
                                    const defaultItem = {
                                        id: undefined,
                                    ...
                                    appLogger.debug('DEBUG: Set default stock item for editing');
                                }
                            }
    ...
    const onRegularExpenseSubmit = async (data) => {
        try {
    ...
    const onStockPurchaseSubmit = async (data) => {
        try {
            setIsSubmitting(true);
            ...
            if (!expenseId) {
                ...
                for (const item of data.items) {
                    try {
                        ...
                        if (item.isExisting) {
                            ...
                            const itemId = item.id || findStockItemByName(item.itemName)?.id;
                            if (!itemId) {
                                throw new Error(`Could not find existing item: ${item.itemName}. Please refresh the page and try again.`);
                            }
                            // Validate that itemId is a proper UUID before sending to backend
                            const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
                            if (!uuidRegex.test(itemId)) {
                                throw new Error(`Invalid item ID for "${item.itemName}". Please refresh the page and try again.`);
                            }
                        ...
                        appLogger.error(`Error processing stock item ${item.itemName}:`, itemError);
                        toast({
                            title: 'Stock Item Error',
                            description: itemError instanceof Error
                                ? itemError.message
                ...
                toast({
                    title: 'Note',
                    description: 'Expense updated. Inventory quantities remain unchanged to avoid double-counting.',
                });
            }
        ...
        <DialogContent className='max-h-[90dvh] max-w-4xl overflow-y-auto'>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className='w-full'>
            ...
            <TabsContent value='regular' className='space-y-4'>
              <Form {...expenseForm}>
                <form onSubmit={expenseForm.handleSubmit(onRegularExpenseSubmit)} className='space-y-4'>
                  <FormField control={expenseForm.control} name='description' render={({ field }) => (<FormItem>
                  ...
                  <div className='grid grid-cols-2 gap-4'>
                    <FormField control={expenseForm.control} name='amount' render={({ field }) => (<FormItem>
                          <FormLabel>Amount</FormLabel>
                          <FormControl>
                            <Input type='text' placeholder='0.00' {...field} onChange={e => {
                ...
                field.onChange(parseFloat(value) || 0);
                    ...
                    <FormField control={expenseForm.control} name='category' render={({ field }) => (<FormItem>
                          ...
                          <Select onValueChange={field.onChange} defaultValue={field.value}>
                            ...
                            <SelectContent>
                              {Object.values(ExpenseCategory).map(category => (<SelectItem key={category} value={category}>
                                  {category
                    .replace('_', ' ')
                    .toLowerCase()
                  ...
                  <FormField control={expenseForm.control} name='date' render={({ field }) => (<FormItem>
                ...
                : ''} onChange={e => field.onChange(new Date(e.target.value))}/>
                  ...
                  <FormField control={expenseForm.control} name='notes' render={({ field }) => (<FormItem>
            ...
            <TabsContent value='stock' className='space-y-4'>
              <Form {...stockForm}>
                <form onSubmit={stockForm.handleSubmit(onStockPurchaseSubmit)} className='space-y-4'>
                  <div className='space-y-4'>
                    <div className='flex items-center justify-between'>
                      <Label className='text-base font-semibold dark:text-white'>
                        Stock Items
                      </Label>
                      <Button type='button' onClick={addStockItem} size='sm' variant='outline'>
                        <Plus className='mr-1 h-4 w-4'/>
                        Add Item
                      </Button>
                    </div>

                    {expenseStockItems.map((item, index) => (<div key={index} className='space-y-3 rounded-lg border p-4'>
                        <div className='flex items-center justify-between'>
                          <div className='flex items-center gap-4'>
                            <Label className='text-sm font-medium dark:text-white'>
                              Item #{index + 1}
                            </Label>
                            <Select value={item.isExisting ? 'existing' : 'new'} onValueChange={value => {
                const isExisting = value === 'existing';
                updateStockItemAtIndex(index, 'isExisting', isExisting);
                // If changed to existing and item name is set, auto-fill fields
                if (isExisting && item.itemName) {
                    const selectedStockItem = findStockItemByName(item.itemName);
                    ...
                    updateStockItemAtIndex(index, 'id', undefined);
                    updateStockItemAtIndex(index, 'unit', '');
                    updateStockItemAtIndex(index, 'costPerUnit', 0);
                    // Keep the name and category as they might still be relevant
                          ...
                          {expenseStockItems.length > 1 && (<Button type='button' onClick={() => removeStockItem(index)} size='sm' variant='ghost' className='text-red-500 hover:text-red-700'>
                ...
                : 'New Item Category'}
                    ...
                    if (e.key === 'Enter') {
                        ...
                        setNewCategoryName('');
                                ...
                                <div className='flex gap-2'>
                                  <Button type='button' size='sm' onClick={() => handleAddNewCategory(index)} disabled={!newCategoryName.trim()}>
                                    Add Category
                                  </Button>
                    ...
                    setNewCategoryName('');
                                ...
                                <Select value={item.category} onValueChange={value => updateStockItemAtIndex(index, 'category', value)}>
                                  <SelectTrigger>
                                    <SelectValue placeholder='Select category'/>
                                  </SelectTrigger>
                                  <SelectContent>
                                    ...
                                    {categories.length === 0 && (<SelectItem value='General'>
                                        General
                                      </SelectItem>)}
                                  </SelectContent>
                                </Select>
                ...
                : 'New Item Name'}
                            ...
                            {isAddingItem ? (<div className='space-y-2'>
                                <Input placeholder='Enter new item name' value={newItemName} onChange={e => setNewItemName(e.target.value)} onKeyDown={e => {
                    if (e.key === 'Enter') {
                        e.preventDefault();
                        ...
                        setNewItemName('');
                                ...
                                <div className='flex gap-2'>
                                  <Button type='button' size='sm' onClick={() => handleAddNewItem(index)} disabled={!newItemName.trim()}>
                                    Add Item
                                  </Button>
                                  <Button type='button' size='sm' variant='outline' onClick={() => {
                    setIsAddingItem(false);
                    setNewItemName('');
                                ...
                                <Select value={item.itemName} onValueChange={value => {
                    // Find the selected stock item (check both 'name' and 'itemName')
                    const selectedStockItem = stockItems.find(stockItem => stockItem.name === value || stockItem.itemName === value);
                    if (selectedStockItem && item.isExisting) {
                        ...
                        updateStockItemAtIndex(index, 'itemName', value);
                    }
            ...
            >
                    ...
                    : 'Select item'}/>
                                  ...
                                  <SelectContent>
                                    {item.category &&
                    getFilteredItems(item.category).map(stockItem => (<SelectItem key={stockItem.id} value={stockItem.name ?? stockItem.itemName}>
                                            {stockItem.name ?? stockItem.itemName}
                                          </SelectItem>))}
                                  </SelectContent>
                                </Select>
                                <Button type='button' variant='outline' size='sm' onClick={() => setIsAddingItem(true)} className='w-full' disabled={!item.category} // Disable if no category selected
            >
                                  <Plus className='mr-2 h-4 w-4'/>
                                  Add New Item
                                </Button>
                ...
                : 'Initial Quantity'}
                            ...
                            <Controller name={`items.${index}.quantity`} control={stockForm.control} render={({ field }) => {
                // Simplified controller debug (only when values differ)
                if (field.value !== item.quantity) {
                    appLogger.debug(`Controller ${field.name} - field: ${field.value}, state: ${item.quantity}`);
                }
                return (<Input {...field} type='text' placeholder='0' value={field.value?.toString() || ''} onChange={e => {
                        ...
                        appLogger.debug(`DEBUG: Quantity onBlur - new value:`, numValue);
                        field.onChange(numValue);
                        // Also update our state for UI calculations
                        updateStockItemAtIndex(index, 'quantity', numValue);
                    }}/>);
            }}/>

                            {/* Show resulting quantity for existing items */}
                            {item.isExisting &&
                item.currentQuantity !== undefined && (<div className='flex justify-between pt-1 text-xs text-muted-foreground dark:text-gray-300'>
                                  <span>
                    ...
                    (item.quantity || 0)}{' '}
                          ...
                          <div>
                            ...
                            <Controller name={`items.${index}.unit`} control={stockForm.control} render={({ field }) => (<Select value={field.value || ''} onValueChange={value => {
                    field.onChange(value);
                    updateStockItemAtIndex(index, 'unit', value);
            ...
            >
                          ...
                          <div>
                            ...
                            <Controller name={`items.${index}.costPerUnit`} control={stockForm.control} render={({ field }) => (<Input {...field} type='text' placeholder='0.00' value={field.value || ''} onChange={e => {
                    const value = e.target.value.replace(/[^0-9.]/g, '');
                    const numValue = parseFloat(value) || 0;
                    field.onChange(numValue);
                    // Also update our state for UI calculations
                    updateStockItemAtIndex(index, 'costPerUnit', numValue);
                }}/>)}/>
                          </div>
                        </div>
                  ...
                  <div className='grid grid-cols-2 gap-4'>
                    <FormField control={stockForm.control} name='totalAmount' render={({ field: _ }) => (<FormItem>
                          <FormLabel>Total Amount</FormLabel>
                          <FormControl>
                            <Input type='text' readOnly value={totalAmount} className='bg-gray-50'/>
                          </FormControl>
                          <FormMessage />
                        </FormItem>)}/>

                    <FormField control={stockForm.control} name='date' render={({ field }) => (<FormItem>
