...
interface AddonGroupDeleteModalProps {
  open: boolean;
  onClose: () => void;
...
export function AddonGroupDeleteModal({
  open,
  onClose,
  onConfirm,
  group,
}: AddonGroupDeleteModalProps) {
    ...
    try {
      ...
      if (impact.addonsCount > 0) {
        impact.warnings.push(
          `${impact.addonsCount} add-on${impact.addonsCount !== 1 ? 's' : ''} will be permanently deleted`
        );
      }

      if (impact.categoriesCount > 0) {
        impact.warnings.push(
          `Will be removed from ${impact.categoriesCount} categor${impact.categoriesCount !== 1 ? 'ies' : 'y'}`
        );
      }
      ...
      if (impact.activeOrdersCount && impact.activeOrdersCount > 0) {
        impact.errors.push(
          `Cannot delete: ${impact.activeOrdersCount} active order${impact.activeOrdersCount !== 1 ? 's' : ''} contain this add-on group`
        );
  ...
  const handleConfirm = async () => {
    if (!group || !deletionImpact?.canSafelyDelete) return;
