...
interface AdminLayoutProps {
  ...
  currentPage?: 'overview' | 'groups' | 'addons' | 'assignments' | 'analytics';
    ...
    <nav className='space-y-2'>
      ...
      <div className='py-2'>
        ...
        <div className='space-y-1'>
          {navigationItems
            .filter(item =>
              ['groups', 'addons', 'assignments'].includes(item.id)
            )
            .map(item => (
              <Button
                key={item.id}
        ...
        <CardContent>
          <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4'>
            {quickActions.map((action, index) => (
              <Card
                key={index}
                className='cursor-pointer border-2 transition-shadow hover:border-primary/20 hover:shadow-md'
