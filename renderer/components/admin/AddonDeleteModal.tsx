...
interface DeletionImpact {
  ...
  categoryAssignmentsCount: number;
...
export function AddonDeleteModal({
  open,
  onClose,
  onConfirm,
  addon,
}: AddonDeleteModalProps) {
    ...
    try {
      ...
      if (addon.price > 0) {
        impact.warnings.push(
          `This add-on generates revenue ($${addon.price.toFixed(2)} per selection)`
        );
      }
      ...
      if (impact.inventoryLinked && impact.inventoryName) {
        impact.warnings.push(
          `Linked to inventory item: ${impact.inventoryName}`
        );
        if (impact.currentStock && impact.currentStock > 0) {
          impact.warnings.push(
            `${impact.currentStock} units in stock will remain unaffected`
          );
        }
      }
      ...
      if (impact.activeOrdersCount && impact.activeOrdersCount > 0) {
        impact.errors.push(
          `Cannot delete: ${impact.activeOrdersCount} active order${impact.activeOrdersCount !== 1 ? 's' : ''} contain this add-on`
        );
  ...
  const handleConfirm = async () => {
    if (!addon || !deletionImpact?.canSafelyDelete) return;
            ...
            deletionImpact && (
              <Card>
                ...
                <CardContent className='space-y-4'>
                  ...
                  {/* Inventory Impact */}
                  {deletionImpact.inventoryLinked && (
                    <Alert>
