...
import { Upload, Download, FileText, AlertCircle, CheckCircle2, AlertTriangle, Loader2, Package, DollarSign, Tag, } from 'lucide-react';
export function AddonBulkImportModal({ open, onClose, onImport, addonGroups, }) {
...
Small Size,Small size option,-0.50,Drink Sizes,3,true`;
    ...
    const validateImportData = useCallback(async (data) => {
        ...
        try {
            ...
            const invalid = [];
            const warnings = [];
            const activeGroupNames = new Set(addonGroups.filter(g => g.isActive).map(g => g.name));
            for (const item of items) {
                ...
                if (item.addonGroupName &&
                    ...
                    errors.push(`Add-on group "${item.addonGroupName}" not found or inactive`);
                }
                // Warnings
                if (item.price > 50) {
      ...
      <DialogContent className='max-h-[90dvh] max-w-4xl overflow-y-auto'>
        ...
        <div className='space-y-6'>
              ...
              <Card>
                ...
                <CardContent className='space-y-4'>
                  <div className='space-y-2'>
                    ...
                    <Textarea id='csvData' placeholder='name,description,price,addonGroupName,sortOrder,isActive&#10;Extra Cheese,Additional cheese topping,2.50,Pizza Toppings,1,true&#10;...' value={importData} onChange={e => setImportData(e.target.value)} rows={10} className='font-mono text-xs'/>
                    ...
                    <AlertDescription>
                      <div className='space-y-1'>
                        ...
                        <div className='flex flex-wrap gap-1'>
                          {addonGroups
                .filter(g => g.isActive)
                .map(group => (<Badge key={group.id} variant='outline' className='text-xs'>
                                {group.name}
                              </Badge>))}
                        </div>
                      </div>
          ...
          {currentStep === 'preview' && validationResult && (<>
              ...
              {/* Errors */}
              {validationResult.invalid.length > 0 && (<Card>
                  <CardHeader className='pb-3'>
                  ...
                  <CardContent>
                    <div className='max-h-40 space-y-3 overflow-y-auto'>
                      {validationResult.invalid.map((item, index) => (<Alert key={index} variant='destructive'>
                          <AlertDescription>
                            <div className='space-y-1'>
                              <div className='font-medium'>
                              ...
                              <ul className='list-inside list-disc text-sm'>
                                {item.errors.map((error, errorIndex) => (<li key={errorIndex}>{error}</li>))}
                              </ul>
                            </div>
                          </AlertDescription>
                        </Alert>))}
              ...
              {/* Warnings */}
              {validationResult.warnings.length > 0 && (<Card>
                  <CardHeader className='pb-3'>
                  ...
                  <CardContent>
                    <div className='max-h-40 space-y-3 overflow-y-auto'>
                      {validationResult.warnings.map((item, index) => (<Alert key={index}>
                          <AlertTriangle className='h-4 w-4'/>
                          <AlertDescription>
                            <div className='space-y-1'>
                              <div className='font-medium'>
                              ...
                              <ul className='list-inside list-disc text-sm text-muted-foreground'>
                                {item.warnings.map((warning, warningIndex) => (<li key={warningIndex}>{warning}</li>))}
                              </ul>
                            </div>
                          </AlertDescription>
                        </Alert>))}
              ...
              {/* Valid Items Preview */}
              {validationResult.valid.length > 0 && (<Card>
                  <CardHeader className='pb-3'>
                  ...
                  <CardContent>
                    ...
                    .slice(0, 10)
                    .map((item, index) => (<div key={index} className='flex items-center justify-between rounded bg-muted/50 p-2'>
                      ...
                      {validationResult.valid.length > 10 && (<div className='py-2 text-center text-xs text-muted-foreground'>
        ...
        <DialogFooter>
          {currentStep === 'input' && (<>
              ...
              <Button onClick={() => validateImportData(importData)} disabled={!importData.trim() || isValidating}>
                {isValidating ? (<>
          ...
          {currentStep === 'importing' && (<Button variant='outline' disabled>
              Importing...
            </Button>)}
        </DialogFooter>
      </DialogContent>
