...
interface DeletionImpact {
  ...
  categoryMenuItemsCount: number;
  addonGroupAddonsCount: number;
...
interface CategoryAssignmentDeleteModalProps {
  open: boolean;
  onClose: () => void;
...
export function CategoryAssignmentDeleteModal({
  open,
  onClose,
  onConfirm,
  assignment,
}: CategoryAssignmentDeleteModalProps) {
    ...
    try {
      ...
      if (impact.isRequiredGroup) {
        impact.warnings.push(
          `This is a required add-on group (min ${assignment.addonGroup.minSelections} selections)`
        );
      }

      if (impact.categoryMenuItemsCount > 0) {
        impact.warnings.push(
          `${impact.categoryMenuItemsCount} menu item${impact.categoryMenuItemsCount !== 1 ? 's' : ''} in "${assignment.category.name}" will lose access to "${assignment.addonGroup.name}" add-ons`
        );
      }

      if (impact.addonGroupAddonsCount > 0) {
        impact.warnings.push(
          `${impact.addonGroupAddonsCount} add-on${impact.addonGroupAddonsCount !== 1 ? 's' : ''} will no longer be available for this category`
        );
      }

      if (assignment.isActive) {
        impact.warnings.push(
          'Currently active and visible to customers in POS'
        );
      }

      // Add contextual warnings based on category/group combination
      if (
        assignment.category.name.toLowerCase().includes('pizza') &&
        assignment.addonGroup.name.toLowerCase().includes('topping')
      ) {
        impact.warnings.push(
          'This appears to be a core pizza-topping assignment that customers expect'
        );
        impact.customerImpact = 'high';
      }

      if (
        assignment.category.name.toLowerCase().includes('drink') &&
        assignment.addonGroup.name.toLowerCase().includes('size')
      ) {
        impact.warnings.push(
          'Size options are typically essential for drink categories'
        );
        impact.customerImpact = 'high';
      }
      ...
      if (impact.activeOrdersCount > 0) {
        impact.errors.push(
          `Cannot delete: ${impact.activeOrdersCount} active order${impact.activeOrdersCount !== 1 ? 's' : ''} contain items from this category with selected add-ons`
        );
      ...
      if (impact.customerImpact === 'high') {
        impact.warnings.push(
          '⚠️ High customer impact - this change may significantly affect the ordering experience'
        );
      }
  ...
  const handleConfirm = async () => {
    if (!assignment || !deletionImpact?.canSafelyDelete) return;
      ...
      <DialogContent className='max-w-lg'>
              ...
              <div className='space-y-2'>
                <div className='flex items-center justify-between'>
                  ...
                  <div className='flex items-center gap-2'>
                    ...
                    <Badge
                      variant={
                        assignment.category.isActive ? 'default' : 'secondary'
                      }
                    >
              ...
              <div className='space-y-2'>
                <div className='flex items-center justify-between'>
                  ...
                  <div className='flex items-center gap-2'>
                    ...
                    <Badge
                      variant={
                        assignment.addonGroup.isActive ? 'default' : 'secondary'
                      }
                    >
            ...
            deletionImpact && (
              <Card>
                ...
                <CardContent className='space-y-4'>
                  ...
                  <Alert
                    className={getImpactColor(deletionImpact.customerImpact)}
                  >
                  ...
                  {/* Errors */}
                  {deletionImpact.errors.length > 0 && (
                    <Alert variant='destructive'>
                      ...
                      <AlertDescription>
                        <div className='space-y-1'>
                          ...
                          <ul className='list-inside list-disc space-y-1 text-sm'>
                            {deletionImpact.errors.map((error, index) => (
                  ...
                  {deletionImpact.canSafelyDelete && (
          ...
          {deletionImpact?.canSafelyDelete && (
            <Alert variant='destructive'>
              ...
              <AlertDescription>
                ...
                <br />
                {deletionImpact.isRequiredGroup && (
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={
              isAnalyzing || isDeleting || !deletionImpact?.canSafelyDelete
            }
          >
            {isDeleting ? (
              <>
