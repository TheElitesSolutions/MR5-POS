...
import { AlertCircle, Info, Package, DollarSign, Warehouse, Plus, Trash2, } from 'lucide-react';
...
export function AddonFormModal({ open, onClose, onSubmit, initialData, title, mode, addonGroups, inventoryItems, }) {
    // State for inventory selection
    const [selectedCategory, setSelectedCategory] = useState('');
    const [selectedInventoryId, setSelectedInventoryId] = useState('');
    const [inventoryQuantity, setInventoryQuantity] = useState(''); // Use string for better UX
    ...
    const { fields: inventoryFields, append: appendInventory, remove: removeInventory, update: updateInventory, } = useFieldArray({
        control,
        name: 'inventoryItems',
    });
    // Watch form values
    ...
    const handleAddInventoryItem = () => {
        ...
        const existingIndex = (inventoryFields || []).findIndex(f => f && f.inventoryId === selectedInventoryId);
        if (existingIndex >= 0) {
            // Update quantity
    ...
    const selectedAddonGroup = addonGroups.find(group => group.id === addonGroupId);
      ...
      <DialogContent className='max-h-[80vh] max-w-3xl overflow-hidden flex flex-col'>
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className='flex flex-col flex-1 overflow-hidden'>
          ...
          <Card>
            ...
            .map(cat => (<SelectItem key={cat} value={cat}>
                          {cat}
                        </SelectItem>))}
                  </SelectContent>
                </Select>

                <Select value={selectedInventoryId} onValueChange={setSelectedInventoryId} disabled={!selectedCategory}>
                  <SelectTrigger>
                    <SelectValue placeholder='Select item'/>
                  </SelectTrigger>
                  <SelectContent>
                    {categoryItems && categoryItems.length > 0 ? (categoryItems
            .filter(item => item &&
            item.id &&
            item.id.trim() !== '' &&
            ...
            .map(item => (<SelectItem key={item.id} value={item.id}>
                            {item.itemName}
                          </SelectItem>))) : (<SelectItem value='no-items' disabled>
                        {selectedCategory ? 'No items in this category' : 'Select a category first'}
                      </SelectItem>)}
                  </SelectContent>
                </Select>

                <Input type='number' min='0' step='0.01' value={inventoryQuantity} onChange={e => setInventoryQuantity(e.target.value)} onFocus={e => e.target.select()} placeholder='Qty (0 = track only)'/>

                <Button type='button' onClick={handleAddInventoryItem} disabled={!selectedInventoryId}>
              ...
              {/* Items List */}
              {inventoryFields.length > 0 && (<div className='space-y-2'>
                ...
                const item = (categoryItems || []).find(i => i && i.id === field.inventoryId) ||
                    (inventoryItems || []).find(i => i && i.id === field.inventoryId);
                return (<div key={field.id} className='flex items-center justify-between rounded border p-2'>
                        ...
                        <Button type='button' variant='ghost' size='sm' onClick={() => removeInventory(index)}>
              ...
              {inventoryFields.length === 0 && (<Alert>
            ...
            <CardContent className='space-y-3'>
              <div className='grid grid-cols-1 gap-3 md:grid-cols-2'>
                ...
                <div className='space-y-2'>
                  ...
                  <Input id='name' placeholder='e.g., Extra Cheese, Large Size' {...register('name')} className={errors.name ? 'border-destructive' : ''}/>
                  {errors.name && (<p className='flex items-center gap-1 text-sm text-destructive'>
                ...
                <div className='space-y-2'>
                  ...
                  <div className='relative'>
                    ...
                    <Input id='price' type='number' step='0.01' min='0' max='999.99' placeholder='0.00' {...register('price', { valueAsNumber: true })} onFocus={e => e.target.select()} className={`pl-9 ${errors.price ? 'border-destructive' : ''}`}/>
                  </div>
                  {errors.price && (<p className='flex items-center gap-1 text-sm text-destructive'>
                      <AlertCircle className='h-3 w-3'/>
                      {errors.price.message}
                    </p>)}
                  {price > 0 && (<p className='text-xs text-muted-foreground'>
              ...
              <div className='space-y-2'>
                ...
                <Textarea id='description' placeholder='Brief description of this add-on...' {...register('description')} className={errors.description ? 'border-destructive' : ''} rows={2}/>
                {errors.description && (<p className='flex items-center gap-1 text-sm text-destructive'>
                    <AlertCircle className='h-3 w-3'/>
          ...
          <Card>
            ...
            <CardContent className='space-y-3'>
              ...
              <div className='space-y-2'>
                ...
                <Select value={addonGroupId} onValueChange={value => setValue('addonGroupId', value)}>
          ...
          <Card>
            ...
            : 'Hidden from POS interface'}
                ...
                <div className='space-y-2'>
                  ...
                  <Input id='sortOrder' type='number' min='0' max='999' {...register('sortOrder', { valueAsNumber: true })} onFocus={e => e.target.select()} className={errors.sortOrder ? 'border-destructive' : ''}/>
