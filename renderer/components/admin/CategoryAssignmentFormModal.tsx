...
type CategoryAssignmentFormData = z.infer<typeof categoryAssignmentSchema>;
...
interface CategoryAssignmentFormModalProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: CategoryAssignmentFormData) => Promise<void>;
...
export function CategoryAssignmentFormModal({
  ...
  const [selectedAddonGroup, setSelectedAddonGroup] =
    useState<AddonGroup | null>(null);
  ...
  const {
    ...
    watch,
    setValue,
  } = useForm<CategoryAssignmentFormData>({
  ...
  useEffect(() => {
    ...
    if (categoryId && addonGroupId) {
      ...
      const categoryAssignments = existingAssignments.filter(
  ...
  const onFormSubmit = async (data: CategoryAssignmentFormData) => {
    try {
      await onSubmit(data);
      reset();
    } catch (error) {
  ...
  const canSubmit =
    categoryId && addonGroupId && validationWarnings.length === 0;
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className='space-y-6'>
          ...
          <Card>
            ...
            <CardContent className='space-y-4'>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Select
                        ...
                        disabled={mode === 'edit'} // Don't allow changing category in edit mode
                      >
                        <SelectTrigger
                          className={
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Select
                        value={field.value}
                        onValueChange={field.onChange}
                        disabled={mode === 'edit'} // Don't allow changing group in edit mode
                      >
                        <SelectTrigger
                          className={
                            errors.addonGroupId ? 'border-destructive' : ''
                          }
                        >
              ...
              {/* Selection Preview */}
              {selectedCategory && selectedAddonGroup && (
              ...
              {validationWarnings.length > 0 && (
                <Alert
                  variant={
                    validationWarnings.some(w => w.includes('already assigned'))
                      ? 'destructive'
                      : 'default'
                  }
                >
          ...
          <Card>
            ...
            <CardContent className='space-y-4'>
              ...
              {/* Assignment Impact */}
              {selectedCategory && selectedAddonGroup && isActive && (
                <Alert>
                  ...
                  <AlertDescription>
                    <div className='space-y-1'>
                      ...
                      <div className='text-sm'>
                        ...
                        {selectedAddonGroup.minSelections > 0 && (
