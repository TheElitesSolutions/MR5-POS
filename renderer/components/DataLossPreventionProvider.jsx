 ...
 */
export default function DataLossPreventionProvider({ children, }) {
    const { hasUnsavedChanges, savePendingChanges, currentOrder, orderChanges, restoreFromLocalStorage, } = usePOSStore();
    ...
    useEffect(() => {
        ...
        const handleBeforeUnload = async (event) => {
            if (hasUnsavedChanges && currentOrder?.id) {
                ...
                try {
                    ...
                    console.error('❌ APP CLOSING: Failed to save pending changes', error);
                    event.returnValue =
        ...
        const checkForDataRecovery = () => {
            ...
            if (backup && backup.newItems?.length > 0) {
                toast({
                    ...
                    description: 'We found unsaved order changes from a previous session. They have been restored.',
                    duration: 5000,
                });
            }
        };
        ...
        const handleUnhandledRejection = (event) => {
            if (event.reason?.message?.includes('save') ||
                ...
                if (currentOrder?.id && orderChanges.has(currentOrder.id)) {
                    ...
                    toast({
                        ...
                        description: 'Your order changes have been backed up locally for recovery.',
                        variant: 'destructive',
                        duration: 8000,
                    });
                }
            ...
            if (hasUnsavedChanges && currentOrder?.id) {
                ...
                if (!fetchInProgress) {
                    ...
                    savePendingChanges(currentOrder.id, { silent: true, maxRetries: 1 })
                        .then(success => {
                        if (!success) {
                            console.warn('⚠️ PERIODIC AUTO-SAVE: Failed to save, changes backed up');
                        }
                    })
                        .catch(error => {
