...
const DateRangeFilter = ({ dateRange, onDateRangeChange, }) => {
    ...
    return (<div className='flex flex-col space-y-2 sm:flex-row sm:items-center sm:space-x-2 sm:space-y-0'>
      <div className='flex space-x-1'>
        <Button variant={dateRange.preset === 'today' ? 'default' : 'outline'} size='sm' onClick={() => handlePresetChange('today')}>
          Today
          {dateRange.preset === 'today' && <Check className='ml-1 h-4 w-4'/>}
        </Button>
        <Button variant={dateRange.preset === 'week' ? 'default' : 'outline'} size='sm' onClick={() => handlePresetChange('week')}>
          Week
          {dateRange.preset === 'week' && <Check className='ml-1 h-4 w-4'/>}
        </Button>
        <Button variant={dateRange.preset === 'month' ? 'default' : 'outline'} size='sm' onClick={() => handlePresetChange('month')}>
          Month
          {dateRange.preset === 'month' && <Check className='ml-1 h-4 w-4'/>}
        </Button>
        <Button variant={dateRange.preset === 'year' ? 'default' : 'outline'} size='sm' onClick={() => handlePresetChange('year')}>
          Year
      ...
      <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
        <PopoverTrigger asChild>
          <Button variant={dateRange.preset === 'custom' ? 'default' : 'outline'} size='sm' className='ml-auto flex items-center'>
        ...
        }} numberOfMonths={2} className='rounded-md border'/>
        </PopoverContent>
      </Popover>
    </div>);
};
