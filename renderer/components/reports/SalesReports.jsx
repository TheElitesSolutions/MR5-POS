...
import { DollarSign, ShoppingCart, TrendingUp, Download, Printer, Package, AlertCircle, } from 'lucide-react';
import { useReportsStore } from '@/stores/reportsStore';
import { useToast } from '@/hooks/use-toast';
const SalesReports = () => {
    const { salesReport, fetchSalesReport, exportSalesReport, dateRange, isLoading, error } = useReportsStore();
        ...
        <CardContent>
          <div className='overflow-x-auto'>
            <table className='w-full'>
              ...
              <tbody>
                {salesReport.orders.length === 0 ? (<tr>
                    <td colSpan={6} className='p-8 text-center text-gray-500'>
                      No orders found for the selected period
                    </td>
                  </tr>) : (salesReport.orders.map((order) => (<tr key={order.id} className='border-b hover:bg-gray-50 dark:hover:bg-gray-800'>
                      ...
                      <td className='p-3 hidden lg:table-cell'>
                        <span className={`inline-flex rounded-full px-2 py-1 text-xs font-medium ${order.type === 'DINE_IN'
