...
const CategoryManagement = ({ isOpen, onClose, onCategoryUpdated }) => {
    // Data from new service layer - automatically cached and deduplicated
    const { categories, isLoading, error, refetch: refreshCategories, clearError, } = useStockCategories();
    ...
    const getCategoryUsageColor = (usageCount) => {
        if (usageCount === 0)
    ...
    return (<>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className='max-h-[80vh] max-w-2xl overflow-y-auto'>
          ...
          {/* Error Display */}
          {error && (<div className='rounded-lg border border-red-200 bg-red-50 p-3'>
          ...
          <div className='space-y-4'>
            {/* Add/Edit Form */}
            {(isAddingCategory || editingCategory) && (<div className='rounded-lg border bg-gray-50 p-4'>
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4'>
                    ...
                    : 'New Category Name'}
                          ...
                          <FormControl>
                            <Input placeholder='e.g., Fresh Produce, Cleaning Supplies' {...field} disabled={isSubmitting}/>
                          </FormControl>
                          <FormMessage />
                        </FormItem>)}/>
                    <div className='flex gap-2'>
                      ...
                      <Button type='button' variant='outline' onClick={handleCancelEdit} disabled={isSubmitting} size='sm'>
                        Cancel
                      </Button>
                    </div>
                  </form>
            ...
            {/* Add Category Button */}
            {!isAddingCategory && !editingCategory && (<Button onClick={handleAddCategory} className='w-full' variant='outline'>
            ...
            <div className='space-y-2'>
              ...
              {categories.length === 0 ? (<div className='flex flex-col items-center justify-center py-8 text-center'>
                  ...
                  {categories.map(category => (<div key={category} className='flex items-center justify-between rounded-lg border bg-white p-3 shadow-sm'>
                      <div className='flex items-center gap-3'>
                        ...
                        <div>
                          ...
                          <Badge className={`ml-2 text-xs ${getCategoryUsageColor(0)}`}>
                            Unused
                          </Badge>
                        </div>
                      </div>
                      <div className='flex items-center gap-1'>
                        <Button variant='ghost' size='sm' onClick={() => handleEditCategory(category)} disabled={isSubmitting}>
                          <Edit className='h-4 w-4'/>
                        </Button>
                        <Button variant='ghost' size='sm' onClick={() => setDeletingCategory(category)} disabled={isSubmitting} className='text-red-600 hover:text-red-700'>
      ...
      <AlertDialog open={!!deletingCategory} onOpenChange={open => !open && setDeletingCategory(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => deletingCategory && handleDeleteCategory(deletingCategory)} disabled={isSubmitting} className='bg-red-600 hover:bg-red-700'>
              {isSubmitting && (<Loader2 className='mr-2 h-4 w-4 animate-spin'/>)}
              Delete Category
            </AlertDialogAction>
          </AlertDialogFooter>
