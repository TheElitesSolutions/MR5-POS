...
export function LoadingProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const getLoadingText = (path: string) => {
      ...
      if (path.includes('/pos')) return 'Loading POS...';
      if (path.includes('/menu')) return 'Loading Menu...';
      if (path.includes('/stock')) return 'Loading Stock...';
      if (path.includes('/orders')) return 'Loading Orders...';
      if (path.includes('/expenses')) return 'Loading Expenses...';
      if (path.includes('/reports')) return 'Loading Reports...';
      if (path.includes('/settings')) return 'Loading Settings...';
      if (path.includes('/login')) return 'Loading Login...';
      if (path.includes('/register')) return 'Loading Register...';
      return 'Loading...';
    };
