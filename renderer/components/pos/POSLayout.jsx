...
import { BarChart3, DollarSign, FileBarChart, LogOut, Menu, Package, Settings, ShoppingBag, User, UtensilsCrossed, } from 'lucide-react';
...
const POSLayout = ({ children }) => {
    ...
    return (<div className='flex h-full flex-col bg-background dark:bg-gray-900'>
      {/* Header */}
        ...
        <div className='flex items-center justify-between px-4 py-3 sm:px-6 sm:py-4'>
          ...
          <nav className='hidden items-center space-x-2 lg:flex'>
            ...
            return (<Link key={item.href} href={item.href}>
                    <Button variant={isActivePage(item.href) ? 'default' : 'ghost'} size='sm' className='flex items-center space-x-2'>
          ...
          <div className='flex items-center space-x-2 sm:space-x-4'>
            ...
            .map(item => {
            const IconComponent = item.icon;
            return (<Link key={item.href} href={item.href} onClick={() => setMobileMenuOpen(false)}>
                            <Button variant={isActivePage(item.href) ? 'default' : 'ghost'} size='sm' className='w-full justify-start space-x-3'>
                              <IconComponent className='h-4 w-4'/>
                              <span>{item.label}</span>
                            </Button>
                          </Link>);
                  ...
                  <div className='space-y-2 border-t pt-4'>
                    ...
                    <Button variant='ghost' size='sm' onClick={handleLogout} className='w-full justify-start space-x-3 text-red-600 hover:bg-red-50 hover:text-red-700 dark:hover:bg-red-950'>
                      <LogOut className='h-4 w-4'/>
                      <span>Logout</span>
                    </Button>
                  </div>
