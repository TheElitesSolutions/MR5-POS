...
const MenuFlow = ({ onItemAdded, selectedCategory: initialCategory, }) => {
    ...
    const [currentStep, setCurrentStep] = useState(initialCategory ? 'items' : 'categories');
    ...
    const handleItemSelect = useCallback((item, quantity) => {
        ...
        const isCustomizableValue = item.isCustomizable;
    ...
    }, [onItemAdded]); // Only depends on onItemAdded callback
    ...
    const renderCategories = () => (<div className='space-y-6'>
      <div>
      ...
      {/* Loading state */}
      {isLoading && categories.length === 0 && (<div className='py-12 text-center'>
          <div className='mx-auto mb-4 h-8 w-8 animate-spin rounded-full border-b-2 border-blue-600'></div>
          <p className='text-gray-500'>Loading categories...</p>
        </div>)}

      {/* Categories grid */}
      {!isLoading && categories.length > 0 && (<div className='grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4'>
          {categories.map(category => {
                ...
                return (<Card key={typeof category === 'string' ? category : category.id} className={cn('cursor-pointer touch-manipulation shadow-sm transition-all hover:shadow-md overflow-hidden', !categoryColor && 'bg-white dark:bg-gray-800')} style={categoryColor
                        ? {
                            backgroundColor: `${categoryColor} !important`,
                            borderColor: categoryColor,
                        }
                        : undefined} onClick={() => handleCategorySelect(categoryName)}>
                <CardContent className='p-6' style={categoryColor
                        ? { backgroundColor: categoryColor }
                        : undefined}>
                  <div className='flex items-center justify-between'>
                    <div>
                      <h3 className={cn('mb-1 text-lg font-medium', categoryColor ? 'text-white' : '')}>
                        {categoryName}
                      </h3>
                      <p className={cn('text-sm', categoryColor
                        ? 'text-white/90'
                        : 'text-gray-600 dark:text-gray-400')}>
                        {count} items available
                      </p>
                    </div>
                    <ChefHat className={cn('h-6 w-6', categoryColor ? 'text-white' : 'text-blue-600')}/>
                  </div>
                </CardContent>
              </Card>);
            })}
    ...
    const renderItems = () => (<div className='space-y-6'>
      <div className='flex items-center justify-between'>
        <div>
        ...
        <Button variant='outline' onClick={() => setCurrentStep('categories')} className='touch-manipulation'>
          Back to Categories
        </Button>
      </div>
      ...
      <div className='relative'>
        ...
        <Input placeholder='Search items...' value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className='touch-manipulation pl-10'/>
      </div>

      {/* Items Grid with Quantity Selection */}
      <div className='grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4'>
        {filteredMenuItems.map(item => (<ItemCard key={item.id} item={item} onItemSelect={handleItemSelect} stockItems={stockItems}/>))}
      </div>

      {filteredMenuItems.length === 0 && (<div className='py-12 text-center'>
          <ChefHat className='mx-auto mb-3 h-12 w-12 text-gray-300'/>
          <p className='text-gray-500'>No items found in this category</p>
        </div>)}
    </div>);
    const renderCustomization = () => (<div className='space-y-6'>
      <div className='flex items-center justify-between'>
        <div>
        ...
        <Button variant='outline' onClick={() => setCurrentStep('items')} className='touch-manipulation'>
          Back to Items
        </Button>
      </div>
      ...
      {selectedItem && (<div className='space-y-6'>
          ...
          {/* Ingredients */}
          {selectedItem.ingredients && selectedItem.ingredients.length > 0 && (<div>
              ...
              <div className='grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-4'>
                {selectedItem.ingredients.map((ingredient, index) => {
                    ...
                    const isRemoved = ingredientAdjustments[ingredientKey] ?? false;
                    const isIncluded = !isRemoved;
                    return (<Card key={`ingredient-${ingredientKey}`} className={cn('cursor-pointer touch-manipulation transition-all', isIncluded
                            ? 'border-green-500 bg-green-50 shadow-md dark:bg-green-950/30'
                            : 'border-red-300 bg-red-50 opacity-60 dark:bg-red-950/30')} onClick={() => handleIngredientToggle(ingredientKey)}>
                      <CardContent className='p-4'>
                        <div className='flex items-center space-x-3'>
                          <div className={cn('flex h-5 w-5 items-center justify-center rounded border-2', isRemoved
                            ? 'border-red-500 bg-red-500'
                            : 'border-green-500 bg-green-500')}>
                            <Check className='h-3 w-3 text-white'/>
                          </div>
                          <div className='flex-1'>
                            <p className={cn('font-medium', isRemoved && 'text-gray-500 line-through')}>
                              {ingredientName}
                            </p>
                            <p className='text-sm text-gray-500'>
          ...
          {(!selectedItem.ingredients ||
                selectedItem.ingredients.length === 0) && (<Card>
            ...
            <CardContent className='p-4'>
              <div className='flex items-center justify-between'>
                ...
                <div className='space-y-2'>
                  ...
                  <Input placeholder='e.g., Extra crispy, Well done, No salt...' value={specialNotes} onChange={e => setSpecialNotes(e.target.value)} className='w-full'/>
                </div>
                ...
                <div className='flex items-center space-x-3'>
                  ...
                  <Button onClick={() => setCurrentStep('addons')} className='flex-1' size='lg'>
    ...
    const renderAddons = () => {
        ...
        const isCustomizableItem = selectedItem.isCustomizable || hasIngredients;
        const backStep = isCustomizableItem ? 'customize' : 'items';
        return (<AddonSelectionStep selectedItem={selectedItem} itemQuantity={itemQuantity} onContinue={selections => {
...
const ItemCard = React.memo(({ item, onItemSelect, stockItems }) => {
    ...
    return (<Card className='cursor-pointer touch-manipulation border-2 shadow-sm transition-all hover:shadow-md active:scale-95 overflow-hidden' style={{
        ...
        <CardContent className='p-4' style={{
          ...
          <div className='space-y-3'>
            ...
            <div className='flex items-start justify-between'>
              ...
              <Badge variant='outline' className={cn('text-sm', itemColor ? 'border-white text-white' : '')}>
                ${item.price.toFixed(2)}
              </Badge>
            </div>

            {/* Description */}
            <p className={cn('line-clamp-2 text-sm', itemColor ? 'text-white/90' : 'text-gray-600 dark:text-gray-400')}>
              {item.description || 'No description available'}
            </p>

            {/* Ingredients Preview */}
            {item.ingredients && item.ingredients.length > 0 && (<div className={cn('text-xs', itemColor ? 'text-white/80' : 'text-gray-500')}>
                Ingredients:{' '}
                {item.ingredients
                .slice(0, 3)
            ...
            <div className={cn('border-t pt-3 text-center', itemColor
            ? 'border-white/30'
            : 'border-gray-200 dark:border-gray-700')}>
              <span className={cn('text-sm', itemColor ? 'text-white/90' : 'text-gray-600 dark:text-gray-400')}>
