...
import { ArrowLeft, Search, UtensilsCrossed, Package, Coffee, Beef, Fish, } from 'lucide-react';
...
const MenuCategoryGrid = ({ onCategorySelect, selectedCategory, }) => {
    ...
    const fetchCategoryStats = async () => {
        try {
            ...
            if (response?.success && response.data) {
                ...
                const stats = response.data.map((stat, index) => {
                    const transformed = {
    ...
    const getCategoryIcon = (category) => {
        ...
        if (lowerCategory.includes('meat') ||
        ...
        else if (lowerCategory.includes('drink') ||
            lowerCategory.includes('beverage') ||
            lowerCategory.includes('coffee')) {
            return Coffee;
        }
        else if (lowerCategory.includes('appetizer') ||
    ...
    const filteredCategories = useMemo(() => {
        return categoryStats.filter(category => category.name.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      <div className='border-b border-gray-200 p-4 dark:border-gray-700'>
        <div className='mb-4 flex items-center justify-between'>
          <div className='flex items-center space-x-3'>
            <Button variant='outline' size='sm' onClick={switchToTables} className='touch-manipulation'>
        ...
        <div className='relative'>
          ...
          <Input placeholder='Search categories...' value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className='touch-manipulation pl-10'/>
        </div>
      </div>
      ...
      <div className='flex-1 overflow-y-auto p-4'>
        <div className='grid grid-cols-1 gap-4 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-4'>
          {filteredCategories.map(category => {
            ...
            return (<Card key={category.name} className={`transform cursor-pointer touch-manipulation border-2 transition-all duration-200 hover:shadow-lg active:scale-95 ${isSelected
                    ? 'border-blue-300 bg-blue-50 ring-2 ring-blue-500 dark:border-blue-700 dark:bg-blue-950/30'
                    : 'border-gray-200 hover:border-blue-300 hover:bg-gray-50 dark:border-gray-700 dark:hover:border-blue-600 dark:hover:bg-gray-800'}`} onClick={() => onCategorySelect(category.name)}>
                ...
                <CardContent className='pt-0'>
                  <div className='space-y-2'>
                    ...
                    {category.availableItems === 0 && (<div className='mt-2 text-xs text-red-500'>
                        No items available
                      </div>)}
                  </div>
                </CardContent>
        ...
        {/* Empty State */}
        {filteredCategories.length === 0 && !isLoading && (<div className='py-12 text-center'>
            ...
            {searchTerm && (<Button variant='outline' onClick={() => setSearchTerm('')} className='mt-4 touch-manipulation'>
                Clear Search
              </Button>)}
          </div>)}
      </div>
