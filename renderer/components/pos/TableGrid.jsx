...
const TableGrid = memo(() => {
    ...
    const getTableBadgeText = (table) => {
        const status = getTableStatus(table.id);
        return status === 'occupied' ? 'In Use' : 'Available';
    };
    const formatElapsedTime = (createdAt) => {
        const now = new Date();
        const created = typeof createdAt === 'string' ? parseLocalDateTime(createdAt) : createdAt;
    ...
    const getTotalItemQuantity = (table) => {
        if (!table.activeOrder?.items)
            return 0;
        return table.activeOrder.items.reduce((total, item) => total + (item.quantity || 0), 0);
    };
    const handleSubmitNewTable = useCallback(async (e) => {
        e.preventDefault();
        if (isCreating)
            return;
        setIsCreating(true);
    ...
    const handleTogglePayLater = useCallback(async (tableId, e) => {
    ...
    return (<div className='space-y-4 sm:space-y-6'>
          ...
          <form onSubmit={handleSubmitNewTable} className='space-y-4'>
            <div className='grid gap-4 py-4'>
              <div className='grid grid-cols-4 items-center gap-4'>
                ...
                <Input id='name' type='text' placeholder='Enter name or number (e.g., Table 1, VIP, Corner)' value={newTable.name} onChange={e => setNewTable({ ...newTable, name: e.target.value })} className='col-span-3' required autoFocus/>
              </div>
            ...
            <DialogFooter>
              <Button type='button' variant='outline' onClick={() => setShowCreateDialog(false)} className='touch-manipulation'>
                Cancel
              </Button>
              <Button type='submit' disabled={!newTable.name.trim() || isCreating} className='touch-manipulation'>
      ...
      <div className='flex items-center justify-between'>
        ...
        {(permissions.isAdmin || permissions.isManager) && (<Button size='sm' className='touch-manipulation' onClick={() => setShowCreateDialog(true)}>
      ...
      <div className='grid grid-cols-1 gap-3 xs:grid-cols-2 sm:grid-cols-2 sm:gap-4 md:grid-cols-3 lg:grid-cols-4 lg:gap-6 xl:grid-cols-5 2xl:grid-cols-6'>
        {filteredTables.map(table => (<Card key={table.id} className={cn('group cursor-pointer touch-manipulation p-3 transition-all duration-200 sm:p-4', 'transform hover:shadow-lg active:scale-95', 'min-h-[140px] border-2 sm:min-h-[160px]', selectedTable?.id === table.id
                ? 'border-blue-300 bg-blue-50 ring-2 ring-blue-500 dark:border-blue-700 dark:bg-blue-950/30'
                : 'border-gray-200 hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800')} onClick={() => handleTableClick(table)}>
            <div className='flex h-full flex-col space-y-3'>
              ...
              <div className='flex items-start justify-between'>
                ...
                <div className='flex items-center gap-1'>
                  ...
                  {(permissions.isAdmin || permissions.isManager) && (<Button variant='ghost' size='icon' className={cn('h-8 w-8 flex-shrink-0', table.isPayLater
                    ...
                    : 'text-gray-400 hover:bg-blue-50 hover:text-blue-600 dark:text-gray-500 dark:hover:bg-blue-950/20 dark:hover:text-blue-400')} onClick={(e) => handleTogglePayLater(table.id, e)} title={table.isPayLater ? 'Move to Dine In tab' : 'Move to Not Paid tab'}>
                      <Timer className='h-4 w-4'/>
                    </Button>)}

                  {/* Delete button - show for all tables if user has permissions */}
                  {(permissions.isAdmin || permissions.isManager) && (<AlertDialog>
                    <AlertDialogTrigger asChild>
                      <Button variant='ghost' size='icon' className={cn('h-8 w-8 flex-shrink-0 hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-950/20 dark:hover:text-red-400', table.activeOrder
                ...
                )} onClick={e => e.stopPropagation()} title={table.activeOrder
                        ...
                        <AlertDialogDescription>
                          ...
                          {table.activeOrder ? (<>
                              ...
                              <strong className='text-red-600 dark:text-red-400'>
                                ...
                                {(table.activeOrder.totalAmount || 0).toFixed(2)}
              ...
              <div className='flex justify-start'>
                <Badge variant='outline' className={cn('px-3 py-1 text-xs font-medium', getTableBadgeColor(table))}>
                  {getTableBadgeText(table)}
                </Badge>
              </div>

              {/* Active Order Info */}
              {table.activeOrder ? (<div className='flex-1 space-y-2'>
                ...
                </div>) : (<div className='flex flex-1 items-end justify-center pb-2'>
      ...
      {/* Empty State */}
      {filteredTables.length === 0 && (<div className='py-12 text-center sm:py-16'>
          ...
          {(permissions.isAdmin || permissions.isManager) && (<Button className='mt-4 touch-manipulation' size='lg' onClick={() => setShowCreateDialog(true)}>
