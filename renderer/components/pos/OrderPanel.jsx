...
import { Plus, Minus, ShoppingCart, Check, X, Trash2, Clock, AlertCircle, FileText, ChefHat, } from 'lucide-react';
import { useAuthStore } from '@/stores/authStore';
import { useSharedStockData } from '@/context/StockDataContext';
// ✅ areAddonsEqual function now imported from @/utils/orderPanelUtils (shared with TakeoutOrderPanel)
const OrderPanel = ({ pendingCustomization, onCustomizationProcessed }) => {
    ...
    const handleQuantityAdjust = async (itemId, newQuantity, itemName) => {
        ...
        enqueueAction('update', async () => {
            ...
            if (itemAddons.length > 0 && currentOrder?.id) {
                ...
                const result = await scaleAddonsForQuantityChange(itemId, quantityChange, currentOrder.id);
    ...
    const handleRemoveItem = async (orderItemId, itemName) => {
    ...
    const handleAddCustomizedItem = async (customizationData) => {
        ...
        try {
            ...
            orderLogger.debug('Existing item search result', {
                ...
                selectedAddonIds: normalizedAddonSelections.map((a) => a.addonId),
            });
            ...
            if (existingItem) {
                ...
                const response = await addOrderItem(selectedItem.id, itemQuantity, customizations.length > 0 ? customizations : undefined, specialNotes || undefined // pass special notes for kitchen
                );
                                ...
                                try {
                                    ...
                                    if (refreshResp.success && refreshResp.data) {
                                        orderLogger.debug('Order refreshed after addon addition', {
                                            ...
                                            items: refreshResp.data.items?.map((item) => ({
                                                id: item.id,
                                                name: item.name,
                                        ...
                                        const refreshedOrder = {
                                            ...refreshResp.data,
                                            items: refreshResp.data.items ? [...refreshResp.data.items.map((item) => ({
                                                    ...item,
                                        ...
                                        updateOrderInStore(refreshedOrder, true);
                                        orderLogger.debug('Updated posStore with refreshed order (forced re-render)', {
                                            refreshedItemPrices: refreshedOrder.items.map((item) => ({
                                                id: item.id,
                                                unitPrice: item.unitPrice,
                                                totalPrice: item.totalPrice,
                                            })),
                    ...
                    try {
                        ...
                        if (orderResp.success && orderResp.data) {
                            ...
                            if (matchingItem) {
                                ...
                                orderLogger.debug('All matching strategies failed - item is in DB but not found in refresh', {
                                    actualItemId: actualItem.id,
                                    menuItemId: actualItem.menuItemId,
                                    refreshedOrderItems: refreshedOrder.items?.map((item) => ({
                                        id: item.id,
    ...
    return (<div className='flex h-full flex-col'>
                ...
                return (<Card key={item.id} className='p-4'>
                  <div className='space-y-3'>
                    <div className='flex items-start justify-between'>
                      ...
                      <Button variant='ghost' size='sm' onClick={() => handleRemoveItem(item.id, item.menuItemName || 'Unknown Item')} className='p-1 text-red-600 hover:text-red-700'>
                    ...
                    <div className='flex items-center justify-between'>
                      <div className='flex items-center space-x-1'>
                        <Button variant='outline' size='icon' className='h-7 w-7' onClick={() => handleQuantityAdjust(item.id, Math.max(1, (item.quantity || 1) - 1), item.menuItemName || 'Item')} disabled={isProcessing ||
                        ...
                        <Button variant='outline' size='icon' className='h-7 w-7' onClick={() => handleQuantityAdjust(item.id, (item.quantity || 1) + 1, item.menuItemName || 'Item')} disabled={isProcessing || updatingItemId === item.id}>
      ...
      <div className='border-t border-gray-200 p-6 dark:border-gray-700'>
        ...
        <div className='space-y-3'>
          ...
          {viewMode === 'tables' && hasChanges && (<Button onClick={async () => {
                try {
          ...
          {viewMode === 'tables' && !hasChanges && (<Button onClick={() => switchToMenu()} variant='outline' className='w-full' disabled={isLoading || isProcessing}>
              <Plus className='mr-2 h-4 w-4'/>
              Add Item
            </Button>)}
                    ...
                    try {
                        ...
                        if (user?.id) {
                            ...
                            if (defaultPrinter && hasChanges && changesSummary) {
                                ...
                                // Use shared utility to prepare kitchen print data
                                const printData = prepareKitchenPrintData(changesSummary, currentOrder.items?.length || 0);
                                const result = await printerAPI.PrinterAPI.printKitchenOrder(currentOrder.id, defaultPrinter.name, 1, user.id, false, [], // No cancelled items (removals not printed)
                                printData.updatedItemIds, printData.changeDetails);
                                if (result.success) {
              ...
              <Button onClick={async () => {
                try {
                    ...
                    if (result.success) {
                        ...
                        toast({
                            title: 'Print Failed',
                            description: result.error || 'Failed to print kitchen order',
                            variant: 'destructive',
                        });
                    }
                }
                    ...
                    toast({
                        title: 'Print Error',
                        description: 'An error occurred while printing the kitchen order',
                        variant: 'destructive',
                    });
                }
                finally {
          ...
          {viewMode === 'tables' && !hasChanges && currentOrder.items && currentOrder.items.length > 0 && (<Button onClick={handleCompleteOrder} className='w-full bg-gradient-to-r from-green-500 to-green-600 text-white shadow-md hover:from-green-600 hover:to-green-700' disabled={isLoading}>
              <Check className='mr-2 h-4 w-4'/>
              {isProcessing ? 'Completing...' : 'Complete Order'}
            </Button>)}

          {/* TERTIARY ACTION - Cancel Button (Subtle) */}
          <Button onClick={() => setIsCancelDialogOpen(true)} variant='ghost' className='w-full text-gray-600 hover:bg-red-50 hover:text-red-600 dark:text-gray-400 dark:hover:bg-red-950 dark:hover:text-red-400' size='sm' disabled={isLoading}>
