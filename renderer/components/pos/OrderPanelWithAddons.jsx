...
import { Plus, ShoppingCart, Check, X, Clock, AlertCircle, FileText, ChefHat, Tag, TrendingUp, } from 'lucide-react';
...
import { calculateOrderSummary, formatPrice, } from '@/utils/addonFormatting';
const OrderPanelWithAddons = ({ pendingCustomization, }) => {
    ...
    const { selectedTable, currentOrder, createOrder, removeOrderItem, completeOrder, cancelOrder, addOrderItem, updateOrderItem, isLoading, error, switchToMenu, switchToTables, viewMode, clearError, } = usePOSStore();
    // Data from shared context - prevents duplicate API calls
    const { stockItems, error: stockError } = useSharedStockData();
    const { toast } = useToast();
    const [startOrderError, setStartOrderError] = useState(null);
    const [isProcessing, setIsProcessing] = useState(false);
    // State for managing add-on related operations
    const [editingAddonsForItem, setEditingAddonsForItem] = useState(null);
    ...
    const { trackNewItem, trackQuantityChange, trackItemRemoval, clearTracking, hasChanges, changeCount, changesSummary, newItemsCount, updatedItemsCount, removedItemsCount, debugTrackingState, } = useSimpleOrderTracking(currentOrder?.id);
    ...
    const handleStartOrder = async () => {
        ...
        if (!selectedTable.id) {
            setStartOrderError('Invalid table selected. Please refresh and try again.');
            return;
        }
    ...
    const handleQuantityAdjust = async (itemId, newQuantity) => {
        if (newQuantity < 1)
            return;
        ...
        if (oldQuantity === newQuantity)
            return;
        const itemName = currentItem.menuItemName || currentItem.name || 'Unknown Item';
        try {
            setIsProcessing(true);
            ...
            if (currentItem.addons && currentItem.addons.length > 0) {
                ...
                await window.electron.ipc.invoke('order:updateItemQuantityWithAddons', {
                    orderItemId: itemId,
                    newQuantity,
                });
            }
            ...
            trackQuantityChange(itemId, itemName, currentItem.menuItemId, oldQuantity, newQuantity);
            toast({
    ...
    const handleRemoveItem = async (orderItemId) => {
        ...
        if (!itemToRemove)
            return;
        const itemName = itemToRemove.menuItemName || itemToRemove.name || 'Unknown Item';
        ...
        try {
            ...
            if (itemToRemove.addons && itemToRemove.addons.length > 0) {
                await window.electron.ipc.invoke('order:removeItemWithAddons', {
                    orderItemId,
                });
            }
            else {
            ...
            trackItemRemoval(orderItemId, itemName, itemToRemove.menuItemId || '', quantity);
    ...
    const handleEditAddons = async (itemId) => {
    ...
    const handleAddCustomizedItem = async (customizationData) => {
        const { selectedItem, itemQuantity, ingredientAdjustments, specialNotes, addonSelections = [], } = customizationData;
        ...
        try {
            ...
            if (addonSelections.length > 0) {
                const response = await window.electron.ipc.invoke('order:addItemWithAddons', {
                    orderId: currentOrder.id,
                ...
                const response = await addOrderItem(selectedItem.id, itemQuantity, customizations.length > 0 ? customizations : undefined, specialNotes || undefined);
                // 🔧 FIX: Don't track items that are already saved to DB
    ...
    if (!currentOrder) {
        ...
        <div className='flex flex-1 items-center justify-center p-6'>
          <div className='text-center'>
            ...
            {/* Error Display */}
            {(startOrderError || error) && (<div className='mb-4 rounded-lg border border-red-200 bg-red-50 p-3'>
            ...
            <Button onClick={handleStartOrder} size='lg' disabled={isLoading || isProcessing} className='w-full transform touch-manipulation bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-lg transition-all duration-300 ease-in-out hover:scale-105 hover:from-green-600 hover:to-emerald-700'>
      ...
      <div className='border-b border-gray-200 p-6 dark:border-gray-700'>
        ...
        <div className='flex items-center justify-between'>
          <div className='flex items-center space-x-4 text-sm text-gray-600 dark:text-gray-400'>
            ...
            {orderSummary.hasAddons && (<div className='flex items-center space-x-1'>
          ...
          <Button variant='ghost' size='sm' onClick={() => setShowDetailedSummary(!showDetailedSummary)} className='text-xs'>
        ...
        {showDetailedSummary && orderSummary.hasAddons && (<div className='mt-3 rounded-lg bg-muted/50 p-3'>
            <PriceImpactIndicator basePrice={orderSummary.subtotal} addonTotal={orderSummary.addonTotal} finalPrice={orderSummary.finalTotal} showBreakdown={true} variant='detailed' size='sm'/>
          </div>)}
      </div>

      {/* Enhanced Order Items with Add-on Support */}
      <div className='flex-1 overflow-hidden p-6'>
        {currentOrder.items && currentOrder.items.length > 0 ? (<OrderListWithAddons orderItems={currentOrder.items} onQuantityChange={handleQuantityAdjust} onRemoveItem={handleRemoveItem} onEditAddons={handleEditAddons} showSearch={currentOrder.items.length > 3} showFilters={currentOrder.items.length > 5} showSummary={false} // We handle summary in footer
         allowSorting={true} compactMode={false} maxHeight={400} showItemActions={true}/>) : (<div className='py-8 text-center'>
      ...
      <div className='border-t border-gray-200 p-6 dark:border-gray-700'>
        ...
        <Card className='mb-4 p-4'>
          <div className='space-y-2'>
            ...
            {orderSummary.hasAddons && (<div className='flex items-center justify-between text-sm'>
        ...
        <div className='space-y-2'>
          ...
          {viewMode === 'tables' && (<Button onClick={hasChanges
                ? async () => {
                ...
                : () => switchToMenu()} variant={hasChanges ? 'default' : 'outline'} className={`w-full ${hasChanges ? 'bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-lg hover:from-green-600 hover:to-emerald-700' : ''}`} size='lg' disabled={isLoading || isProcessing}>
              {hasChanges ? (<>
          ...
          {viewMode === 'menu' &&
            currentOrder.items &&
            currentOrder.items.length > 0 && (<Button onClick={async () => {
                try {
                    setIsProcessing(true);
                    ...
                    toast({
                        title: 'Order Updated',
                        description: 'Items with add-ons sent to kitchen successfully',
                    });
                }
                catch (error) {
            ...
            }} className='w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-lg hover:from-green-600 hover:to-emerald-700' size='lg' disabled={isLoading || isProcessing}>
                <Check className='mr-2 h-4 w-4'/>
          ...
          {currentOrder.items && currentOrder.items.length > 0 && (<div className='grid grid-cols-2 gap-2'>
              <Button onClick={async () => {
                ...
                try {
                    ...
                    if (user?.id) {
                        ...
                        const defaultPrinter = printers.find(p => p.isDefault) || printers[0];
                        if (defaultPrinter) {
                            const result = await printerAPI.PrinterAPI.printKitchenOrder(currentOrder.id, defaultPrinter.name, 1, user.id, false, [], [], []);
                            if (result.success) {
                                toast({
            ...
            }} variant='outline' className='bg-gradient-to-r from-amber-500 to-orange-600 text-white hover:from-amber-600 hover:to-orange-700' disabled={isLoading || isProcessing}>
            ...
            }} variant='outline' className='bg-gradient-to-r from-blue-500 to-purple-600 text-white hover:from-blue-600 hover:to-purple-700' disabled={isLoading || isProcessing}>
          ...
          {viewMode === 'tables' &&
            currentOrder.items &&
            currentOrder.items.length > 0 && (<Button onClick={handleCompleteOrder} className='w-full' size='lg' disabled={isLoading || isProcessing}>
                <Check className='mr-2 h-4 w-4'/>
                {isProcessing ? 'Completing...' : 'Complete Order'}
              </Button>)}

          <Button onClick={() => setIsCancelDialogOpen(true)} variant='outline' className='w-full' size='lg' disabled={isLoading || isProcessing}>
