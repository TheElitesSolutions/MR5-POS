...
const MenuSelector = ({ onClose }) => {
    ...
    const { menuItems, categories, isLoading: menuLoading, error: menuError, refetch, } = useAvailableMenuItems();
    // Use shared stock data from context - no direct API calls
    const { stockItems, isLoading: stockLoading, error: stockError, } = useSharedStockData();
    ...
    const getCategoryColor = (categoryName) => {
        const category = categories.find(cat => typeof cat === 'string' ? cat === categoryName : cat.name === categoryName);
        return typeof category === 'object' ? category.color : undefined;
    };
    const checkIngredientAvailability = async (menuItemId, orderQuantity) => {
        ...
        try {
            ...
            const availability = ingredients.map(ingredient => {
                const stockItem = stockItems.find(stock => stock.id === ingredient.stockItemId);
            ...
            const unavailableIngredients = availability.filter(item => !item.isAvailable);
            const lowStockIngredients = availability.filter(item => item.isLowStock && item.isAvailable);
            if (unavailableIngredients.length > 0) {
                // Show toast for unavailable ingredients
                toast({
                    title: 'Insufficient Stock',
                    description: `Not enough stock for: ${unavailableIngredients
                        .map(item => getIngredientNameSafe(item.ingredient.stockItemId, stockItems, 'Unknown'))
            ...
            toast({
                ...
                description: 'Unable to verify ingredient availability. Please try again.',
                variant: 'destructive',
            });
            return false;
        }
    ...
    const handleAddToOrder = async () => {
        ...
        if (!selectedItem || isAddingItem)
            return;
        // ✅ FIX: Disable immediately to prevent race conditions
        setIsAddingItem(true);
        try {
            // Final stock check before adding to order
            const canProceed = await checkIngredientAvailability(selectedItem.id, quantity);
            if (!canProceed) {
                return; // Don't proceed if stock is insufficient
            }
            await addOrderItem(selectedItem.id, quantity, customizations, specialInstructions || undefined);
            // Reset form
    ...
    const calculateItemTotal = () => {
        ...
        const customizationPrice = customizations.reduce((total, custom) => total + custom.priceAdjustment * quantity, 0);
        return basePrice + customizationPrice;
    };
    const getStockStatusBadge = (availability) => {
        if (!availability.isAvailable) {
          ...
          Out of Stock
            ...
            return (<Badge variant='outline' className='border-yellow-600 text-xs text-yellow-600'>
          Low Stock
        </Badge>);
        }
        else {
            return (<Badge variant='outline' className='border-green-600 text-xs text-green-600'>
          Available
        </Badge>);
        }
    };
      ...
      <DialogContent className='max-h-[90dvh] max-w-4xl overflow-hidden'>
        ...
        <div className='flex h-[70vh]'>
          ...
          <div className='flex flex-1 flex-col pr-4'>
            ...
            return (<Button key={typeof category === 'string' ? category : category.id} size='sm' variant={selectedCategory === categoryName ? 'default' : 'outline'} onClick={() => setSelectedCategory(categoryName)} style={categoryColor && selectedCategory === categoryName
                    ? {
            ...
            return (<Card key={item.id} className={cn('cursor-pointer transition-all hover:shadow-md overflow-hidden', selectedItem?.id === item.id && 'ring-2 ring-blue-500')} onClick={() => handleItemSelect(item)} style={itemColor
                    ? {
                        backgroundColor: itemColor,
                        borderColor: itemColor,
                    }
                    : undefined}>
                      <CardContent className='p-4' style={itemColor
                    ? { backgroundColor: itemColor }
                    : undefined}>
                        <div className='space-y-2'>
                          <div className='flex items-start justify-between'>
                            <h4 className={cn('text-sm font-medium', itemColor && 'text-white')}>
                              {item.name}
                            </h4>
                            <Badge variant='outline' className={cn(itemColor && 'border-white text-white')}>
                              ${item.price.toFixed(2)}
                            </Badge>
                          </div>

                          <p className={cn('line-clamp-2 text-xs', itemColor ? 'text-white/90' : 'text-gray-600')}>
                            {item.description}
                          </p>

                          <div className={cn('flex items-center justify-between text-xs', itemColor ? 'text-white/80' : 'text-gray-500')}>
              ...
              {filteredItems.length === 0 && (<div className='py-8 text-center'>
            ...
            {selectedItem ? (<div className='space-y-4'>
                <div>
                  ...
                  <div className='mt-2 flex items-center justify-between'>
                    <div className='flex items-center gap-2'>
                      {(selectedItem.color || getCategoryColor(selectedItem.category)) && (<div className='h-4 w-4 rounded-full' style={{
                    backgroundColor: selectedItem.color || getCategoryColor(selectedItem.category),
                }}/>)}
                ...
                <div>
                  ...
                  <div className='mt-1 flex items-center space-x-2'>
                    <Button size='sm' variant='outline' onClick={() => handleQuantityChange(Math.max(1, quantity - 1))} disabled={isLoadingIngredients}>
                      <Minus className='h-3 w-3'/>
                    </Button>
                    <span className='w-12 text-center'>{quantity}</span>
                    <Button size='sm' variant='outline' onClick={() => handleQuantityChange(quantity + 1)} disabled={isLoadingIngredients}>
                      <Plus className='h-3 w-3'/>
                    </Button>
                  </div>
                  {isLoadingIngredients && (<div className='mt-1 text-xs text-gray-500'>
                      Checking stock availability...
                    </div>)}
                </div>

                {/* Ingredient Availability */}
                {ingredientAvailability.length > 0 && (<div>
                    <Label>Ingredient Availability</Label>
                    <div className='mt-1 max-h-32 space-y-2 overflow-y-auto'>
                      {ingredientAvailability.map((availability, index) => (<div key={index} className='flex items-center justify-between rounded bg-gray-50 p-2 text-xs'>
                          <span className='font-medium'>
                            {getIngredientNameSafe(availability.ingredient.stockItemId, stockItems, 'Loading...')}
                          </span>
                ...
                <div>
                  ...
                  <Textarea id='instructions' placeholder='Any special requests...' value={specialInstructions} onChange={e => setSpecialInstructions(e.target.value)} className='mt-1' rows={3}/>
                </div>

                {/* Stock Status Warning */}
                {showStockWarning &&
                ingredientAvailability.some(item => !item.isAvailable) && (<div className='rounded-lg border border-red-200 bg-red-50 p-3'>
                      <div className='flex items-center space-x-2'>
                ...
                {showStockWarning &&
                ingredientAvailability.some(item => item.isLowStock && item.isAvailable) &&
                !ingredientAvailability.some(item => !item.isAvailable) && (<div className='rounded-lg border border-yellow-200 bg-yellow-50 p-3'>
                      <div className='flex items-center space-x-2'>
                ...
                {/* Add to Order Button */}
                <Button onClick={handleAddToOrder} className='w-full' disabled={isLoading || isAddingItem}>
