...
const AlertDialogContent = React.forwardRef(({ className, ...props }, ref) => {
    ...
    React.useEffect(() => {
        ...
        const dialogElement = contentRef.current?.closest('[role="alertdialog"]');
        if (dialogElement) {
    ...
    return (<AlertDialogPortal>
      <AlertDialogOverlay />
      <AlertDialogPrimitive.Content ref={(node) => {
            ...
            const dialogElement = e.currentTarget.closest('[role="alertdialog"]');
            if (dialogElement) {
                dialogElement.removeAttribute('aria-hidden');
                dialogElement.removeAttribute('data-aria-hidden');
            }
            // Focus first button after a brief delay
            setTimeout(() => {
                if (e.currentTarget) {
                    const firstButton = e.currentTarget.querySelector('button');
                    if (firstButton && firstButton instanceof HTMLElement) {
                        firstButton.focus();
                    }
                }
            }, 50);
