...
import { calculateIngredientCosts, calculateProfitMetrics, } from '@/utils/ingredientUtils';
import { validateMenuItem, checkDuplicateName, } from '@/utils/menuValidation';
...
const MenuItemForm = ({ itemId, onClose, defaultCategory }) => {
    ...
    const { stockItems, isLoading: stockLoading, error: stockError, } = useStockItems();
    const { categories, isLoading: categoriesLoading, error: categoriesError, refresh: refreshCategories, } = useMenuCategories();
    ...
    useEffect(() => {
        if (editingItem) {
            console.log('🔄 Loading editing item data:', {
                ...
                isCustomizable: editingItem.isCustomizable,
            });
            // Convert Ingredient[] to IngredientWithQuantity[] for the form
            const ingredients = Array.isArray(editingItem.ingredients)
            ...
            const colorValue = editingItem.color || undefined;
            console.log('🎨 Setting color in form:', {
                rawColor: editingItem.color,
            ...
            form.reset({
                ...
                isCustomizable: editingItem.isCustomizable || false, // ✅ NEW: Load isCustomizable value
                isPrintableInKitchen: editingItem.isPrintableInKitchen !== undefined ? editingItem.isPrintableInKitchen : true, // Default to true if not set
                isVisibleOnWebsite: editingItem.isVisibleOnWebsite !== undefined ? editingItem.isVisibleOnWebsite : true, // Load current website visibility setting
            ...
            form.reset({
                ...
                isVisibleOnWebsite: true, // Default new items to show on website
                ingredients: [],
            });
        }
    ...
    const calculateCosts = () => {
        ...
        const { totalCost: totalIngredientCost, hasLowStock } = calculateIngredientCosts(ingredients, stockItems || []);
        const { profitMargin, grossProfit } = calculateProfitMetrics(price, totalIngredientCost);
        return {
            totalIngredientCost,
            profitMargin,
            grossProfit,
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const formattedData = {
                ...
                category: data.category || (categories.length > 0 ? categories[0] : 'default'),
            ...
            const validationResult = await validateMenuItem(formattedData, itemId || undefined);
            ...
            if (!validationResult.isValid) {
                ...
                if (Object.keys(validationResult.errors).length > 0) {
                    ...
                    console.error('Validation failed but no specific errors were returned');
                }
                toast({
            ...
            const ingredientsList = data.ingredients
                ? data.ingredients
                    ...
                    if (typeof ing === 'object' && ing !== null && 'stockItemId' in ing && ing.stockItemId) {
                        ...
                        const stockItem = stockItems?.find(stock => stock.id === ing.stockItemId);
                        if (stockItem) {
                            return {
                ...
                await tryCatch(async () => {
                    ...
                    if (result && result.id) {
                        try {
                            ...
                            console.warn('⚠️ Menu item created but verification failed:', verifyError);
                        }
                    }
                    toast({
                    ...
                    return result;
                }, operationTitle, 'Failed to create menu item. Please check your input and try again.', { rethrow: false });
            }
            ...
            handleError(error, itemId ? 'Update Menu Item' : 'Create Menu Item', 'An unexpected error occurred while saving the menu item.', { severity: ErrorSeverity.ERROR });
    ...
    return (<Form {...form}>
      <form onSubmit={(e) => {
        ...
        <div className='flex-1 space-y-4 pb-4'>
          {form.formState.errors.root && (<div className='rounded-md bg-red-50 p-4 dark:bg-red-900/20'>
              <div className='flex'>
                <div className='ml-3'>
        ...
        <Card>
          ...
          <CardContent className='space-y-3'>
            ...
            <FormField control={form.control} name='name' render={({ field }) => (<FormItem>
                  ...
                  <FormControl>
                    <Input placeholder='e.g., Grilled Ribeye Steak' {...field} onBlur={async (e) => {
                field.onBlur(); // Call the original onBlur
                // Only check for duplicates if the name is not empty
                if (e.target.value.trim()) {
                    await tryCatch(async () => {
                        const result = await checkDuplicateName(e.target.value, itemId || undefined);
                        if (!result.isUnique) {
                            setNameError(result.message || 'Duplicate name');
                            form.setError('name', {
                                type: 'manual',
                        ...
                        return result;
                    }, 'Validate Menu Name', 'Failed to check for duplicate menu item names.', { showToast: false, rethrow: false });
                }
            }}/>
                  </FormControl>
                  {nameError && (<p className='text-sm font-medium text-destructive'>
                      {nameError}
                    </p>)}
                  <FormDescription>
            ...
            <FormField control={form.control} name='description' render={({ field }) => (<FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder='Describe the dish and its preparation...' className='min-h-[100px]' {...field}/>
                  </FormControl>
                  <FormDescription>
            ...
            <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
              <FormField control={form.control} name='price' render={({ field }) => (<FormItem>
                    ...
                    <FormControl>
                      <Input type='text' placeholder='0.00' {...field} value={field.value?.toString() || ''} onChange={e => {
              ...
              <FormField control={form.control} name='category' render={({ field }) => (<FormItem>
            ...
            }} value={field.value || undefined} defaultValue={field.value || undefined}>
                      <FormControl>
                        <SelectTrigger>
                ...
                category.id.trim() !== '' &&
                category.name &&
                category.name.trim() !== '') // ✅ Filter out invalid categories including empty strings
                .map(category => (<SelectItem key={category.id} value={category.id}>
                                {category.name}
                              </SelectItem>))) : (<SelectItem value='no-category-placeholder'>
            ...
            <FormField control={form.control} name='color' render={({ field }) => (<FormItem>
                  <FormControl>
                    <ColorPicker value={field.value || undefined} onChange={field.onChange} label='Item Color (Optional)'/>
                  </FormControl>
                  <FormDescription>
        ...
        <Card>
          ...
          <CardContent className='space-y-3'>
            {fields.length === 0 ? (<div className='py-8 text-center text-gray-500'>
                ...
                {fields.map((field, index) => (<div key={field.id} className='flex items-end space-x-2 rounded-lg border p-2'>
                    <FormField control={form.control} name={`ingredients.${index}.stockItemId`} render={({ field }) => (<FormItem className='flex-1'>
                        ...
                        if (selectedStockItem) {
                            ...
                            <SelectContent>
                              {stockLoading ? (<SelectItem value='loading-placeholder' disabled>
                                  Loading stock items...
                                </SelectItem>) : stockItems && stockItems.length > 0 ? ((() => {
                        // 🔍 DEBUG: Log exactly what we're about to render
                        console.log('🎯 SelectContent Render Debug:', {
                            totalStockItems: stockItems.length,
                            stockItemsSample: stockItems.slice(0, 3),
                        });
                        const filteredItems = stockItems.filter(item => item.id &&
                            item.id.trim() !== '' &&
                            item.supplier !==
                        ...
                        console.log('🎯 Filtered Items Debug:', {
                            ...
                            filterReasons: stockItems.map(item => ({
                                ...
                                hasValidId: !!(item.id && item.id.trim() !== ''),
                                supplier: item.supplier,
                                passesSupplierFilter: item.supplier !==
                                    'System-Generated Category',
                            })),
                        });
                        return filteredItems.map(item => (<SelectItem key={item.id} value={item.id}>
                                      {item.name ||
                                item.itemName ||
                                'Unknown Item'}{' '}
                                      ({item.unit}) - $
                    ...
                    <FormField control={form.control} name={`ingredients.${index}.quantityRequired`} render={({ field }) => (<FormItem className='w-40'>
                          <FormLabel>Quantity</FormLabel>
                          <div className='flex items-center space-x-2'>
                            <FormControl>
                              <Input type='text' placeholder='0' className='w-24' {...field} value={field.value?.toString() || ''} onChange={e => {
            ...
            {fields.length > 0 && (<>
                ...
                <div className='space-y-2 rounded-lg bg-gray-50 p-3'>
                  ...
                  <div className='grid grid-cols-1 gap-3 text-sm md:grid-cols-3'>
                    ...
                    <div>
                      ...
                      <div className={`font-semibold ${costs.grossProfit >= 0
                ? 'text-green-600'
                : 'text-red-600'}`}>
                    ...
                    <div>
                      ...
                      <div className={`font-semibold ${costs.profitMargin >= 50
                ? 'text-green-600'
                : costs.profitMargin >= 30
                  ...
                  {costs.profitMargin < 30 && (<div className='flex items-center space-x-2 rounded bg-orange-50 p-2 text-sm text-orange-600'>
                      <span>⚠️</span>
                      <span>
                  ...
                  {costs.hasLowStock && (<div className='mt-2 flex items-center space-x-2 rounded bg-amber-50 p-2 text-sm text-amber-600'>
                      <span>📉</span>
                      <span>
        ...
        <Card>
          ...
          <CardContent className='space-y-3'>
            <FormField control={form.control} name='isAvailable' render={({ field }) => (<FormItem className='flex flex-row items-center justify-between rounded-lg border p-4'>
            ...
            <FormField control={form.control} name='isCustomizable' render={({ field }) => (<FormItem className='flex flex-row items-center justify-between rounded-lg border p-4'>
                  <div className='space-y-0.5'>
                    <FormLabel className='text-base'>
                      Allow Customization
                    </FormLabel>
            ...
            <FormField control={form.control} name='isPrintableInKitchen' render={({ field }) => (<FormItem className='flex flex-row items-center justify-between rounded-lg border p-4'>
                  <div className='space-y-0.5'>
                    <FormLabel className='text-base'>
            ...
            <FormField control={form.control} name='isVisibleOnWebsite' render={({ field }) => (<FormItem className='flex flex-row items-center justify-between rounded-lg border p-4'>
                  <div className='space-y-0.5'>
                    <FormLabel className='text-base'>
                      Show on Website
                    </FormLabel>
