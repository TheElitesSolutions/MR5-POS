...
export const MenuItemIngredientsForm = ({ menuItemId, initialIngredients = [], onSave, onCancel }) => {
    ...
    const handleSubmit = async (data) => {
        ...
        try {
            ...
            const response = await inventoryAPI.updateMenuItemIngredients(menuItemId, data.ingredients);
            if (response.success) {
                toast({
            ...
            toast({
                ...
                description: error instanceof Error ? error.message : 'Failed to save ingredients',
                variant: 'destructive',
            });
        }
        finally {
    ...
    return (<div className='space-y-4'>
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)}>
        {fields.length === 0 ? (<div className='py-8 text-center text-gray-500'>
            ...
            {fields.map((field, index) => (<div key={field.id} className='flex items-end space-x-3 rounded-lg border p-3'>
                <FormField control={form.control} name={`ingredients.${index}.inventoryId`} render={({ field }) => (<FormItem className='flex-1'>
                      ...
                      <Select onValueChange={value => {
                        ...
                        <SelectContent>
                          {isLoading ? (<SelectItem value='loading-placeholder' disabled>
                              Loading inventory items...
                            </SelectItem>) : stockItems && stockItems.length > 0 ? (stockItems
                        .filter(item => item && item.id && item.id.trim() !== '')
                        .map(item => (<SelectItem key={item.id} value={item.id}>
                                  {item.name} ({item.unit}) - $
                                  {item.costPerUnit.toFixed(2)}
                                </SelectItem>))) : (<SelectItem value='no-items-placeholder' disabled>
                ...
                <FormField control={form.control} name={`ingredients.${index}.quantity`} render={({ field }) => (<FormItem className='w-40'>
                ...
                <Button type='button' variant='outline' size='sm' onClick={() => remove(index)} className='mb-2' disabled={isLoading}>
