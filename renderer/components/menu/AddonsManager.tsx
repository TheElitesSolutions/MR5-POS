...
// Simplified interfaces for menu-focused addon management
interface Addon {
  id: string;
  name: string;
...
export function AddonsManager() {
  ...
  const [editTarget, setEditTarget] = useState<AddonGroup | Addon | null>(null);
  ...
  const [addonFormMode, setAddonFormMode] = useState<'create' | 'edit'>('create');
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [groupsResponse, categoriesResponse, assignmentsResponse, inventoryResponse] =
        await Promise.all([
      ...
      if (groupsResponse?.success && groupsResponse.data) {
        for (const group of groupsResponse.data) {
          try {
            const groupAddons = await window.electronAPI?.ipc.invoke(
              'addon:getByGroup',
              group.id
            );
  ...
  const handleAssignToCategory = async () => {
    ...
    try {
      const res = await window.electronAPI?.ipc.invoke(
  ...
  const handleUnassign = async (categoryId: string, groupId: string) => {
    try {
      const res = await window.electronAPI?.ipc.invoke(
  ...
  const handleAddonFormSubmit = async (data: any) => {
    try {
                    ...
                    <CardContent className='pt-4'>
                      ...
                      <div className='flex items-center justify-between text-sm text-gray-500'>
                        <span>
                          {
                            addons.filter(
                              a => a.addonGroupId === group.id && a.isActive
                            ).length
                          }{' '}
                          add-ons
                        </span>
                        <div className='flex space-x-1'>
                          ...
                          <Button
                            ...
                            size='sm'
                            onClick={() =>
                              confirmDelete('group', group.id, group.name)
                            }
                          >
                        ...
                        <div className='flex items-center justify-between'>
                          ...
                          <div className='flex space-x-1'>
                            ...
                            <Button
                              ...
                              size='sm'
                              onClick={() =>
                                confirmDelete('addon', addon.id, addon.name)
                              }
                            >
                              <Trash2 className='h-4 w-4' />
                            </Button>
                            ...
                            <Button
                              ...
                              onClick={() =>
                                confirmDelete(
                                  ...
                                  `${assignment.category?.name} - ${assignment.addonGroup?.name}`,
  ...
  return (
    <div className='space-y-6'>
      ...
      <AddonGroupFormModal
        ...
        title={groupFormMode === 'create' ? 'Create Add-on Group' : 'Edit Add-on Group'}
        mode={groupFormMode}
      />
      ...
      <AddonFormModal
        ...
        onSubmit={handleAddonFormSubmit}
        initialData={editingAddon}
        title={addonFormMode === 'create' ? 'Create Add-on' : 'Edit Add-on'}
        mode={addonFormMode}
