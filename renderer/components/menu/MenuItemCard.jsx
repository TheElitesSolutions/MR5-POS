...
const MenuItemCard = ({ item, onEdit }) => {
    ...
    return (<Card className={cn('transition-shadow hover:shadow-md overflow-hidden bg-white dark:bg-gray-800', !item.isAvailable && 'opacity-75')}>
      <CardHeader className='pb-3'>
        <div className='flex items-start justify-between'>
          <div className='flex-1'>
            <div className='flex items-center gap-2 mb-2'>
              {itemColor && (<div className='h-3 w-3 rounded-full flex-shrink-0' style={{ backgroundColor: itemColor }} title='Item color'/>)}
          ...
          <div className='flex items-center space-x-2'>
            <Button variant='ghost' size='sm' onClick={handleToggleAvailability} disabled={isToggling} className={item.isAvailable ? 'text-green-600' : 'text-gray-400'}>
              {item.isAvailable ? (<Eye className='h-4 w-4'/>) : (<EyeOff className='h-4 w-4'/>)}
            </Button>
            <Button variant='ghost' size='sm' onClick={handleEdit} className='text-blue-600'>
      ...
      <CardContent>
        ...
        <div className='flex items-center justify-between'>
          ...
          <div className='flex items-center space-x-2'>
            <Badge variant={item.isAvailable ? 'default' : 'secondary'} className={item.isAvailable
            ? 'bg-green-100 text-green-800'
            : 'bg-gray-100 text-gray-600'}>
              {item.isAvailable ? 'Available' : 'Unavailable'}
            </Badge>
            {!item.isAvailable && (<Badge variant='destructive' className='text-xs'>
                Inactive
              </Badge>)}
          </div>
        </div>
