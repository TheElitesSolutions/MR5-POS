import { inventoryAPI, orderAPI, tableAPI } from '@/lib/ipc-api';
...
function convertIPCOrderToRendererOrder(ipcOrder) {
    return {
        id: ipcOrder.id,
        ...
        type: ipcOrder.type,
        status: ipcOrder.status,
        items: (ipcOrder.items || []),
...
function convertIPCTableToRendererTable(ipcTable) {
    return {
        id: ipcTable.id,
        name: ipcTable.name,
        status: ipcTable.status,
        isPayLater: ipcTable.isPayLater,
        createdAt: ipcTable.createdAt,
        updatedAt: ipcTable.updatedAt,
...
export const usePOSStore = create((set, get) => ({
    ...
    setOrderChanges: (changes) => {
        const hasAnyChanges = Array.from(changes.values()).some(tracking => tracking.newItems.length > 0 ||
    ...
    savePendingChanges: async (orderId, options = {}) => {
        ...
        try {
            ...
            set({
                ...
                isSaving: false,
                hasUnsavedChanges: Array.from(updatedChanges.values()).some(t => t.newItems.length > 0 ||
                    Object.keys(t.netChanges).length > 0 ||
                    t.removedItems.length > 0),
            });
            // Clear localStorage backup after successful save
            get().clearLocalStorageBackup(orderId);
            console.log('✅ SAVE: All pending changes saved successfully for order', orderId);
            return true;
        }
        catch (error) {
            set({ isSaving: false });
            ...
            if (newRetryCount <= maxRetries) {
                ...
                console.log(`🔄 SAVE: Retrying save in ${delay}ms (attempt ${newRetryCount}/${maxRetries})`);
            ...
            if (!silent && typeof window !== 'undefined') {
                const userConfirmed = confirm(`Failed to save your order changes after ${maxRetries} attempts. ` +
    ...
    fetchTables: async () => {
        try {
            ...
            const tables = response.data.map(convertIPCTableToRendererTable);
    ...
    selectTable: async (table) => {
        ...
        if (currentOrder?.id && orderChanges.has(currentOrder.id)) {
            ...
            const hasChanges = tracking &&
                (tracking.newItems.length > 0 ||
                    Object.keys(tracking.netChanges).length > 0 ||
                    tracking.removedItems.length > 0);
            if (hasChanges) {
                console.log('🔄 AUTO-SAVE: Saving pending changes before switching tables', {
                ...
                if (!saveSuccess) {
                    console.error('❌ AUTO-SAVE: Failed to save pending changes, blocking table switch');
        ...
        if (typeof table === 'object') {
            ...
            if (tableExists) {
                ...
                if (table.activeOrder?.id) {
                    try {
                        console.log('🔍 POS Store: Fetching fresh order data for table', table.name, {
                        ...
                        const freshOrder = orderResp.success && orderResp.data
                            ? convertIPCOrderToRendererOrder(orderResp.data)
                            : table.activeOrder;
                        ...
                        console.warn('⚠️ POS Store: Failed to fetch fresh order data, using cached data', error);
                        set({
                            selectedTable: table,
                            viewMode: 'tables',
                ...
                console.warn('POS Store: Attempted to select non-existent table', table.name);
            }
        }
        else {
            // Handle table ID
            ...
            if (foundTable) {
                ...
                if (foundTable.activeOrder?.id) {
                    try {
                        ...
                        console.log('🔍 DEBUG: After setting state (table ID) - currentOrder check:', {
                            ...
                            stateSetSuccessfully: verifyCurrentOrder?.id === ultraFreshOrder.id,
                        });
                        console.log('🔄 POS Store: SelectTableById - Ultra fresh order state set:', {
                            orderId: ultraFreshOrder.id,
                            itemsCount: ultraFreshOrder.items?.length || 0,
                            hasItemsArray: Array.isArray(ultraFreshOrder.items),
                            timestamp: new Date().toISOString(),
                        });
                        console.log('✅ POS Store: Selected table by ID with fresh order data', foundTable.name, {
                            orderId: freshOrder?.id,
                ...
                console.warn('POS Store: Attempted to select table by ID that does not exist', table);
            }
        }
    },
    createTable: async (name) => {
        try {
            ...
            const newTable = convertIPCTableToRendererTable(response.data);
            const { tables } = get();
    ...
    toggleTablePayLater: async (tableId) => {
        try {
            set({ isLoading: true, error: null });
            ...
            const updatedTable = convertIPCTableToRendererTable(response.data);
            const { tables, selectedTable } = get();
            ...
            set({
                error: error instanceof Error ? error.message : 'Failed to toggle pay later status',
                isLoading: false,
            });
            throw error;
        }
    ...
    createOrder: async (tableId) => {
        try {
            ...
            const table = tables.find(t => t.id === tableId);
            if (!table) {
                throw new Error('Selected table not found. Please refresh and try again.');
            }
            ...
            const newOrder = convertIPCOrderToRendererOrder(response.data);
            ...
            if (updatedTable?.activeOrder) {
                ...
                console.log('✅ POS Store: CurrentOrder synced with fresh table data:', {
                    orderId: freshOrder.id,
                    itemsCount: freshOrder.items?.length || 0,
    ...
    createTakeawayDeliveryOrder: async (orderType, customerDetails) => {
        try {
            set({ isLoading: true, error: null });
            // Validate user is authenticated
            const user = useAuthStore.getState().user;
            ...
            const orderData = {
                ...
                deliveryAddress: orderType === 'DELIVERY' ? customerDetails.address?.trim() : undefined,
            };
            if (process.env.NODE_ENV === 'development') {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.log('POS Store: Created takeaway/delivery order successfully', newOrder);
            }
        }
        catch (error) {
            console.error('POS Store: Failed to create takeaway/delivery order', error);
            set({
                error: error instanceof Error ? error.message : 'Failed to create order',
                isLoading: false,
            });
    ...
    selectTakeawayDeliveryOrder: async (order) => {
        ...
        if (currentOrder?.id && orderChanges.has(currentOrder.id)) {
            ...
            const hasChanges = tracking &&
                (tracking.newItems.length > 0 ||
                    ...
                    tracking.removedItems.length > 0);
            if (hasChanges) {
                console.log('🔄 AUTO-SAVE: Saving pending changes before switching orders', {
                    fromOrderId: currentOrder.id,
                    toOrderId: order.id,
                    changes: {
                        newItems: tracking.newItems.length,
                ...
                if (!saveSuccess) {
                    console.error('❌ AUTO-SAVE: Failed to save pending changes, blocking order switch');
        ...
        try {
            console.log('🔍 POS Store: Fetching fresh order data for takeaway/delivery order', order.id);
            ...
            set({
                currentOrder: ultraFreshOrder,
                activeOrder: ultraFreshOrder,
                orderType: ultraFreshOrder.type,
                selectedTable: null, // Clear table selection since this is takeaway/delivery
            });
            console.log('🔄 POS Store: SelectTakeaway - Ultra fresh order state set:', {
                orderId: ultraFreshOrder.id,
                itemsCount: ultraFreshOrder.items?.length || 0,
                hasItemsArray: Array.isArray(ultraFreshOrder.items),
                timestamp: new Date().toISOString(),
            });
            console.log('✅ POS Store: Selected takeaway/delivery order with fresh data', {
                orderId: freshOrder.id,
                itemsCount: freshOrder.items?.length || 0,
            ...
            if (process.env.NODE_ENV === 'development') {
                console.log('POS Store: Selected takeaway/delivery order (cached data)', order);
            }
        }
    },
    addItemToOrder: (menuItem, quantity, notes) => {
    ...
    addOrderItem: async (menuItemId, quantity, customizations, notes) => {
        try {
            set({ isLoading: true, error: null });
            const { currentOrder } = get();
            if (!currentOrder) {
            ...
            const existingItemQuantity = currentOrder.items?.find(item => item.menuItemId === menuItemId)
                ?.quantity || 0;
            ...
            if (!stockCheck.success || !stockCheck.data?.available) {
                ...
                if (unavailableItems.length > 0) {
                    ...
                    const itemDetails = unavailableItems
                        .map(item => `${item.name}: have ${item.available} ${item.unit}, need ${item.required} ${item.unit}`)
                        .join(', ');
            ...
            const updatedOrder = convertIPCOrderToRendererOrder(orderResp.data);
            // Preserve the delivery fee if it exists in the current order
            if ((currentOrder.deliveryFee ?? 0) > 0 &&
            ...
            console.log('🔄 POS Store: AddItem state updated with fresh object references:', {
                orderId: freshCurrentOrder.id,
                itemsCount: freshCurrentOrder.items?.length || 0,
                lastItem: freshCurrentOrder.items?.[freshCurrentOrder.items.length - 1]
                    ?.menuItemName || 'none',
            });
            ...
            if (updatedOrder.tableId) {
                try {
                    ...
                    console.warn('POS Store: Failed to refresh tables after adding item:', refreshError);
                }
            }
            ...
            if (process.env.NODE_ENV === 'development') {
                console.log('POS Store: Added item to order successfully', updatedOrder);
            }
        ...
        try {
            ...
            if (updatedOrder.tableId) {
                try {
                    ...
                    // Log but don't fail the operation for table refresh errors
                    console.warn('POS Store: Failed to refresh tables after removing item:', refreshError);
                }
            }
            if (process.env.NODE_ENV === 'development') {
    ...
    updateOrderItemQuantity: (orderItemId, quantity) => {
        ...
        const updatedOrder = {
            ...currentOrder,
            items: (currentOrder.items || []).map(item => item.id === orderItemId
                ? {
                    ...item,
                    quantity,
    ...
    updateOrderItem: async (orderItemId, quantity) => {
        try {
            set({ isLoading: true, error: null });
            const { currentOrder } = get();
            if (!currentOrder) {
            ...
            const updatedOrder = convertIPCOrderToRendererOrder(response.data);
            set({
                currentOrder: updatedOrder,
                activeOrder: updatedOrder,
                isLoading: false,
            ...
            if (updatedOrder.tableId) {
                try {
                    ...
                    // Log but don't fail the operation for table refresh errors
                    console.warn('POS Store: Failed to refresh tables after updating item:', refreshError);
                }
            }
            if (process.env.NODE_ENV === 'development') {
    ...
    completeOrder: async () => {
        try {
            ...
            const response = await orderAPI.updateStatus({
                id: currentOrder.id,
                status: 'COMPLETED', // Assuming this is the status for completed orders
            });
            if (!response.success || !response.data) {
            ...
            try {
                ...
                console.warn('POS Store: Failed to refresh tables after order completion', tableError);
    ...
    cancelCurrentOrder: async () => {
        try {
            ...
            const response = await orderAPI.updateStatus({
                id: currentOrder.id,
                status: 'CANCELLED', // Assuming this is the status for cancelled orders
            });
            if (!response.success || !response.data) {
            ...
            if (tableToDelete && tableToDelete.id) {
                try {
                    console.log(`Deleting table ${tableToDelete.name} (${tableToDelete.id}) after order cancellation`);
                    await tableAPI.delete(tableToDelete.id);
                    console.log(`Table ${tableToDelete.name} deleted successfully`);
                }
                catch (tableDeleteError) {
                    console.error('Failed to delete table after order cancellation:', tableDeleteError);
            ...
            try {
                ...
                console.warn('POS Store: Failed to refresh tables after order cancellation', tableError);
                // Don't throw here, the order cancellation was successful
            }
            if (process.env.NODE_ENV === 'development') {
                console.log('POS Store: Cancelled current order successfully and deleted table', {
                    cancelledOrder,
    ...
    cancelOrder: async () => {
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.log('POS Store: Cancelled order successfully and deleted table', {
                    cancelledOrder,
                    deletedTable: tableToDelete?.name,
                });
            }
    ...
    fetchAllOrders: async (retryCount = 0) => {
        ...
        if (isLoadingOrders && retryCount === 0)
        ...
        try {
            ...
            if (takeoutResponse?.success && Array.isArray(takeoutResponse.data)) {
                const activeTakeout = takeoutResponse.data
            ...
            if (deliveryResponse?.success && Array.isArray(deliveryResponse.data)) {
                const activeDelivery = deliveryResponse.data
                    .map(convertIPCOrderToRendererOrder)
                    .filter((order) => ['PENDING', 'READY'].includes(order.status));
            ...
            if (dineInResponse?.success && Array.isArray(dineInResponse.data)) {
                const activeDineIn = dineInResponse.data
                    .map(convertIPCOrderToRendererOrder)
                    .filter((order) => ['PENDING', 'READY'].includes(order.status));
                ...
                sortedOrders.forEach(order => {
                    ...
                    if (tracking && tracking.newItems.length > 0) {
                        ...
                        const reconciledNewItems = tracking.newItems.filter(newItem => {
                            ...
                            const matchingItem = order.items?.find(existingItem => existingItem.id === newItem.id);
            ...
            console.error(`POS Store: Failed to fetch orders (attempt ${retryCount + 1})`, error);
            // Retry logic
            ...
            const userFriendlyMessage = retryCount >= maxRetries
    ...
    updateOrderInStore: (updatedOrder, forceUpdate = false) => {
        ...
        const { allOrders } = get();
        const updatedOrders = allOrders.map(order => order.id === cleanedOrder.id ? cleanedOrder : order);
    ...
    processPendingActions: async () => {
        ...
        if (!isOnline || pendingActions.length === 0)
            return;
        if (process.env.NODE_ENV === 'development') {
            console.log('POS Store: Processing', pendingActions.length, 'pending actions');
        }
        ...
        if (successfulActions.length > 0) {
            const remainingActions = pendingActions.filter((_, index) => !successfulActions.includes(index));
            set({ pendingActions: remainingActions });
            if (process.env.NODE_ENV === 'development') {
                console.log('POS Store: Processed', successfulActions.length, 'actions,', remainingActions.length, 'remaining');
            }
        }
    },
    enableOfflineMode: () => {
    ...
    backupToLocalStorage: (orderId) => {
        const { orderChanges } = get();
        const tracking = orderChanges.get(orderId);
        if (tracking) {
            try {
                localStorage.setItem(`orderBackup_${orderId}`, JSON.stringify({
                    tracking,
                    timestamp: Date.now(),
                }));
                console.log('💾 BACKUP: Order changes backed up to localStorage', orderId);
            }
            catch (error) {
    ...
    restoreFromLocalStorage: (orderId) => {
        try {
            const backup = localStorage.getItem(`orderBackup_${orderId}`);
            if (backup) {
                const parsed = JSON.parse(backup);
                console.log('🔄 RESTORE: Restored order changes from localStorage', orderId);
    ...
    clearLocalStorageBackup: (orderId) => {
        try {
