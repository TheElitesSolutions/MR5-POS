...
const mapBackendOrderItem = (item) => ({
    ...item,
    status: item.status, // Cast backend status to frontend type
});
// Helper function to map Backend Order to Frontend Order
const mapBackendOrderToFrontendOrder = (order) => {
    ...
    return {
        ...
        status: order.status, // Cast backend OrderStatus enum to frontend literal type
...
export const useOrdersStore = create((set, get) => ({
    ...
    closeCashbox: async (date, actualCashAmount, userId) => {
        try {
            ...
            set({
                error: error instanceof Error ? error.message : 'Failed to close cashbox',
                isLoading: false,
            });
            throw error;
        }
