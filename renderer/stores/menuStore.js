...
import { getCurrentUserId, verifyPermission, getUserDisplayName, } from '@/utils/authUtils';
...
import { convertToUIMenuItem, convertToAPIMenuItem, } from '@/types/menu';
...
export const useMenuStore = create((set, get) => ({
    ...
    _internalFetchMenuItems: async (params) => {
        try {
            ...
            const queryParams = {
                ...
                search: params?.search !== undefined ? params.search : pagination.search,
            ...
            if (process.env.NODE_ENV === 'development') {
                console.log('Menu Store: _internalFetchMenuItems called with params:', queryParams);
            }
            ...
            if (cachedData) {
                ...
                const cachedResponse = cachedData;
                ...
                set({
                    ...
                    pagination: {
                        ...
                        totalPages: Math.ceil(cachedResponse.total / cachedResponse.pageSize),
            ...
            const { items: apiMenuItems = [], total = 0, page = 1, pageSize = 12, } = response.data || {};
            const totalPages = Math.ceil(total / pageSize);
            console.log('📥 Menu Store: Received API items:', {
                count: apiMenuItems.length,
                items: apiMenuItems.map((item) => ({
                    id: item.id,
                    name: item.name,
    ...
    _internalFetchCategories: async () => {
        try {
            ...
            if (!Array.isArray(response.data)) {
                console.warn('Menu Store: Categories response is not an array:', response.data);
            ...
            const categoryObjects = response.data
                ...
                .filter((c) => c.id && c.name && typeof c.name === 'string' && c.name.trim())
                .map((c) => ({ id: c.id, name: c.name, color: c.color }));
            // Make sure we have unique categories (by id)
            const uniqueCategories = Array.from(new Map(categoryObjects.map(c => [c.id, c])).values());
            ...
            if (uniqueCategories.length === 0 && existingCategories.length > 0) {
                console.warn('Menu Store: Received empty categories from server, keeping existing categories');
                set({ isLoading: false });
            }
            else {
                set({ categories: uniqueCategories, isLoading: false });
                if (process.env.NODE_ENV === 'development') {
                    console.log('Menu Store: Fetched categories successfully', uniqueCategories);
                }
            }
            ...
            if (Array.isArray(menuItems) &&
                ...
                if (categoriesFromItems.length > 0) {
                    ...
                    if (process.env.NODE_ENV === 'development') {
                        console.log('Menu Store: Used categories from menu items as fallback', categoriesFromItems);
                    }
                }
            }
        }
            ...
            try {
                ...
                if (Array.isArray(menuItems) && menuItems.length > 0) {
                    const categoriesFromItems = menuItems
                        ...
                        .filter((category) => Boolean(category && typeof category === 'string'))
                    ...
                    if (categoriesFromItems.length > 0) {
                        ...
                        if (process.env.NODE_ENV === 'development') {
                            console.log('Menu Store: Used categories from menu items as fallback after error', categoriesFromItems);
                        }
                    }
                }
            }
            catch (fallbackError) {
                console.error('Menu Store: Fallback category extraction failed', fallbackError);
            }
        }
    },
    createMenuItem: async (itemData) => {
        ...
        try {
            ...
            const sanitizedData = {
                ...
                isPrintableInKitchen: itemData.isPrintableInKitchen !== undefined ? !!itemData.isPrintableInKitchen : true,
                isVisibleOnWebsite: itemData.isVisibleOnWebsite !== undefined ? !!itemData.isVisibleOnWebsite : true,
            ...
            const createRequest = {
                menuItem: convertToAPIMenuItem(sanitizedData),
                userId,
            };
            ...
            set(state => ({
                menuItems: state.menuItems.map(item => item.id === tempId ? newItem : item),
                isLoading: false,
            }));
            ...
            try {
                ...
                if (categoriesResponse.success && categoriesResponse.data) {
                    const refreshedCategories = categoriesResponse.data.map((cat) => cat.name || cat);
                    set(state => ({
                        ...state,
                        categories: refreshedCategories,
                    }));
                    if (process.env.NODE_ENV === 'development') {
                        console.log('Menu Store: Categories refreshed after item creation', refreshedCategories);
                    }
                }
            }
            catch (categoryError) {
                console.warn('Menu Store: Failed to refresh categories after item creation:', categoryError);
            ...
            if (process.env.NODE_ENV === 'development') {
                console.log(`Menu Store: Invalidated ${removedCacheEntries} cache entries`);
            }
            // Record audit trail
    ...
    updateMenuItem: async (id, updates) => {
        ...
        try {
                ...
                if (conflict) {
                    switch (conflict.type) {
                        case ConflictType.DELETED:
                            throw new Error('This item has been deleted and cannot be updated.');
                        case ConflictType.MODIFIED:
                            // We could handle this differently with a confirmation,
                            // but for now, we'll just throw an error
                            throw new Error('This item has been modified by another user. Please refresh and try again.');
                        default:
            ...
            if (!response.success || !response.data) {
                ...
                if (response.error?.includes('version conflict') ||
            ...
            const finalMenuItems = menuItems.map(item => item.id === id ? updatedItem : item);
            // Finalize categories
            ...
            const removedCacheEntries = cache.removeByPrefix('menuItems');
            if (process.env.NODE_ENV === 'development') {
                console.log(`Menu Store: Invalidated ${removedCacheEntries} cache entries after item update`);
            }
    ...
    deleteMenuItem: async (id) => {
        ...
        try {
            ...
            const integrityResult = await checkMenuItemDeletionIntegrity(id, itemToDelete.name);
            ...
            if (!itemResponse.success || !itemResponse.data) {
                ...
                console.warn(`Menu item ${id} not found on server, may already be deleted`);
            }
            else {
            ...
            const removedCacheEntries = cache.removeByPrefix('menuItems');
            if (process.env.NODE_ENV === 'development') {
                console.log(`Menu Store: Invalidated ${removedCacheEntries} cache entries after item deletion`);
            }
            ...
            try {
                ...
                console.warn('Failed to refresh categories after deletion:', refreshError);
            }
            // Record audit trail
            // Get user display name directly
            await recordAudit({
    ...
    toggleMenuItemAvailability: async (id) => {
        ...
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.log('Menu Store: Toggled menu item availability successfully', updatedItem);
            }
        }
        catch (error) {
    ...
    createCategory: async (name, color) => {
        try {
            ...
            if (!createCategoryResponse.success) {
                throw new Error(createCategoryResponse.error || 'Failed to create category');
            }
            // Get the newly created category from the response
            const newCategory = createCategoryResponse.data;
            if (process.env.NODE_ENV === 'development') {
                console.log('Menu Store: Created new category successfully', newCategory);
            }
    ...
    updateCategory: async (oldName, newName, color) => {
        try {
            ...
            const match = categoriesResponse.data.find((c) => c.name === oldName);
            if (!match)
            ...
            set({ isLoading: false });
            if (process.env.NODE_ENV === 'development') {
                console.log('Menu Store: Updated category successfully', oldName, 'to', newName);
            }
        }
        catch (error) {
    ...
    deleteCategory: async (name) => {
        try {
            ...
            const match = categoriesResponse.data.find((c) => c && c.name === name);
            if (!match)
            ...
            try {
                ...
                console.warn('Menu Store: Failed to refresh categories after delete:', refreshError);
            }
            try {
                const { pagination } = get();
                ...
                console.warn('Menu Store: Failed to refresh menu items after delete:', refreshError);
            }
            set({ isLoading: false });
            if (process.env.NODE_ENV === 'development') {
    ...
    restoreMenuItem: async (id) => {
        try {
            ...
            const deletedItem = getLastDeletedEntity(id, 'menuItem');
            if (!deletedItem) {
                throw new Error(`Menu item with ID ${id} cannot be found in deletion history`);
            }
            ...
            const restoredItem = {
                ...
                createdAt: typeof deletedItem.createdAt === 'string'
            ...
            const removedCacheEntries = cache.removeByPrefix('menuItems');
            if (process.env.NODE_ENV === 'development') {
                console.log(`Menu Store: Invalidated ${removedCacheEntries} cache entries after item restoration`);
            }
            // Record audit trail
            // Get user display name directly
            await recordAudit({
            ...
            if (process.env.NODE_ENV === 'development') {
                console.log('Menu Store: Restored menu item successfully', restoredUIItem);
            }
        }
        catch (error) {
