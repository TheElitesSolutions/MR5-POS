...
const AddonSelectionContext = createContext(undefined);
export const AddonSelectionProvider = ({ children, }) => {
    ...
    const loadCategoryAddonGroups = useCallback(async (categoryId) => {
        setState(prev => ({
            ...prev,
            loadingGroups: true,
        ...
        try {
            ...
            const response = await window.electronAPI.ipc.invoke('addon:getCategoryAddonGroups', {
                categoryId,
                ...
                setState(prevState => {
                    ...
                    addons?.forEach((addon) => {
                        ...
                        if (addon.inventoryItems && addon.inventoryItems.length > 0) {
                            ...
                            const totalStock = addon.inventoryItems.reduce((sum, item) => sum + (item.inventory?.currentStock || 0), 0);
    ...
    const selectAddon = useCallback((menuItemId, addon, quantity) => {
        setState(prev => {
            ...
            const existingIndex = currentSelections.findIndex(s => s.addonId === addon.id);
    ...
    // Deselect an addon
    const deselectAddon = useCallback((menuItemId, addonId) => {
        setState(prev => {
            const newSelectedAddons = new Map(prev.selectedAddons);
            const currentSelections = newSelectedAddons.get(menuItemId) || [];
            const filteredSelections = currentSelections.filter(s => s.addonId !== addonId);
    ...
    const updateAddonQuantity = useCallback((menuItemId, addonId, quantity) => {
        setState(prev => {
            const newSelectedAddons = new Map(prev.selectedAddons);
            const currentSelections = newSelectedAddons.get(menuItemId) || [];
            const selectionIndex = currentSelections.findIndex(s => s.addonId === addonId);
    ...
    const clearAddons = useCallback((menuItemId) => {
        setState(prev => {
            if (menuItemId) {
                const newSelectedAddons = new Map(prev.selectedAddons);
                newSelectedAddons.delete(menuItemId);
    ...
    // Update stock level
    const updateStockLevel = useCallback((addonId, stockLevel) => {
        setState(prev => {
    ...
    const checkStockAvailability = useCallback((addonId, requiredQuantity) => {
        const currentStock = state.stockLevels.get(addonId) || 0;
        return currentStock >= requiredQuantity;
    }, [state.stockLevels]);
    // Validate selections for a menu item
    const validateSelections = useCallback((menuItemId) => {
        const errors = [];
    ...
    const getSelectionsForMenuItem = useCallback((menuItemId) => {
        return state.selectedAddons.get(menuItemId) || [];
    }, [state.selectedAddons]);
    // Calculate price impact for menu item
    const calculatePriceImpact = useCallback((menuItemId) => {
        const selections = state.selectedAddons.get(menuItemId) || [];
    ...
    const calculateTotalImpact = useCallback(() => {
        ...
        state.selectedAddons.forEach(selections => {
            total += selections.reduce((sum, selection) => sum + selection.totalPrice, 0);
        });
        return total;
    }, [state.selectedAddons]);
    // Reset state
    ...
    return (<AddonSelectionContext.Provider value={contextValue}>
      {children}
...
export const useAddonSelection = () => {
    ...
    if (context === undefined) {
        throw new Error('useAddonSelection must be used within an AddonSelectionProvider');
    }
    return context;
};
