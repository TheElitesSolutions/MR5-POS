...
import { PrintReceiptRequest, Printer } from '../../shared/ipc-types';
import type {
  EnhancedPrintRequest,
  EnhancedPrintResponse,
} from '../../shared/enhanced-printer-types';
...
// Define local interfaces for printer API responses
...
// Define local test print request interface
...
export class PrinterAPI {
   ...
   */
  static async printEnhanced(
    request: EnhancedPrintRequest
  ): Promise<EnhancedPrintResponse> {
    if (!isElectron()) {
      console.warn(
        'PrinterAPI: Not in Electron environment, simulating enhanced print'
      );
      return {
        success: true,
        data: {
    ...
    try {
      ...
      console.log(
        `🚀 ENHANCED PRINT: Processing ${printType} for order ${request.orderId}`,
        {
      ...
      try {
        ...
        console.warn(
          'Enhanced channel not available, falling back to optimized:',
          error
        );

        // Convert to legacy format
        const legacyRequest = this.convertToLegacyRequest(request);
        response = await window.electronAPI!.ipc.invoke(
          fallbackChannel,
          legacyRequest
        );
  ...
  static async printOptimized(
    ...
    options: {
      ...
      copies?: number;
      cancelledItems?: any[]; // Support for cancelled items
      updatedItemIds?: string[]; // Support for printing only specific updated items
      itemChanges?: any[]; // Enhanced change tracking information
    } = {}
  ): Promise<PrinterResponse> {
    if (!isElectron()) {
      console.warn(
        'PrinterAPI: Not in Electron environment, simulating optimized print'
      );
      return {
        success: true,
        data: {
    ...
    try {
      ...
      console.log(
        `🚀 INSTANT PRINT: Queuing ${printType} for order ${orderId} (150x faster)`
      );
      ...
      console.log(
        `⚡ INSTANT RESPONSE: Print job queued successfully`,
        response
      );
  ...
  static async getPrinters(): Promise<Printer[]> {
    if (!isElectron()) {
      console.warn(
        'PrinterAPI: Not in Electron environment, returning mock printers'
      );
      return mockPrinters;
    }
  ...
  static async testPrint(
    ...
    if (!isElectron()) {
      console.warn(
        'PrinterAPI: Not in Electron environment, simulating test print'
      );
      return {
        success: true,
        data: {
    ...
    try {
      ...
      const response = await window.electronAPI!.ipc.invoke(
        PRINTER_CHANNELS.PRINT_TEST_PAGE,
        request
      );
      return {
        success: response.success,
  ...
  static async printReceipt(
    ...
    if (!isElectron()) {
      console.warn(
        'PrinterAPI: Not in Electron environment, simulating receipt print'
      );
      return {
        success: true,
        data: {
    ...
    try {
      console.log(
        `🚀 Printing receipt for order ${orderId} using OPTIMIZED instant printing (150x faster)`
      );

      // Try optimized system first
      const optimizedResult = await this.printOptimized(
        orderId,
        printerName,
        userId,
        {
      ...
      console.warn(
        `⚠️ Optimized system failed, falling back to original system`
      );
  ...
  static async printKitchenOrder(
    ...
    if (!isElectron()) {
      console.warn(
        'PrinterAPI: Not in Electron environment, simulating kitchen order print'
      );
      return {
        success: true,
        data: {
          message:
            'Kitchen order print simulated successfully (development mode)',
        },
      };
    }
    ...
    try {
      console.log(
        `🚀 Printing kitchen order for order ${orderId} using OPTIMIZED instant printing (150x faster, onlyUnprinted: ${onlyUnprinted})`
      );
   ...
   */
  static async printCancellationTicket(
    orderId: string,
    printerName: string,
    cancelledItems: any[],
    userId: string,
    ...
    if (!isElectron()) {
      console.warn(
        'PrinterAPI: Not in Electron environment, simulating cancellation ticket'
      );
      return {
        success: true,
        data: {
    ...
    try {
      console.log(
        `⚠️ Printing CANCELLATION ticket for order ${orderId} with ${cancelledItems.length} items`
      );

      // Print using the kitchen order method with cancelled items
      return await this.printKitchenOrder(
        orderId,
        printerName,
        copies,
        userId,
  ...
  static async printInvoice(
    ...
    if (!isElectron()) {
      console.warn(
        'PrinterAPI: Not in Electron environment, simulating invoice print'
      );
      return {
        success: true,
        data: {
    ...
    try {
      console.log(
        `🚀 Printing invoice for order ${orderId} using OPTIMIZED instant printing (150x faster)`
      );
  ...
  static async checkPrinterStatus(
    ...
    if (!isElectron()) {
      console.warn(
        'PrinterAPI: Not in Electron environment, simulating printer status check'
      );
      return {
        success: true,
        data: {
   ...
   */
  private static convertToLegacyRequest(
    request: EnhancedPrintRequest
  ): PrintReceiptRequest {
    return {
   ...
   */
  private static convertToEnhancedResponse(
    legacyResponse: any,
    originalRequest: EnhancedPrintRequest
  ): EnhancedPrintResponse {
   ...
   */
  static async getEnhancedPrinters(): Promise<
    Array<{
      name: string;
      displayName: string;
      isDefault: boolean;
