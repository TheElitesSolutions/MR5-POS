...
jest.mock('../../renderer/context/StockDataContext', () => ({
    ...
    StockDataProvider: ({ children }) => children,
}));
...
jest.mock('../../renderer/context/AddonSelectionContext', () => ({
    AddonSelectionProvider: ({ children }) => children,
...
jest.mock('../../renderer/components/addon/AddonSelectionStep', () => ({
    AddonSelectionStep: function MockAddonSelectionStep({ onContinue, onBack }) {
        return (<div data-testid="addon-step">
...
describe('Menu Flow - Frontend Integration Tests', () => {
    ...
    describe('Ingredient Customization', () => {
        ...
        it('should allow adjusting quantity', async () => {
            ...
            const plusButton = screen.getAllByRole('button').find(btn => btn.querySelector('[class*="Plus"]'));
        ...
        it('should not allow quantity to go below 1', async () => {
            ...
            const minusButton = screen.getAllByRole('button').find(btn => btn.querySelector('[class*="Minus"]'));
    ...
    describe('Complete Flow', () => {
        it('should complete entire menu selection flow', async () => {
            ...
            await waitFor(() => {
                expect(mockOnItemAdded).toHaveBeenCalledWith(expect.objectContaining({
                    selectedItem: expect.objectContaining({
                        id: '1',
                        name: 'Burger',
                    }),
