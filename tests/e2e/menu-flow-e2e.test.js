...
describe('E2E: Menu Flow in POS', () => {
    ...
    beforeEach(() => {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).run(user.id, user.name, user.email, user.password, user.role, user.createdAt.toISOString(), user.updatedAt.toISOString());
        });
        // Create categories
        const categories = [
        ...
        VALUES (?, ?, ?, ?)
      `).run(category.id, category.name, category.createdAt.toISOString(), category.updatedAt.toISOString());
        });
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(stock.id, stock.name, stock.currentQuantity, stock.minimumQuantity || 10, stock.unit, stock.category || 'Ingredients', stock.createdAt.toISOString(), stock.updatedAt.toISOString());
        });
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(item.id, item.name, item.description, item.price, item.category, item.categoryId, item.isAvailable ? 1 : 0, item.isActive ? 1 : 0, item.ingredients, item.isCustomizable ? 1 : 0, item.createdAt.toISOString(), item.updatedAt.toISOString());
        });
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(table.id, table.number, table.capacity, table.status, table.section || 'Main Hall', table.createdAt.toISOString(), table.updatedAt.toISOString());
    ...
    describe('Menu Item Fetching', () => {
      ...
      `).all(mainCourseCategory.id);
        ...
        it('should include ingredient information with menu items', () => {
            ...
            const menuItem = db.prepare('SELECT * FROM MenuItem WHERE id = ?').get(burger.id);
    ...
    describe('Stock Availability Check', () => {
        it('should verify sufficient stock for menu item ingredients', () => {
            ...
            ingredients.forEach((ingredient) => {
                const stock = db.prepare('SELECT * FROM Inventory WHERE id = ?').get(ingredient.id);
        ...
        it('should identify low stock warnings', () => {
            ...
            const updatedStock = db.prepare('SELECT * FROM Inventory WHERE id = ?').get(beefPatty.id);
    ...
    describe('Order Creation with Menu Items', () => {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(order.id, order.orderNumber, order.userId, order.type, order.status, order.tableId, 0, 0, 0, 0, // Will be updated after adding items
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(orderItem.id, orderItem.orderId, orderItem.menuItemId, orderItem.quantity, orderItem.price, orderItem.subtotal, orderItem.customizations, orderItem.createdAt.toISOString(), orderItem.updatedAt.toISOString());
      ...
      `).get(orderItem.id);
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(order.id, order.orderNumber, order.userId, order.type, order.status, burger.price, 0, 0, burger.price, order.createdAt.toISOString(), order.updatedAt.toISOString());
            const orderItem = {
                id: uuidv4(),
                orderId: order.id,
                menuItemId: burger.id,
      ...
      `).run(orderItem.id, orderItem.orderId, orderItem.menuItemId, orderItem.quantity, orderItem.price, orderItem.subtotal, orderItem.customizations, orderItem.createdAt.toISOString(), orderItem.updatedAt.toISOString());
        ...
        INSERT INTO "Order" (id, orderNumber, userId, type, status, subtotal, tax, discount, total, createdAt, updatedAt)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(order.id, order.orderNumber, order.userId, order.type, order.status, 0, 0, 0, 0, order.createdAt.toISOString(), order.updatedAt.toISOString());
            // Add multiple items
            const orderItems = [
    ...
    describe('Complete Menu Flow Integration', () => {
      ...
      `).all(mainCourse.id);
            ...
            const ingredients = JSON.parse(burger.ingredients);
            const stockAvailable = ingredients.every((ingredient) => {
                const stock = db.prepare('SELECT * FROM Inventory WHERE id = ?').get(ingredient.id);
        ...
        INSERT INTO "Order" (id, orderNumber, userId, type, status, tableId, subtotal, tax, discount, total, createdAt, updatedAt)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(order.id, order.orderNumber, order.userId, order.type, order.status, order.tableId, burger.price, 0, 0, burger.price, order.createdAt.toISOString(), order.updatedAt.toISOString());
