...
describe('Menu Management', () => {
  ...
  describe('Menu Item Search and Filtering', () => {
    ...
    it('should filter menu items by price range', () => {
      ...
      expect(itemsInRange.every((item: any) => item.price >= minPrice && item.price <= maxPrice)).toBe(true);
    });
    ...
    it('should get menu items with category information', () => {
      ...
      const itemWithCategory = itemsWithCategory.find((item: any) => item.categoryId !== null);
    ...
    it('should identify best-selling items', () => {
      ...
      orders.forEach(order => {
        ...
        `).run(
          Factory.createMenuItem().id,
          order.id,
          testData.menuItems.burger.id,
          1,
          testData.menuItems.burger.price,
          'COMPLETED',
          new Date().toISOString(),
          new Date().toISOString()
        );
