...
describe('Add-ons System', () => {
  ...
  describe('Addon Group Management', () => {
    ...
    it('should support required addon groups', () => {
      ...
      `).run(
        requiredGroup.id,
        requiredGroup.name,
        requiredGroup.description,
        requiredGroup.maxSelections,
        requiredGroup.isRequired ? 1 : 0,
        requiredGroup.displayOrder,
        requiredGroup.createdAt.toISOString(),
        requiredGroup.updatedAt.toISOString()
      );
    ...
    it('should get addon groups for category', () => {
      ...
      groups.forEach(group => {
        ...
        db.prepare(`
          ...
          VALUES (?, ?, ?, ?)
        `).run(
          Factory.createAddon(group.id).id,
          testData.categories.foodCategory.id,
          group.id,
          new Date().toISOString()
        );
      });
  ...
  describe('Order Item Addons', () => {
    ...
    it('should add addon to order item', () => {
      ...
      `).run(
        ...
        orderItemAddon.addonId,
        orderItemAddon.quantity,
        orderItemAddon.price,
        new Date().toISOString()
      );
  ...
  describe('Addon Validation', () => {
    ...
    it('should enforce required addon groups', () => {
      ...
      const selectedAddons: any[] = []; // No selection
