...
describe('Authentication & Authorization', () => {
    ...
    describe('User Authentication', () => {
        ...
        it('should generate and verify refresh tokens', () => {
            ...
            const decoded = jwt.verify(refreshToken, process.env.REFRESH_SECRET || 'test-refresh-secret');
            expect(decoded.id).toBe(user.id);
        });
        ...
        it('should handle token expiration', () => {
            const user = Factory.createUser();
            const expiredToken = jwt.sign({ id: user.id, email: user.email, role: user.role }, process.env.JWT_SECRET || 'test-secret', { expiresIn: '-1h' } // Already expired
            );
    ...
    describe('Session Management', () => {
        ...
        it('should handle session timeout correctly', async () => {
            const user = Factory.createUser();
            const shortLivedToken = jwt.sign({ id: user.id, email: user.email, role: user.role }, process.env.JWT_SECRET || 'test-secret', { expiresIn: '1s' });
    ...
    describe('Password Management', () => {
        ...
        it('should handle password reset flow', async () => {
            ...
            const resetToken = jwt.sign({ id: user.id, purpose: 'password-reset' }, process.env.JWT_SECRET || 'test-secret', { expiresIn: '1h' });
            const decoded = jwt.verify(resetToken, process.env.JWT_SECRET || 'test-secret');
            expect(decoded.id).toBe(user.id);
