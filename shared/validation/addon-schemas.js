...
export const CategoryAddonGroupSchema = BaseCategoryAddonGroupSchema.refine(data => data.minSelections <= data.maxSelections, {
...
export const UpdateCategoryAddonGroupSchema = BaseCategoryAddonGroupSchema.partial()
    .required({ id: true })
    .refine(data => {
...
export const OrderItemAddonSchema = BaseOrderItemAddonSchema.refine(data => {
    const expectedTotal = Math.round(data.unitPrice * data.quantity * 100) / 100;
 ...
 */
export function validateAddonSelections(selections, categoryRules) {
    const errors = [];
 ...
 */
export function calculateAddonTotal(selections, addonPrices) {
 ...
 */
export function validatePricePrecision(price) {
