...
export const idSchema = z.string().min(1, 'ID is required').refine((val) => {
...
}).refine((data) => data.itemName || data.name, { message: 'Item name is required', path: ['itemName'] }).refine((data) => data.currentStock !== undefined || data.currentQuantity !== undefined, { message: 'Current stock is required', path: ['currentStock'] }).refine((data) => data.minimumStock !== undefined || data.minimumQuantity !== undefined, { message: 'Minimum stock is required', path: ['minimumStock'] });
