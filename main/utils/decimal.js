...
export function toDecimal(value) {
    ...
    try {
        ...
        logError(error, 'Decimal Conversion Error');
...
export function decimalToNumber(value) {
    ...
    if (typeof value === 'object' && 'toNumber' in value) {
        try {
            ...
            if (!isFinite(num)) {
                logError(new Error(`Invalid decimal value converted to ${num}`), 'Decimal Conversion');
                return 0;
            }
            return num;
        }
 ...
 */
export function decimalToString(value, precision = 2) {
    if (!value)
        return '0.00';
    try {
        ...
        logError(error, 'Decimal String Conversion Error');
        return '0.00';
    }
}
/**
...
export function addDecimals(a, b) {
    try {
        ...
        logError(error, 'Decimal Addition Error');
        return new DecimalJS(0);
    }
}
/**
...
export function subtractDecimals(a, b) {
    try {
        ...
        logError(error, 'Decimal Subtraction Error');
        return new DecimalJS(0);
    }
}
/**
...
export function multiplyDecimals(a, b) {
    try {
        ...
        logError(error, 'Decimal Multiplication Error');
        return new DecimalJS(0);
    }
}
/**
 ...
 */
export function divideDecimals(a, b, precision = 2) {
    try {
        // Convert to DecimalJS if needed
        const decimalA = toDecimal(a);
        const decimalB = typeof b === 'number' ? new DecimalJS(b) : toDecimal(b);
        ...
        logError(error, 'Decimal Division Error');
        return new DecimalJS(0);
    }
}
/**
...
export function sumDecimals(values) {
    try {
        ...
        logError(error, 'Decimal Sum Error');
        return new DecimalJS(0);
    }
}
/**
 ...
 */
export function averageDecimals(values) {
    if (values.length === 0)
        return new DecimalJS(0);
 ...
 */
export function validateCurrencyAmount(value, min = 0, max = 999999.99) {
    ...
    try {
        ...
        logError(error, 'Currency Validation Error');
        return false;
    }
}
/**
...
export function fromJSON(json) {
    try {
        ...
        logError(error, 'Decimal JSON Conversion Error');
        return new DecimalJS(0);
    }
}
/**
 ...
 */
export function formatAsCurrency(value, locale = 'en-US', currency = 'USD') {
    try {
        const num = typeof value === 'object' && value && 'toNumber' in value
            ? value.toNumber()
            : typeof value === 'number'
                ? value
                : 0;
        ...
        logError(error, 'Currency Formatting Error');
