...
import { getDatabaseIntegrityChecker } from './databaseIntegrityChecker';
...
export class UpdateSafety {
    constructor() {
        this.integrityChecker = getDatabaseIntegrityChecker();
        // Crash detection constants
        this.CRASH_WINDOW_MS = 2 * 60 * 1000; // 2 minutes
        this.MAX_CRASH_COUNT = 3; // Max crashes before rollback
        try {
    ...
    async initialize() {
        try {
            ...
            logError(error, 'UpdateSafety.initialize');
        }
    }
    /**
     ...
     */
    async createPreUpdateBackup(newVersion) {
        ...
        try {
            ...
            if (!healthCheck.isHealthy) {
                logWarning('Database health check failed before backup, proceeding with caution', 'UpdateSafety');
            }
            ...
            logError(error, 'UpdateSafety.createPreUpdateBackup');
            return {
                success: false,
                error: errorMessage,
            };
    ...
    async verifyBackupIntegrity(backupPath) {
        try {
            ...
            logError(error, 'UpdateSafety.verifyBackupIntegrity');
            return false;
        }
    }
    /**
     ...
     */
    async cleanOldPreUpdateBackups(keepCount) {
        try {
            ...
            logError(error, 'UpdateSafety.cleanOldPreUpdateBackups');
        }
    }
    /**
    ...
    async getLatestBackup() {
        try {
            ...
            const backupsWithStats = await Promise.all(backups.map(async (backupPath) => ({
                path: backupPath,
            ...
            logError(error, 'UpdateSafety.getLatestBackup');
            return null;
        }
    }
    /**
    ...
    async verifyBackupExists() {
        try {
            ...
            if (!latestBackup) {
                logError(new Error('No pre-update backup found'), 'UpdateSafety');
                return false;
            }
            const backupValid = await this.verifyBackupIntegrity(latestBackup);
            if (!backupValid) {
                logError(new Error('Latest backup is invalid'), 'UpdateSafety');
                return false;
            }
            ...
            logError(error, 'UpdateSafety.verifyBackupExists');
            return false;
        }
    }
    /**
    ...
    async verifyPostUpdateIntegrity() {
        ...
        try {
            ...
            const healthCheck = await this.integrityChecker.runHealthCheck();
            if (!healthCheck.isHealthy) {
                logError(new Error('Post-update health check failed'), 'UpdateSafety');
                logError(new Error(`Errors: ${healthCheck.errors.join('; ')}`), 'UpdateSafety');
                return false;
            }
            ...
            if (crashDetection.shouldRollback) {
                logError(new Error(`Too many crashes detected (${crashDetection.crashCount})`), 'UpdateSafety');
                return false;
            }
            ...
            logError(error, 'UpdateSafety.verifyPostUpdateIntegrity');
            return false;
        }
    }
    /**
    ...
    async handleUpdateFailure() {
        logError(new Error('Update failure detected - attempting recovery'), 'UpdateSafety');
        try {
            // 1. Get latest backup
            const latestBackup = await this.getLatestBackup();
            if (!latestBackup) {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            logError(error, 'UpdateSafety.handleUpdateFailure');
            return {
                success: false,
                error: errorMessage,
            };
    ...
    async recordStartup(version) {
        try {
            ...
            logError(error, 'UpdateSafety.recordStartup');
        }
    }
    /**
     * Record successful startup (called after app runs for CRASH_WINDOW_MS)
     */
    async recordSuccessfulStartup() {
        try {
            const crashData = await this.loadCrashDetection();
            crashData.crashCount = 0;
            ...
            logError(error, 'UpdateSafety.recordSuccessfulStartup');
        }
    }
    /**
     * Check crash history and determine if rollback is needed
     */
    async checkCrashHistory() {
        try {
            const crashData = await this.loadCrashDetection();
            const now = Date.now();
            ...
            logError(error, 'UpdateSafety.checkCrashHistory');
    ...
    async saveBackupMetadata(metadata) {
        try {
            ...
            logError(error, 'UpdateSafety.saveBackupMetadata');
        }
    }
    /**
     * Load crash detection data
     */
    async loadCrashDetection() {
        try {
            ...
            logError(error, 'UpdateSafety.loadCrashDetection');
        }
        // Return default
        return {
     ...
     */
    async saveCrashDetection(data) {
        try {
            await fs.writeJson(this.crashDetectionPath, data, { spaces: 2 });
        }
        catch (error) {
            logError(error, 'UpdateSafety.saveCrashDetection');
        }
    }
}
// Singleton instance
let _instance = null;
export function getUpdateSafety() {
    if (!_instance) {
...
// Export singleton
export const updateSafety = new Proxy({}, {
    get(_target, prop) {
        return getUpdateSafety()[prop];
    },
});
