...
export class DatabaseIntegrityChecker {
  ...
  async runHealthCheck(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      if (!result.checks.requiredTables) {
        result.errors.push(
          `Missing required tables: ${tableCheck.missingTables.join(', ')}`
        );
        result.isHealthy = false;
      }
      ...
      if (result.isHealthy) {
        ...
        logError(
          new Error(`Errors: ${result.errors.join('; ')}`),
          'DatabaseIntegrityChecker'
        );
      }
      ...
      logError(error as Error, 'DatabaseIntegrityChecker.runHealthCheck');
      return result;
    }
  }
  ...
  async checkDatabaseConnection(): Promise<boolean> {
    try {
      ...
      logError(error as Error, 'DatabaseIntegrityChecker.checkConnection');
      return false;
    }
  }
    ...
    try {
      ...
      if (result && result.length > 0) {
        ...
        if (integrityStatus === 'ok') {
          ...
          logError(
            new Error(`Integrity check failed: ${integrityStatus}`),
            'DatabaseIntegrityChecker'
          );
          return false;
        }
      ...
      logError(error as Error, 'DatabaseIntegrityChecker.checkIntegrity');
      return false;
    }
  }
  ...
  async verifyRequiredTables(): Promise<{
    ...
    tableChecks: Record<string, TableCheckResult>;
  }> {
    ...
    const tableChecks: Record<string, TableCheckResult> = {};
    ...
    try {
      ...
      if (allTablesExist) {
        ...
        logError(
          new Error(`Missing tables: ${missingTables.join(', ')}`),
          'DatabaseIntegrityChecker'
        );
      }
      ...
      logError(error as Error, 'DatabaseIntegrityChecker.verifyTables');
      return {
   ...
   */
  async checkDatabaseWriteable(): Promise<boolean> {
    try {
      ...
      logError(error as Error, 'DatabaseIntegrityChecker.checkWriteable');
      return false;
    }
  }
  ...
  async quickHealthCheck(): Promise<boolean> {
    try {
      ...
      logError(error as Error, 'DatabaseIntegrityChecker.quickHealthCheck');
      return false;
    }
  }
...
// Singleton instance
let _instance: DatabaseIntegrityChecker | null = null;

export function getDatabaseIntegrityChecker(): DatabaseIntegrityChecker {
  if (!_instance) {
...
export const databaseIntegrityChecker = new Proxy(
  {} as DatabaseIntegrityChecker,
  {
    get(_target, prop) {
      return (getDatabaseIntegrityChecker() as any)[prop];
    },
  }
);
