 ...
 */
export async function generateSimpleKitchenTicket(order, onlyUnprinted = false, cancelledItems = [], updatedItemIds = [], changeDetails = []) {
    ...
    const orderTypeDisplay = orderType === 'DINE_IN'
        ? 'DINE IN'
        : orderType === 'TAKEOUT'
            ? 'TAKEAWAY'
            : orderType === 'DELIVERY'
    ...
    if (cancelledItems && cancelledItems.length > 0) {
        ...
        return await generateUpdateTicket(order, onlyUnprinted, updatedItemIds, changeDetails);
    }
    else {
 ...
 */
function parseNotesForCustomizations(notes) {
 ...
 */
function generateRemovalTicket(order, cancelledItems) {
    const printData = [];
    ...
    printableCancelledItems.forEach((item, index) => {
        // CRITICAL FIX: Ensure we get the actual menu item name from the relation
        const name = (item.menuItem && item.menuItem.name) || // Try menuItem relation first
            item.name ||
            item.menuItemName ||
        ...
        if (item.notes) {
            const { regularNotes, removedIngredients, otherCustomizations } = parseNotesForCustomizations(item.notes);
            if (regularNotes) {
                printData.push({
                    type: 'text',
    ...
    console.log(`REMOVAL TICKET GENERATED: ${printableCancelledItems.length} printable items removed (${cancelledItems.length} total)`);
 ...
 */
async function generateUpdateTicket(order, onlyUnprinted, updatedItemIds, changeDetails = []) {
    const printData = [];
    ...
    if (onlyUnprinted) {
        ...
        console.log(`📋 UNPRINTED FILTER: ${itemsToProcess.length} unprinted items found`);
    }
    // Apply update filter
    let matchingItems = itemsToProcess.filter((item) => updatedItemIds.includes(item.id));
    // TIMING FIX: If no matches found, implement retry mechanism for database timing issues
    if (matchingItems.length === 0) {
        console.log('⏱️ TIMING FIX: No items found on first attempt, implementing retry mechanism...');
        ...
        matchingItems = itemsToProcess.filter((item) => updatedItemIds.includes(item.id));
        console.log('🎯 RETRY RESULTS:', {
            foundAfterDelay: matchingItems.length,
            matchedIds: matchingItems.map((item) => item.id),
        });
    }
    ...
    if (matchingItems.length === 0 && changeDetails && changeDetails.length > 0) {
        console.log('🔄 FALLBACK: Using changeDetails to generate net change ticket');
    ...
    console.log('🎯 UPDATE FILTER RESULTS:', {
        ...
        matchedIds: matchingItems.map((item) => item.id),
        unmatchedRequestedIds: updatedItemIds.filter(id => !matchingItems.find((item) => item.id === id)),
    });
    ...
    if (matchingItems.length === 0) {
        ...
        console.log('😨 DEBUGGING: Frontend sent these IDs but none were found even after delay:', {
            requestedIds: updatedItemIds,
            availableIds: itemsToProcess.map((item) => item.id),
    ...
    printableMatchingItems.forEach((item, index) => {
        // CRITICAL FIX: Ensure we get the actual menu item name from the relation
        const name = (item.menuItem && item.menuItem.name) || // Try menuItem relation first
            item.name ||
            item.menuItemName ||
        ...
        const itemLine = formatUpdateItemLine(name, quantity, item.id, changeDetails);
        printData.push({
            type: 'text',
            value: itemLine,
            style: { fontSize: '14px', fontFamily: 'monospace', fontWeight: 'bold' },
    ...
    console.log(`✅ UPDATE TICKET GENERATED: ${printableMatchingItems.length} printable items updated (${matchingItems.length} total matching)`);
    return JSON.stringify(printData);
}
/**
 ...
 */
function generateDebugTicket(order, requestedIds, availableItems) {
    const printData = [];
    // Header
    printData.push({
        type: 'text',
    ...
    console.log(`🔍 DEBUG TICKET GENERATED: ${requestedIds.length} requested, ${availableItems.length} available`);
    return JSON.stringify(printData);
}
/**
 ...
 */
function generateStandardTicket(order, onlyUnprinted) {
    const printData = [];
    let itemsToProcess = order?.items || [];
    ...
    const orderType = order?.type || 'DINE_IN';
    const locationDisplay = orderType === 'DINE_IN'
    ...
    console.log(`✅ STANDARD TICKET GENERATED: ${printableItems.length} printable items (${itemsToProcess.length} total items)`);
    return JSON.stringify(printData);
}
/**
 * Helper function to format update line with net changes
 */
function formatUpdateItemLine(name, quantity, itemId, changeDetails) {
    // Find change details for this item
    const change = changeDetails?.find(c => c.itemId === itemId || c.id === itemId);
    ...
    if (change) {
        ...
        if (change.type === 'NEW' || change.changeType === 'NEW') {
            ...
            return (`REDUCE: ${name}`.padEnd(25) + `-${Math.abs(netChange)}`.padStart(8));
        }
            ...
            return (`REDUCE: ${name}`.padEnd(25) + `-${change.removedQuantity}`.padStart(8));
        }
 ...
 */
function generateNetChangeTicket(order, changeDetails) {
    console.log('GENERATING NET CHANGE TICKET from changeDetails:', changeDetails);
    const printData = [];
    const currentDate = new Date();
    const dateString = currentDate.toLocaleDateString('en-US', {
        month: 'numeric',
    ...
    changeDetails.forEach((change, index) => {
        ...
        const orderItem = order?.items?.find((item) => item.id === changeId);
    ...
    console.log(`✅ NET CHANGE TICKET GENERATED: ${changeDetails.length} changes processed`);
    return JSON.stringify(printData);
}
