...
class BackupManager {
    ...
    async initialize() {
        try {
            ...
            this.isInitialized = false;
            logger.error(`Failed to create backup directory: ${error}`, 'BackupManager');
        }
    }
    /**
        ...
        try {
            ...
            return new Promise((resolve, reject) => {
                output.on('close', async () => {
                    logger.info(`Backup created successfully: ${backupFileName}`, 'BackupManager');
                ...
                archive.on('warning', (err) => {
                    if (err.code === 'ENOENT') {
                        logger.warn(`File not found during backup: ${err.path}`, 'BackupManager');
                    }
                    else {
                        reject(err);
                    }
    ...
    async listBackups() {
        if (!this.isInitialized) {
            logger.error('Cannot list backups, manager not initialized.', 'BackupManager');
            return [];
        }
        try {
    ...
    async restoreBackup(timestamp) {
        ...
        const confirmation = await dialog.showMessageBox({
            ...
            detail: 'This will overwrite your current data. The application will restart after the restore.',
        });
        ...
        try {
            ...
            logger.info(`Backup extracted to temporary directory: ${tempDir}`, 'BackupManager');
            // Restore database
            ...
            if (fs.existsSync(dbSourcePath)) {
                ...
                logger.info(`Database restored from backup: ${dbSourcePath}`, 'BackupManager');
            }
            // Restore settings
            ...
            if (fs.existsSync(settingsSourcePath)) {
                ...
                logger.info(`Settings restored from backup: ${settingsSourcePath}`, 'BackupManager');
            }
            // Clean up temporary directory
            await fs.remove(tempDir);
            logger.info(`Backup restoration completed successfully: ${backupFileName}`, 'BackupManager');
            return { success: true };
        }
        catch (error) {
     ...
     */
    scheduleAutoBackups(intervalHours) {
        if (!this.isInitialized) {
            logger.warn('Cannot schedule auto-backups, manager not initialized.', 'BackupManager');
            return;
        }
        logger.info(`Scheduling automatic backups every ${intervalHours} hours.`, 'BackupManager');
     ...
     * @returns A promise that resolves with the backup result
     */
    async createPreUpdateBackup(version) {
        if (!this.isInitialized) {
        ...
        try {
            ...
            await fs.writeJson(`${backupFilePath}.meta`, metadata, { spaces: 2 });
            logger.info(`Pre-update backup created successfully: ${backupFileName}`, 'BackupManager');
    ...
    async cleanOldPreUpdateBackups(keepCount) {
        try {
            ...
            const backupsWithStats = await Promise.all(backupFiles.map(async (backup) => ({
                ...backup,
            ...
            if (backupsToDelete.length > 0) {
                logger.info(`Cleaned ${backupsToDelete.length} old pre-update backups`, 'BackupManager');
            }
        }
        catch (error) {
     ...
     */
    async getLatestPreUpdateBackup() {
        try {
            const preUpdateDir = path.join(this.backupDir, 'pre-update');
            if (!await fs.pathExists(preUpdateDir)) {
                return null;
            ...
            const backupsWithStats = await Promise.all(backupFiles.map(async (file) => ({
                path: file,
     ...
     * @returns A promise that resolves with the restore result
     */
    async restoreFromBackupFile(backupPath) {
        if (!this.isInitialized) {
            await this.initialize();
        }
        try {
            ...
            if (await fs.pathExists(this.dbPath)) {
                ...
                logger.info(`Current database backed up to ${corruptedBackupPath}`, 'BackupManager');
            }
            // Restore from backup
            await fs.copy(backupPath, this.dbPath, { overwrite: true });
            logger.info(`Database restored successfully from ${backupPath}`, 'BackupManager');
            return { success: true };
        }
        catch (error) {
...
export const backupManager = new Proxy({}, {
    get(_target, prop) {
        return getBackupManager()[prop];
    }
});
