...
export class AddonInvoiceGenerator {
     ...
     */
    async generateEnhancedThermalInvoiceWithAddons(order, businessInfo, getResourcesPath) {
        ...
        return this.generateAddonEnhancedInvoice(enrichedOrder, businessInfo, getResourcesPath);
    }
    /**
    ...
    async enrichOrderItemsWithAddons(order) {
        ...
        const enrichedItems = await Promise.all(order.items.map(async (item) => {
            try {
                ...
                const addonTotal = addons.reduce((sum, addon) => sum + Number(addon.totalPrice), 0);
                return {
                    ...item,
                    addons: addons,
     ...
     */
    generateAddonEnhancedInvoice(order, businessInfo, getResourcesPath) {
        const printData = [];
            ...
            if (groupedItem.addons.length > 0) {
                ...
                <span style="margin-left: 15px;">${groupName}:</span>
                    ...
                    addons.forEach(addon => {
                        ...
                        const marginLeft = Object.keys(addonsByGroup).length > 1 ? '25px' : '15px';
     ...
     */
    async generateThermalInvoiceWithAddons(order, businessInfo) {
     ...
     */
    async generatePDFInvoiceWithAddons(order, businessInfo) {
