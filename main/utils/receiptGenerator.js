...
export class LebanesReceiptGenerator {
     ...
     */
    static buildReceiptContent(doc, data) {
     ...
     */
    static addBusinessHeader(doc, businessInfo) {
        // Business Name
        doc
            .fontSize(16)
            .font('Helvetica-Bold')
     ...
     */
    static addReceiptInfo(doc, order) {
        ...
        doc
            ...
            .text(`Server: ${order.user?.firstName || 'N/A'} ${order.user?.lastName || ''}`);
        // Separator Line
     ...
     */
    static addItemsSection(doc, order) {
     ...
     */
    static addTotalsSection(doc, calculations) {
     ...
     */
    static addLegalFooter(doc, businessInfo, calculations) {
        ...
        doc
            ...
            .text(`VAT Amount: $${calculations.vatAmount.toFixed(2)} (${(calculations.vatRate * 100).toFixed(0)}%)`, { align: 'center' });
        doc.moveDown(0.3);
     ...
     */
    static generateThermalReceipt(receiptData) {
        ...
        const formatTableRow = (item, qty, unitPrice, total) => {
            // Adjust column widths for 32-character thermal printer
            const itemWidth = 12;
            const qtyWidth = 3;
            const priceWidth = 6;
            const totalWidth = 8;
            const truncatedItem = item.length > itemWidth ? item.substring(0, itemWidth - 1) : item;
        ...
        order.items.forEach((item) => {
            ...
            totalQuantity += qty;
            receipt +=
                formatTableRow(name, qty.toString(), unitPrice.toFixed(1), totalPrice.toFixed(1)) + '\n';
        });
        receipt += '-'.repeat(LINE_WIDTH) + '\n';
        // Totals section
     ...
     */
    static generateKitchenTicket(receiptData) {
        ...
        lines.push(`DATE ${dateString}${' '.repeat(dateTimeSpacing)}TIME ${timeString}`);
        lines.push('');
...
LebanesReceiptGenerator.VAT_RATE = 0.11; // 11% VAT rate for Lebanon
LebanesReceiptGenerator.RECEIPT_WIDTH = 80; // 80mm thermal printer width
