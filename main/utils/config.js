...
const validateRequiredVars = () => {
    ...
    if (missing.length > 0) {
        ...
        if (isProduction) {
            ...
            console.warn('⚠️  Using default values for development. This is NOT secure for production!');
        }
    }
};
...
const generateSecureDefault = (length = 32) => {
...
const createConfig = () => {
    ...
    const config = {
        ...
        APP_VERSION: parseString(process.env.APP_VERSION, defaultConfig.APP_VERSION),
        DATABASE_URL: process.env.DATABASE_URL ||
            'postgresql://postgres:password@localhost:5432/mr5_pos_dev',
        DB_CONNECTION_LIMIT: parseNumber(process.env.DB_CONNECTION_LIMIT, defaultConfig.DB_CONNECTION_LIMIT),
        DB_POOL_TIMEOUT: parseNumber(process.env.DB_POOL_TIMEOUT, defaultConfig.DB_POOL_TIMEOUT),
        JWT_SECRET: process.env.JWT_SECRET || generateSecureDefault(64),
        JWT_REFRESH_SECRET: process.env.JWT_REFRESH_SECRET || generateSecureDefault(64),
        JWT_EXPIRES_IN: parseString(process.env.JWT_EXPIRES_IN, defaultConfig.JWT_EXPIRES_IN),
        JWT_REFRESH_EXPIRES_IN: parseString(process.env.JWT_REFRESH_EXPIRES_IN, defaultConfig.JWT_REFRESH_EXPIRES_IN),
        SESSION_SECRET: process.env.SESSION_SECRET || generateSecureDefault(64),
        SESSION_NAME: parseString(process.env.SESSION_NAME, defaultConfig.SESSION_NAME),
        ...
        LOG_FILE_ENABLED: parseBoolean(process.env.LOG_FILE_ENABLED, defaultConfig.LOG_FILE_ENABLED),
        LOG_FILE_PATH: parseString(process.env.LOG_FILE_PATH, defaultConfig.LOG_FILE_PATH),
        ENABLE_CORS: parseBoolean(process.env.ENABLE_CORS, defaultConfig.ENABLE_CORS),
        ENABLE_DEV_TOOLS: parseBoolean(process.env.ENABLE_DEV_TOOLS, defaultConfig.ENABLE_DEV_TOOLS),
        PRINTER_ENABLED: parseBoolean(process.env.PRINTER_ENABLED, defaultConfig.PRINTER_ENABLED),
        PRINTER_TYPE: parseString(process.env.PRINTER_TYPE, defaultConfig.PRINTER_TYPE),
        PRINTER_WIDTH: parseNumber(process.env.PRINTER_WIDTH, defaultConfig.PRINTER_WIDTH),
        ALLOWED_ORIGINS: parseStringArray(process.env.ALLOWED_ORIGINS, defaultConfig.ALLOWED_ORIGINS),
    };
    // Validate secrets
...
export const getConfigReport = () => {
    return {
        environment: config.NODE_ENV,
        security: {
            secretsConfigured: !!(process.env.JWT_SECRET && process.env.SESSION_SECRET),
