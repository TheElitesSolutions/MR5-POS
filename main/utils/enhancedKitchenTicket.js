 ...
 */
export async function generateEnhancedKitchenTicket(order, onlyUnprinted = false, cancelledItems = [], updatedItemIds = [], changeDetails = []) {
    ...
    const newItemsInOrder = orderItems.filter((item) => {
    ...
    console.log('🔍 ORDER ANALYSIS:', {
        ...
        newItemIds: newItemsInOrder.map((i) => i.id),
    ...
    if (cancelledItems && cancelledItems.length > 0) {
        ...
        return generateEnhancedUpdateTicket(order, onlyUnprinted, updatedItemIds, changeDetails);
    }
    else {
 ...
 */
function generateEnhancedStandardTicket(order, onlyUnprinted) {
    const printData = [];
    ...
    filteredOutItems.forEach((item, index) => {
    ...
    console.log(`✅ ENHANCED STANDARD TICKET: ${printableItems.length} printable items (${itemsToProcess.length} total items)`);
    return JSON.stringify(printData);
}
/**
 * Generate enhanced removal ticket
 */
function generateEnhancedRemovalTicket(order, cancelledItems) {
    const printData = [];
    const currentDate = new Date();
    const timeString = currentDate.toLocaleTimeString('en-US', {
        hour: 'numeric',
 ...
 */
function generateEnhancedUpdateTicket(order, onlyUnprinted, updatedItemIds, changeDetails) {
    const printData = [];
    const currentDate = new Date();
    const timeString = currentDate.toLocaleTimeString('en-US', {
        hour: 'numeric',
    ...
    const itemsToProcess = order?.items?.filter((item) => updatedItemIds.includes(item.id)) || [];
    console.log('========================================');
    ...
    printableUpdatedItems.forEach((item, index) => {
        const itemName = getItemName(item);
        const quantity = item.quantity || 0;
        const change = changeDetails.find(c => c.itemId === item.id || c.id === item.id);
        if (index > 0) {
            printData.push({
                type: 'text',
                value: '',
 ...
 */
function filterPrintableAddons(addons) {
