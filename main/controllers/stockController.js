...
import { CreateInventoryItemSchema, AdjustStockSchema, } from '../../shared/validation-schemas';
...
export class StockController extends BaseController {
    constructor() {
        super();
        this.inventoryModel = new InventoryModel(prisma);
     ...
     */
    transformStockItemData(frontendData) {
        return {
            itemName: frontendData.name || frontendData.itemName,
            category: frontendData.category || 'Other',
            currentStock: toDecimal(frontendData.currentQuantity || frontendData.currentStock || 0),
            minimumStock: toDecimal(frontendData.minimumQuantity || frontendData.minimumStock || 0),
     ...
     */
    transformPartialStockItemData(frontendData) {
        const transformed = {};
    ...
    registerHandlers() {
        ...
        this.registerHandler(INVENTORY_CHANNELS.GET_ALL, async (_event) => {
            try {
        ...
        this.registerHandler(INVENTORY_CHANNELS.GET_BY_ID, async (_event, itemId) => {
            try {
        ...
        this.registerHandler(INVENTORY_CHANNELS.UPDATE, async (_event, updatePayload) => {
            try {
                const { id: itemId, updates: itemData } = updatePayload;
                logDebug(`Updating inventory item ${itemId}`, 'StockController');
                logDebug(`Update data: ${JSON.stringify(itemData)}`, 'StockController');
                // Transform frontend data to backend format
                const transformedData = this.transformPartialStockItemData(itemData);
                logDebug(`Transformed update data: ${JSON.stringify({
                    ...transformedData,
                ...
                const result = await this.inventoryModel.update(itemId, transformedData);
                return this.createSuccessResponse(result.data);
            }
            catch (error) {
                return this.createErrorResponse(error instanceof Error ? error : new Error(String(error)));
        ...
        this.registerHandler(INVENTORY_CHANNELS.CREATE, async (_event, itemData) => {
            try {
                // Runtime validation with Zod
                const validation = validateWithSchema(CreateInventoryItemSchema, itemData, 'CreateInventoryItem');
                ...
                logDebug(`Creating inventory item: ${JSON.stringify(validatedData)}`, 'StockController');
                // Transform frontend data to backend format
                const transformedData = this.transformStockItemData(validatedData);
                logDebug(`Transformed create data: ${JSON.stringify({
                    ...transformedData,
                    currentStock: transformedData.currentStock.toString(),
                    minimumStock: transformedData.minimumStock.toString(),
                    costPerUnit: transformedData.costPerUnit.toString(),
                ...
                if (!result.success || !result.data) {
                    return this.createErrorResponse(new Error(result.error || 'Failed to create inventory item'));
                }
                return this.createSuccessResponse(result.data);
            }
            catch (error) {
        ...
        this.registerHandler(INVENTORY_CHANNELS.DELETE, async (_event, itemId) => {
            try {
        ...
        // Adjust stock level
        this.registerHandler(INVENTORY_CHANNELS.ADJUST_STOCK, async (_event, adjustmentData) => {
            try {
                // Runtime validation with Zod
                const validation = validateWithSchema(AdjustStockSchema, adjustmentData, 'AdjustStock');
                if (!validation.success) {
                ...
                logDebug(`Adjusting stock for item ${itemId} by ${adjustment.quantity}`, 'StockController');
                const result = await this.inventoryModel.adjustStock(itemId, toDecimal(adjustment.quantity), adjustment.reason || 'Stock adjustment');
                return this.createSuccessResponse(result.data);
            }
            catch (error) {
                return this.createErrorResponse(error instanceof Error ? error : new Error(String(error)));
        ...
        // Get low stock items
        this.registerHandler(INVENTORY_CHANNELS.GET_LOW_STOCK, async (_event) => {
            try {
        ...
        // Get categories
        this.registerHandler(INVENTORY_CHANNELS.GET_CATEGORIES, async (_event) => {
            try {
        ...
        this.registerHandler('inventory:getByCategory', async (_event, category) => {
            try {
        ...
        this.registerHandler(INVENTORY_CHANNELS.UPDATE_CATEGORY_NAME, async (_event, { oldName, newName }) => {
            try {
                logDebug(`Updating category name from "${oldName}" to "${newName}"`, 'StockController');
                const result = await this.inventoryModel.updateCategoryName(oldName, newName);
        ...
        this.registerHandler(INVENTORY_CHANNELS.GET_MENU_ITEM_INGREDIENTS, async (_event, { menuItemId }) => {
            try {
                logDebug(`Fetching ingredients for menu item ${menuItemId}`, 'StockController');
                // Use the inventory service to get the menu item's ingredients
                const result = await this.inventoryService.getMenuItemIngredients(menuItemId);
                if (!result.success) {
                    throw new Error(result.error || 'Failed to get menu item ingredients');
                }
        ...
        this.registerHandler(INVENTORY_CHANNELS.UPDATE_MENU_ITEM_INGREDIENTS, async (_event, { menuItemId, ingredients, }) => {
            try {
                logDebug(`Updating ingredients for menu item ${menuItemId}`, 'StockController');
                const result = await this.inventoryService.updateMenuItemIngredients(menuItemId, ingredients);
                return this.createSuccessResponse(result.data || null);
            }
            catch (error) {
                return this.createErrorResponse(error instanceof Error ? error : new Error(String(error)));
        ...
        this.registerHandler(INVENTORY_CHANNELS.CHECK_STOCK_AVAILABILITY, async (_event, { orderItems, }) => {
            try {
                logDebug(`Checking stock availability for ${orderItems.length} items`, 'StockController');
                const result = await this.inventoryService.checkStockAvailability(orderItems);
                return this.createSuccessResponse(result.data || { available: true, unavailableItems: [] });
            }
            catch (error) {
                return this.createErrorResponse(error instanceof Error ? error : new Error(String(error)));
            }
