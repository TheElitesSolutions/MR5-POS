...
import { enhancedLogger, LogCategory, LogLevel, } from '../utils/enhanced-logger';
...
export class LogController extends BaseController {
    constructor() {
        ...
        enhancedLogger.info('LogController initialized', LogCategory.SYSTEM, 'LogController');
    }
    registerHandlers() {
        this.registerHandler(LOGGING_CHANNELS.GET_FILES, this.getLogFiles.bind(this));
        this.registerHandler(LOGGING_CHANNELS.GET_CONTENT, this.getLogContent.bind(this));
        this.registerHandler(LOGGING_CHANNELS.SEARCH, this.searchLogs.bind(this));
        this.registerHandler(LOGGING_CHANNELS.GET_STATS, this.getLogStats.bind(this));
        ...
        this.registerHandler(LOGGING_CHANNELS.WRITE_LOG, this.writeLog.bind(this));
    }
    unregisterHandlers() {
    ...
    async getLogFiles(_event) {
        try {
            ...
            /* const _duration = */ enhancedLogger.endTimer('getLogFiles', 'Retrieved log files list', LogCategory.FILESYSTEM, 'LogController');
            return {
                success: true,
                data: logFiles,
            ...
            enhancedLogger.error('Failed to get log files', LogCategory.FILESYSTEM, 'LogController', { error: error instanceof Error ? error.message : String(error) }, error instanceof Error ? error : undefined);
            return {
                success: false,
     ...
     */
    async getLogContent(_event, params) {
        try {
            ...
            /* const _duration = */ enhancedLogger.endTimer('getLogContent', 'Retrieved log file content', LogCategory.FILESYSTEM, 'LogController');
            return {
                success: true,
                data: lines,
            ...
            enhancedLogger.error('Failed to get log content', LogCategory.FILESYSTEM, 'LogController', { filePath: params.filePath }, error instanceof Error ? error : undefined);
            return {
                success: false,
                error: error instanceof Error ? error.message : String(error),
                timestamp: getCurrentLocalDateTime(),
     ...
     */
    async searchLogs(event, options) {
        try {
            enhancedLogger.startTimer('searchLogs');
            const { level, category, module, startTime, endTime, search, limit = 100, offset = 0, } = options;
            // Get all log files
            ...
            for (const logFile of logFiles) {
                ...
                try {
                    ...
                    enhancedLogger.warn(`Skipping log file ${logFile.name} due to error`, LogCategory.FILESYSTEM, 'LogController', { error: error instanceof Error ? error.message : String(error) });
                    continue;
                }
                ...
                for (const line of lines) {
                    try {
                        ...
                        // Apply filters
                        if (level !== undefined && entry.levelCode < level)
                            continue;
            ...
            const paginatedResults = results
                ...
                .slice(offset, offset + limit);
            /* const _duration = */ enhancedLogger.endTimer('searchLogs', 'Searched logs', LogCategory.FILESYSTEM, 'LogController');
            return {
                success: true,
            ...
            enhancedLogger.error('Failed to search logs', LogCategory.FILESYSTEM, 'LogController', { options }, error instanceof Error ? error : undefined);
            return {
                success: false,
                error: error instanceof Error ? error.message : String(error),
                timestamp: getCurrentLocalDateTime(),
        ...
        try {
            ...
            if (logFiles.length > 0) {
                try {
                    ...
                    enhancedLogger.warn(`Error processing recent log file: ${error}`, LogCategory.FILESYSTEM, 'LogController');
                }
            }
            ...
            /* const _duration = */ enhancedLogger.endTimer('getLogStats', 'Generated log statistics', LogCategory.SYSTEM, 'LogController');
            return {
                success: true,
                data: stats,
            ...
            enhancedLogger.error('Failed to get log statistics', LogCategory.SYSTEM, 'LogController', {}, error instanceof Error ? error : undefined);
            return {
                success: false,
                error: error instanceof Error ? error.message : String(error),
                timestamp: getCurrentLocalDateTime(),
        ...
        try {
            ...
            for (const file of filesToDelete) {
                try {
                    ...
                    enhancedLogger.warn(`Failed to delete log file: ${file.name}`, LogCategory.FILESYSTEM, 'LogController', { error: error instanceof Error ? error.message : String(error) });
                }
            }
            /* const _duration = */ enhancedLogger.endTimer('clearLogs', 'Cleared old logs', LogCategory.FILESYSTEM, 'LogController');
            return {
                success: true,
                data: { deletedCount },
            ...
            enhancedLogger.error('Failed to clear logs', LogCategory.FILESYSTEM, 'LogController', {}, error instanceof Error ? error : undefined);
            return {
                success: false,
                error: error instanceof Error ? error.message : String(error),
                timestamp: getCurrentLocalDateTime(),
    ...
    async exportLogs(event, params) {
        try {
            ...
            // Write logs to file
            await fs.writeFile(normalizedPath, logs.map(log => JSON.stringify(log)).join('\n'), 'utf8');
            /* const _duration = */ enhancedLogger.endTimer('exportLogs', 'Exported logs to file', LogCategory.FILESYSTEM, 'LogController');
            return {
                success: true,
            ...
            enhancedLogger.error('Failed to export logs', LogCategory.FILESYSTEM, 'LogController', { outputPath: params.outputPath }, error instanceof Error ? error : undefined);
            return {
                success: false,
                error: error instanceof Error ? error.message : String(error),
                timestamp: getCurrentLocalDateTime(),
    ...
    async writeLog(_event, params) {
        try {
            ...
            if (logLevel === LogLevel.ERROR) {
                enhancedLogger.error(message, category || LogCategory.UI, module || 'Renderer', context);
            }
            else if (logLevel === LogLevel.WARN) {
                enhancedLogger.warn(message, category || LogCategory.UI, module || 'Renderer', context);
            }
            else if (logLevel === LogLevel.DEBUG) {
                enhancedLogger.debug(message, category || LogCategory.UI, module || 'Renderer', context);
            }
            else {
                enhancedLogger.info(message, category || LogCategory.UI, module || 'Renderer', context);
            }
            return {
                success: true,
                data: true,
            ...
            enhancedLogger.error('Failed to write renderer log', LogCategory.SYSTEM, 'LogController', { params }, error instanceof Error ? error : undefined);
            return {
                success: false,
                error: error instanceof Error ? error.message : String(error),
                timestamp: getCurrentLocalDateTime(),
