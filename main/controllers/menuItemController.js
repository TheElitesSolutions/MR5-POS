...
import { CreateMenuItemSchema, UpdateMenuItemSchema, } from '../../shared/validation-schemas';
...
export class MenuItemController extends BaseController {
    ...
    registerHandlers() {
        this.registerHandler(MENU_ITEM_CHANNELS.GET_ALL, this.getAllMenuItems.bind(this));
        this.registerHandler(MENU_ITEM_CHANNELS.GET_BY_ID, this.getMenuItemById.bind(this));
        this.registerHandler(MENU_ITEM_CHANNELS.GET_BY_CATEGORY, this.getMenuItemsByCategory.bind(this));
        this.registerHandler(MENU_ITEM_CHANNELS.GET_AVAILABLE, this.getAvailableMenuItems.bind(this));
        this.registerHandler(MENU_ITEM_CHANNELS.CREATE, this.createMenuItem.bind(this));
        this.registerHandler(MENU_ITEM_CHANNELS.UPDATE, this.updateMenuItem.bind(this));
        this.registerHandler(MENU_ITEM_CHANNELS.DELETE, this.deleteMenuItem.bind(this));
        this.registerHandler(MENU_ITEM_CHANNELS.SEARCH, this.searchMenuItems.bind(this));
        this.registerHandler(MENU_ITEM_CHANNELS.GET_STATS, this.getMenuItemStats.bind(this));
        // Category-specific handlers
        this.registerHandler(MENU_ITEM_CHANNELS.CREATE_CATEGORY, this.createCategory.bind(this));
        this.registerHandler(MENU_ITEM_CHANNELS.UPDATE_CATEGORY, this.updateCategory.bind(this));
        this.registerHandler(MENU_ITEM_CHANNELS.DELETE_CATEGORY, this.deleteCategory.bind(this));
        this.registerHandler(MENU_ITEM_CHANNELS.GET_CATEGORIES, this.getCategories.bind(this));
        this.registerHandler(MENU_ITEM_CHANNELS.GET_CATEGORY_STATS, this.getCategoryStats.bind(this));
    }
    unregisterHandlers() {
     ...
     * Get all menu items
     */
    async getAllMenuItems(_event, params) {
        try {
            console.log('MenuItemController: getAllMenuItems called with params:', params);
     ...
     * Get menu item by ID
     */
    async getMenuItemById(_event, id) {
        try {
     ...
     */
    async getMenuItemsByCategory(_event, category) {
        try {
     ...
     */
    async getAvailableMenuItems(_event, params) {
        try {
            console.log('MenuItemController: getAvailableMenuItems called with params:', params);
            // Set defaults for pagination
            const page = params?.page || 1;
            const pageSize = params?.pageSize || 12;
            const offset = (page - 1) * pageSize;
     ...
     */
    async createMenuItem(_event, itemData) {
        try {
            // Runtime validation with Zod
            const validation = validateWithSchema(CreateMenuItemSchema, itemData, 'CreateMenuItem');
            ...
            const result = await this.menuItemService.create(validatedData);
            if (!result.success || !result.data) {
            ...
            if (this.syncService && result.data.id) {
                this.syncService.syncMenuItem(result.data.id).catch(err => {
                    logInfo(`Background sync failed for item ${result.data?.id}: ${err.message}`);
                });
            }
            // Type assertion to handle the mismatch between main/types and shared/ipc-types
            return this.createSuccessResponse(result.data);
     ...
     */
    async updateMenuItem(_event, updateData) {
        try {
            // Runtime validation with Zod
            const validation = validateWithSchema(UpdateMenuItemSchema, updateData, 'UpdateMenuItem');
            if (!validation.success) {
                logInfo(`UpdateMenuItem: Validation failed - ${validation.error}`);
                return this.createErrorResponse(new Error(validation.error));
            }
            const validatedData = validation.data;
            const result = await this.menuItemService.update(validatedData);
            if (!result.success || !result.data) {
                throw new Error(result.error || 'Failed to update menu item');
            }
            // Trigger real-time sync to Supabase (non-blocking)
            const updateDataWithId = updateData;
            if (this.syncService && updateDataWithId.id) {
                this.syncService.syncMenuItem(updateDataWithId.id).catch((err) => {
                    logInfo(`Background sync failed for item ${updateDataWithId.id}: ${err.message}`);
                });
            }
            // Type assertion to handle the mismatch between main/types and shared/ipc-types
            return this.createSuccessResponse(result.data);
     ...
     * Delete a menu item
     */
    async deleteMenuItem(_event, id) {
        try {
            ...
            if (this.syncService && id) {
                this.syncService.syncMenuItem(id).catch(err => {
                    logInfo(`Background sync failed for deleted item ${id}: ${err.message}`);
                });
            }
            return this.createSuccessResponse(!!result.data);
        }
     ...
     * Search menu items
     */
    async searchMenuItems(_event, query) {
        try {
     ...
     */
    async createCategory(_event, data) {
        try {
            ...
            if (this.syncService && category.id) {
                this.syncService.syncCategory(category.id).catch(err => {
                    logInfo(`Background sync failed for category ${category.id}: ${err.message}`);
                });
            }
     ...
     */
    async updateCategory(_event, data) {
        try {
     ...
     * Delete a category
     */
    async deleteCategory(_event, id) {
        try {
            ...
            if (itemCount > 0) {
                return this.createErrorResponse(new Error('Cannot delete category with menu items'));
            }
            ...
            if (this.syncService && id) {
                this.syncService.syncCategory(id).catch(err => {
                    logInfo(`Background sync failed for deleted category ${id}: ${err.message}`);
                });
            }
    ...
    async getCategoryStats(_event) {
        ...
        try {
            ...
            const categoryStats = await Promise.all(categories.map(async (category, index) => {
