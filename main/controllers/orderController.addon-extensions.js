...
export class OrderControllerAddonExtensions {
    constructor(prisma, addonService, orderModel) {
        this.prisma = prisma;
     ...
     */
    async addOrderItemWithAddons(data) {
        try {
            logInfo(`Adding item with add-ons to order: ${data.orderId}`, 'OrderController');
            ...
            const result = await this.prisma.$transaction(async (tx) => {
                ...
                let addonAssignments = [];
                if (data.item.addonSelections && data.item.addonSelections.length > 0) {
                    logInfo(`Processing ${data.item.addonSelections.length} add-on selections`, 'OrderController');
                    for (const selection of data.item.addonSelections) {
                        ...
                        if (!addon || !addon.isActive) {
                            throw new Error(`Add-on ${selection.addonId} not found or inactive`);
                        }
                        ...
                        if (addon.inventoryItems && addon.inventoryItems.length > 0) {
                            logInfo(`Processing ${addon.inventoryItems.length} inventory items for addon: ${addon.name}`, 'OrderController');
                            for (const addonInvItem of addon.inventoryItems) {
                                ...
                                if (newStock < 0) {
                                    throw new Error(`Insufficient stock for addon ${addon.name} ingredient ${inventory.itemName}. Available: ${currentStock}, Required: ${totalQuantityNeeded}`);
                                }
                                ...
                                logInfo(`Deducted addon inventory: ${addon.name} -> ${inventory.itemName} - ${totalQuantityNeeded} ${inventory.unit}`, 'OrderController');
                            }
                        }
                    }
                }
                ...
                const orderSubtotal = allItems.reduce((sum, item) => sum + Number(item.totalPrice), 0);
                ...
                logInfo(`Order item with add-ons added successfully`, 'OrderController');
     ...
     */
    async addAddonsToExistingOrderItem(data) {
        try {
            logInfo(`Adding add-ons to existing order item: ${data.orderItemId}`, 'OrderController');
            const result = await this.addonService.addAddonsToOrderItem(data.orderItemId, data.addonSelections);
            if (!result.success) {
                return {
                    success: false,
            ...
            logInfo(`Order totals recalculated after adding add-ons to order: ${orderId}`, 'OrderController');
            return {
                success: true,
     ...
     */
    async removeAddonFromOrderItem(data) {
        try {
            logInfo(`Removing add-on from order item: ${data.orderItemId}`, 'OrderController');
            const result = await this.addonService.removeAddonFromOrderItem(data.orderItemId, data.addonId);
            if (!result.success) {
                return {
                    success: false,
                    error: result.error.message,
            ...
            const orderId = result.data.orderId;
            await this.orderModel.recalculateOrderTotals(orderId);
            logInfo(`Order totals recalculated after removing add-on from order: ${orderId}`, 'OrderController');
            return {
                success: true,
     ...
     */
    async deductMenuItemInventory(tx, menuItemId, quantity) {
     ...
     */
    async removeOrderItemWithAddons(orderId, itemId, userId) {
        try {
            ...
            const result = await this.prisma.$transaction(async (tx) => {
                ...
                for (const addonAssignment of orderItem.orderItemAddons) {
                    if (addonAssignment.addon.inventoryItems && addonAssignment.addon.inventoryItems.length > 0) {
                        logInfo(`Restoring ${addonAssignment.addon.inventoryItems.length} inventory items for addon: ${addonAssignment.addon.name}`, 'OrderController');
                        for (const addonInvItem of addonAssignment.addon.inventoryItems) {
                            ...
                            logInfo(`Restored addon inventory: ${addonAssignment.addon.name} -> ${addonInvItem.inventory.itemName} + ${totalQuantityToRestore} ${addonInvItem.inventory.unit}`, 'OrderController');
                        }
                    }
                }
                ...
                const orderSubtotal = remainingItems.reduce((sum, item) => sum + Number(item.totalPrice), 0);
                await tx.order.update({
                    where: { id: orderId },
                    data: {
                        subtotal: orderSubtotal,
