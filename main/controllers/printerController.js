...
import { PrinterType, ConnectionType, } from './types/printer-types';
import { PrinterDetectionError, PrinterValidationError, PrinterConnectionError, PrinterTimeoutError, PrinterConfigurationError, } from './errors/printer-errors';
...
export class PrinterController extends BaseController {
    constructor() {
        ...
        this.CACHE_TTL = 600000; // 10 minutes - optimized for old hardware performance
    ...
    registerHandlers() {
        ...
        this.registerHandler(PRINTER_CHANNELS.GET_ALL, this.getPrinters.bind(this));
        // Get default printer
        this.registerHandler(PRINTER_CHANNELS.GET_DEFAULT, this.getDefaultPrinter.bind(this));
        // Test print functionality
        this.registerHandler(PRINTER_CHANNELS.PRINT_TEST_PAGE, this.printTestPage.bind(this));
        // Diagnostic test print (NEW)
        this.registerHandler(PRINTER_CHANNELS.TEST_PRINT, this.testPrintDiagnostic.bind(this));
        // Print receipt
        this.registerHandler(PRINTER_CHANNELS.PRINT_RECEIPT, this.printReceipt.bind(this));
        // Check printer status
        this.registerHandler(PRINTER_CHANNELS.CHECK_STATUS, this.checkPrinterStatus.bind(this));
        // Validate printer capabilities
        this.registerHandler(PRINTER_CHANNELS.VALIDATE_PRINTER, this.validatePrinter.bind(this));
        // Detect RONGTA devices
        this.registerHandler(PRINTER_CHANNELS.DETECT_RONGTA, this.detectRONGTADevices.bind(this));
        // Test RONGTA connection
        this.registerHandler(PRINTER_CHANNELS.TEST_RONGTA_CONNECTION, this.testRONGTAConnection.bind(this));
    ...
    async getPrinters(_event) {
        try {
            ...
            this.logger.info(`Printer detection completed using ${detectionResult.method} method in ${detectionResult.duration}ms (cached: ${detectionResult.cached})`);
            // Transform PrinterInfo[] to Printer[] with enriched metadata
            const printers = detectionResult.printers.map(printer => {
                const connectionType = this.detectConnectionType(printer.portName);
                const printerType = this.detectPrinterType(printer.name, printer.driverName);
                return {
                    name: printer.name,
            ...
            if (printers.length > 0) {
                try {
                    ...
                    const { stdout } = await execAsync('powershell -command "Get-WmiObject -Query \\"SELECT * FROM Win32_Printer WHERE Default=TRUE\\" | Select-Object -ExpandProperty Name"', { windowsHide: true, timeout: 8000 });
                    const defaultPrinterName = stdout.trim();
                    const defaultPrinter = printers.find(p => p.name === defaultPrinterName);
                    if (defaultPrinter) {
                        defaultPrinter.isDefault = true;
                    }
                }
                catch (error) {
                    this.logger.warn(`Could not determine default printer: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
     ...
     * Get default printer
     */
    async getDefaultPrinter(_event) {
        try {
            ...
            if (!defaultPrinter) {
                if (printers.length > 0) {
                    this.logger.warn('No default printer found, using first available printer');
                    if (printers[0]) {
     ...
     */
    async printTestPage(_event, request) {
        try {
            const { printerName, testType = 'standard' } = request;
            this.logger.info(`Enhanced test print for ${printerName}, type: ${testType}`);
            if (!printerName) {
            ...
            if (!diagnostics.printerExists) {
                ...
                return this.createErrorResponse(new Error(`Printer '${printerName}' not found. Available printers: ${diagnostics.availablePrinters.join(', ')}`));
            }
            ...
            if (!diagnostics.isAccessible) {
                if (diagnostics.status === 'Pending Deletion') {
                    this.logger.warn(`⚠️ Printer '${printerName}' is in "Pending Deletion" status. Attempting to print anyway...`);
                    ...
                    return this.createErrorResponse(new Error(`Printer '${printerName}' exists but is not accessible. Status: ${diagnostics.status}`));
                }
            }
            let success = false;
            let methodUsed = 'Unknown';
            let details = '';
            let dataBuffer = Buffer.alloc(0); // Initialize with empty buffer
            // ENHANCED: Handle "direct" test type that bypasses all status checks
            if (testType === 'direct') {
                this.logger.info(`🚀 Direct test print requested for ${printerName} - bypassing all checks`);
                const directTestSuccess = await this.directTestPrint(printerName);
                if (directTestSuccess) {
                    return this.createSuccessResponse({
                        success: true,
                        method_used: 'Direct Bypass Test',
                        details: 'Direct test successful - printer works despite any status issues',
                        timestamp: getCurrentLocalDateTime(),
                        bypassed_all_checks: true,
                    });
                }
                else {
                    return this.createErrorResponse(new Error('Direct test failed - indicates serious hardware or driver issue'));
                }
            }
            ...
            if (testType === 'diagnostic') {
                ...
                try {
                    this.logger.info(`Testing with hybrid thermal printing for ${printerName}`);
                ...
                try {
                    this.logger.info(`Testing ultimate thermal printing solution for ${printerName}`);
                    // Get detailed result with actual method used
                    const ultimateResult = await this.ultimateThermalTestPrintWithDetails(printerName);
...
--------------------------------`;
                    ...
                    success = await this.printWithPowerShell(printerName, testContent, true);
                    ...
                    if (success) {
                        this.logger.info(`✅ RONGTA plain text test successful for ${printerName}`);
                    }
                    else {
                        this.logger.warn(`❌ RONGTA plain text test failed, will try ESC/POS fallback`);
                        // If plain text fails, try the complex ESC/POS method as fallback
                        this.logger.info(`Generating RONGTA/thermal test buffer for ${printerName}`);
...
--------------------------------`;
            ...
            this.logger.info(`Test print result for ${printerName}: ${success ? 'SUCCESS' : 'FAILED'} (${methodUsed})`);
            ...
            if (!success &&
                ...
                this.logger.warn(`⚠️ Detected "Pending Deletion" status for ${printerName}. Trying bypass methods first...`);
                // Try bypass methods first since they often work even with status issues
                const bypassResult = await this.performRawPrintBypass(printerName, dataBuffer);
                ...
                this.logger.warn(`🔄 Bypass failed. Attempting automatic print spooler reset...`);
                const spoolerResetSuccess = await this.resetPrintSpooler();
                if (spoolerResetSuccess) {
                    this.logger.info(`✅ Print spooler reset successful. Retrying print operation...`);
                    // Wait a moment for system to stabilize
                    await new Promise(resolve => setTimeout(resolve, 2000));
                    // Retry the print operation
                    const retryResult = await this.performRawPrint(printerName, dataBuffer);
     ...
     * Print receipt
     */
    async printReceipt(_event, request) {
        try {
            ...
            let prisma = await this.getPrismaInstance(getPrismaClient, initializePrisma);
            ...
            const targetPrinter = printerInfo.data?.find(p => p.name === request.printerName);
            const isThermalPrinter = targetPrinter?.printerType === PrinterType.THERMAL ||
            ...
            const isKitchenOrder = request.isKitchenOrder === true;
            const isInvoice = request.isInvoice === true;
            const useUltimateSolution = request.useUltimateThermalSolution === true;
            if (useUltimateSolution && isThermalPrinter) {
                this.logger.info(isKitchenOrder
                ...
                try {
                    ...
                    const { generateEnhancedKitchenTicket, } = require('../utils/enhancedKitchenTicket');
                    ...
                    if (isKitchenOrder) {
                        ...
                        const addonTicketGenerator = new AddonKitchenTicketGenerator(addonService);
                        receiptContent =
                            await addonTicketGenerator.generateKitchenTicketWithAddons(order, request.onlyUnprinted, request.cancelledItems || [], request.updatedItemIds || [], request.itemChanges || [] // FIXED: Pass the net change data!
                            );
                    }
                    else if (isInvoice) {
                        ...
                        receiptContent =
                            await addonInvoiceGenerator.generateEnhancedThermalInvoiceWithAddons(order, businessInfo, getResourcesPath);
                    }
                    else {
                        receiptContent = await this.generateEnhancedThermalReceipt(order, businessInfo);
                    }
                    // Get detailed result with actual method used
                    const ultimateResult = await this.directTextPrintWithFormatting(request.printerName, receiptContent);
                    if (ultimateResult.success) {
                        this.logger.info(`✅ Ultimate thermal ${isKitchenOrder ? 'kitchen' : isInvoice ? 'invoice' : 'receipt'} printing successful using ${ultimateResult.method}`);
                        // For kitchen orders, mark the printed items
                        if (isKitchenOrder) {
                            try {
                                // Determine which items to mark as printed based on what was actually printed
                                const itemsToMark = (() => {
                                    if (request.updatedItemIds &&
                                        request.updatedItemIds.length > 0) {
                                        // Only mark items that were specifically printed (selective printing)
                                        return order.items.filter(item => request.updatedItemIds.includes(item.id));
                                    }
                                ...
                                if (itemsToMark.length > 0) {
                                    ...
                                    await Promise.all(itemsToMark.map(async (item) => {
                                        await prisma.orderItem.update({
                                            where: { id: item.id },
                                            data: { printed: true },
                                        });
                                    }));
                                    const markingMethod = request.updatedItemIds && request.updatedItemIds.length > 0
                                        ? 'selective'
                                        : request.onlyUnprinted
                                            ? 'unprinted-only'
                                            : 'all-items';
                                    this.logger.info(`✅ Marked ${itemsToMark.length} items as printed for order ${request.orderId} (method: ${markingMethod})`);
                                }
                            }
                            catch (markError) {
                                this.logger.error(`Failed to mark items as printed: ${markError}`);
                        ...
                        throw new Error(`Ultimate thermal printing failed: ${ultimateResult.details}`);
                    }
                }
                catch (error) {
                    this.logger.error(`❌ Ultimate thermal ${isKitchenOrder ? 'kitchen' : isInvoice ? 'invoice' : 'receipt'} printing failed: ${error}`);
            ...
            if (isThermalPrinter) {
                ...
                if (isKitchenOrder) {
                    ...
                    const addonTicketGenerator = new AddonKitchenTicketGenerator(addonService);
                    receiptText =
                        await addonTicketGenerator.generateKitchenTicketWithAddons(order, request.onlyUnprinted || false, request.cancelledItems || [], request.updatedItemIds || [], request.itemChanges || []);
                }
                else if (isInvoice) {
                    ...
                    const addonInvoiceGenerator = new AddonInvoiceGenerator(addonService);
                    receiptText =
                        await addonInvoiceGenerator.generateThermalInvoiceWithAddons(order, businessInfo);
                }
                else {
                    receiptText = LebanesReceiptGenerator.generateThermalReceipt({
                        order: order, // Type casting for compatibility
                        businessInfo,
                    });
                }
                ...
                if (isInvoice) {
                    ...
                    const addonInvoiceGenerator = new AddonInvoiceGenerator(addonService);
                    receiptBuffer =
                        await addonInvoiceGenerator.generatePDFInvoiceWithAddons(order, businessInfo);
                }
                else {
            ...
            const success = await this.printWithPowerShell(request.printerName, receiptBuffer, isThermalPrinter // Pass as plain text for thermal printers
            );
            if (!success) {
     ...
     */
    async generateEnhancedThermalReceipt(order, businessInfo) {
     ...
     */
    async generateEnhancedThermalInvoice(order, businessInfo) {
            ...
            if (Math.abs(calculatedTotal - total) > 0.01) {
                ...
                try {
                    if (this.logger && typeof this.logger.warn === 'function') {
                        this.logger.warn(`Invoice total calculation mismatch: stored=${total}, calculated=${calculatedTotal}, using calculated value`);
                    }
                    else {
                        console.warn(`[PRINTER] Invoice total calculation mismatch: stored=${total}, calculated=${calculatedTotal}, using calculated value`);
                    }
                }
                catch (e) {
        ...
        if (order.type?.toUpperCase() === 'DELIVERY') {
            ...
            const effectiveDeliveryFee = parseFloat((order.deliveryFee || 0).toString()) || deliveryFee;
            printData.push({
                type: 'text',
     ...
     */
    async generateEnhancedThermalKitchenTicket(order, onlyUnprinted = false, cancelledItems = [], // Parameter for cancelled items
    updatedItemIds = [], // Parameter for updated items
    itemChanges = [] // Enhanced change tracking information
        ...
        if (updatedItemIds && updatedItemIds.length > 0) {
            ...
            console.log('🔍 ENHANCED FILTERING DEBUG:', {
                updatedItemIds,
                allItemIds: itemsToProcess.map((item) => item.id),
                availableItems: itemsToProcess.map((item) => ({
                    id: item.id,
                    name: item.name || item.menuItemName,
                })),
                matchingItems: itemsToProcess.filter((item) => updatedItemIds.includes(item.id)).length,
            });
            itemsToProcess = itemsToProcess.filter((item) => updatedItemIds.includes(item.id));
            ...
            if (itemsToProcess.length === 0) {
                console.log('⚠️ ENHANCED: NO MATCHING ITEMS FOUND after filtering - returning empty ticket');
                console.log('🔍 ENHANCED DEBUG INFO:', {
                    requestedIds: updatedItemIds,
                    allOrderItemIds: order.items?.map((item) => item.id) || [],
        ...
        if (hasEnhancedChanges) {
            ...
            itemChanges.forEach((change) => {
                ...
                const item = itemsToProcess.find((i) => i.id === change.id);
                if (!item)
     ...
     * Check printer status
     */
    async checkPrinterStatus(_event, printerName) {
        try {
            ...
            try {
                const { stdout } = await execAsync(`powershell -command "Get-WmiObject -Query \\"SELECT * FROM Win32_Printer WHERE Name='${printerName.replace(/'/g, "''")}'\\""`, { timeout: 5000, windowsHide: true });
                ...
                this.logger.warn(`Failed to check printer status: ${error instanceof Error ? error.message : String(error)}`);
                return this.createSuccessResponse({
                    isConnected: false,
                    status: 'Error',
     ...
     */
    async validatePrinter(_event, printerName) {
        try {
            this.logger.info(`Validating printer capabilities for: ${printerName}`);
            const validationResult = await this.validatePrinterCapabilities(printerName);
            return this.createSuccessResponse(validationResult, `Printer validation completed for ${printerName}`);
        }
        catch (error) {
            this.logger.error(`Printer validation failed for ${printerName}:`, error);
            // Use enhanced error handling
            const categorizedError = this.handlePrinterError(error, 'Printer validation', printerName, { validationStep: 'complete-validation' });
     ...
     */
    async detectRONGTADevices(_event) {
        try {
            ...
            try {
                ...
                this.logger.info(`Found ${registryInfo.length} registry RONGTA entries`);
            }
            catch (error) {
                const errorMessage = error instanceof Error ? error.message : String(error);
            ...
            this.logger.info(`RONGTA detection completed: ${result.devices.length} unique devices found in ${result.detectionTime}ms`);
            return this.createSuccessResponse(result);
        }
        catch (error) {
            this.logger.error('RONGTA device detection failed:', error);
            const categorizedError = this.handlePrinterError(error, 'Enhanced RONGTA device detection', undefined, { detectionMethod: 'comprehensive' });
            return this.createErrorResponse(categorizedError);
        }
    }
    /**
     ...
     */
    async testRONGTAConnection(_event, deviceId, testType = 'COMPREHENSIVE') {
        try {
            this.logger.info(`Starting RONGTA connection test for device: ${deviceId}, type: ${testType}`);
            const startTime = Date.now();
            ...
            if (!detectionResult.success || !detectionResult.data) {
                throw new Error('Failed to detect RONGTA devices for connection testing');
            }
            const device = detectionResult.data.devices.find(d => d.deviceId === deviceId);
            if (!device) {
            ...
            if (testType === 'USB' || testType === 'COMPREHENSIVE') {
                testMethods.push(() => RONGTAConnectionUtility.testUSBConnection(device));
            }
            if (testType === 'SPOOLER' || testType === 'COMPREHENSIVE') {
                testMethods.push(() => RONGTAConnectionUtility.testSpoolerConnection(device));
            }
            if (testType === 'ESCPOS' || testType === 'COMPREHENSIVE') {
                testMethods.push(() => RONGTAConnectionUtility.testESCPOSCommands(device));
            }
            ...
            for (const testMethod of testMethods) {
                try {
                    const retryResult = await RetryUtility.executeWithRetry(testMethod, retryConfig, true, `connection-test-${device.name}`);
                    if (retryResult.success && retryResult.data) {
                        testResults.push(retryResult.data);
                        this.logger.info(`Connection test succeeded: ${retryResult.data.method} for ${device.name}`);
                    }
                    else {
            ...
            const partialTests = testResults.filter(result => result.status === 'PARTIAL');
            ...
            this.logger.info(`RONGTA connection test completed for ${device.name}: ${connectionTest.overallStatus} (${connectionTest.testDuration}ms)`);
            return this.createSuccessResponse(connectionTest, `Connection test completed for RONGTA device ${device.name}`);
        }
        catch (error) {
            this.logger.error('RONGTA connection test failed:', error);
            const categorizedError = this.handlePrinterError(error, 'RONGTA connection testing', undefined, { deviceId, testType });
            return this.createErrorResponse(categorizedError);
        }
    }
    /**
    ...
    async getPowerShellPrinters() {
      ...
      `;
            const tempScriptPath = path.join(os.tmpdir(), `printer-detect-${Date.now()}.ps1`);
            ...
            try {
                ...
                try {
                    ...
                    this.logger.error(`Failed to parse PowerShell JSON output: ${parseError instanceof Error ? parseError.message : String(parseError)}`);
                    this.logger.error(`Raw output was: ${stdout.substring(0, 500)}...`);
                    throw new PrinterDetectionError('Failed to parse printer data from PowerShell', parseError instanceof Error
                        ? parseError
                        : new Error(String(parseError)), { rawOutput: stdout.substring(0, 200) });
                }
                ...
                this.logger.info(`Successfully detected ${validPrinters.length} valid printers via PowerShell`);
                ...
                try {
                    ...
                    this.logger.warn(`Failed to clean up temporary script: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`);
                }
            }
        }
        catch (error) {
            ...
            throw new PrinterDetectionError(`Failed to enumerate printers via PowerShell: ${errorMessage}`, error instanceof Error ? error : new Error(String(error)), {
    ...
    isThermalPrinter(name, description) {
        ...
        const nameLC = name.toLowerCase();
        const descLC = (description || '').toLowerCase();
        return thermalPatterns.some(pattern => nameLC.includes(pattern) || descLC.includes(pattern));
    }
    /**
     * Check if printer is a document printer
     */
    isDocumentPrinter(name, description) {
        ...
        const nameLC = name.toLowerCase();
        const descLC = (description || '').toLowerCase();
        return documentPatterns.some(pattern => nameLC.includes(pattern) || descLC.includes(pattern));
    }
    getFallbackPrinters() {
        return [
            {
     ...
     */
    handlePrinterError(error, operation, printerName, context) {
        const baseMessage = error instanceof Error ? error.message : String(error);
        // Check for timeout errors
        if (baseMessage.includes('timeout') ||
            baseMessage.includes('ETIMEDOUT') ||
            baseMessage.includes('timed out')) {
            return new PrinterTimeoutError(`${operation} timed out`, printerName, undefined, operation, error instanceof Error ? error : undefined, context);
        }
        // Check for connection errors
        if (baseMessage.includes('ECONNREFUSED') ||
            baseMessage.includes('network') ||
            baseMessage.includes('connection')) {
            return new PrinterConnectionError(`${operation} failed due to connection issues`, printerName, undefined, undefined, error instanceof Error ? error : undefined, context);
        }
        // Check for configuration errors
        if (baseMessage.includes('driver') ||
            baseMessage.includes('not installed') ||
            baseMessage.includes('configuration')) {
            return new PrinterConfigurationError(`${operation} failed due to configuration issues`, printerName, 'Configuration problem', 'Check printer drivers and system configuration', error instanceof Error ? error : undefined, context);
        }
        // Return a generic validation error
        return new PrinterValidationError(`${operation} failed`, printerName, error instanceof Error ? error : undefined, context);
    }
    /**
     * Validate printer capabilities and test actual functionality
     */
    async validatePrinterCapabilities(printerName) {
        const startTime = Date.now();
        this.logger.info(`Starting capability validation for printer: ${printerName}`);
        const result = {
            printerName,
            isOnline: false,
            isReady: false,
        ...
        try {
            ...
            const statusRetryResult = await RetryUtility.executeWithRetry(() => this.testPrinterStatus(printerName), retryConfig, true, `printer-status-${printerName}`);
            if (statusRetryResult.success) {
                ...
                result.errors.push(`Printer status check failed after ${statusRetryResult.totalAttempts} attempts: ${statusRetryResult.finalError?.message}`);
                return result;
            }
            if (!result.isOnline) {
                result.errors.push(`Printer status check failed: ${statusRetryResult.data?.error}`);
                return result;
            }
            ...
            this.logger.info(`Printer validation completed for ${printerName} - Online: ${result.isOnline}, Ready: ${result.isReady}`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            result.errors.push(`Validation failed with error: ${errorMessage}`);
            this.logger.error(`Printer validation failed for ${printerName}: ${errorMessage}`);
        }
     ...
     */
    async testPrinterStatus(printerName) {
        const startTime = Date.now();
        try {
            const scriptContent = `
        try {
      ...
      `;
            const tempScriptPath = path.join(os.tmpdir(), `printer-status-${Date.now()}.ps1`);
            try {
                fs.writeFileSync(tempScriptPath, scriptContent, 'utf8');
                const { stdout, stderr } = await execAsync(`powershell -NoProfile -ExecutionPolicy Bypass -File "${tempScriptPath}"`, {
                    timeout: 10000,
                ...
                try {
                    ...
                    this.logger.warn(`Failed to clean up status test script: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`);
                }
            }
        }
        catch (error) {
     ...
     */
    async getPrismaInstance(getPrismaClient, initializePrisma) {
        try {
    ...
    async generateRONGTATestBuffer(request) {
...
`;
    ...
    async printWithPowerShell(printerName, data, _isPlainText = false) {
        try {
            ...
            for (const { name: methodName, method } of printingMethods) {
                try {
                    ...
                    if (success) {
                        this.logger.info(`✅ ${methodName} method succeeded for ${printerName}`);
                        return true;
                    }
                    else {
                        this.logger.warn(`❌ ${methodName} method failed for ${printerName}`);
                    }
                }
                catch (error) {
     ...
     */
    async printWithWMI(printerName, data) {
      ...
      `;
            const tempScriptPath = path.join(os.tmpdir(), `wmi-script-${Date.now()}.ps1`);
            try {
                fs.writeFileSync(tempScriptPath, printScript, 'utf8');
                const { stdout } = await execAsync(`powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -WindowStyle Hidden -File "${tempScriptPath}"`, {
                    timeout: 15000,
                    windowsHide: true,
                });
            ...
            if (error instanceof Error &&
                ...
                this.logger.warn(`Printer operation handled gracefully: ${error.message}`);
                return false;
            }
            throw error;
        }
     ...
     */
    async printWithNet(printerName, data) {
      ...
      `;
            const tempScriptPath = path.join(os.tmpdir(), `net-script-${Date.now()}.ps1`);
            try {
                fs.writeFileSync(tempScriptPath, printScript, 'utf8');
                const { stdout } = await execAsync(`powershell -NoProfile -ExecutionPolicy Bypass -File "${tempScriptPath}"`, { timeout: 10000, windowsHide: true });
     ...
     */
    async printWithCopyCommand(printerName, data) {
      ...
      `;
            const tempScriptPath = path.join(os.tmpdir(), `copy-script-${Date.now()}.ps1`);
            try {
                fs.writeFileSync(tempScriptPath, printScript, 'utf8');
                const { stdout } = await execAsync(`powershell -NoProfile -ExecutionPolicy Bypass -File "${tempScriptPath}"`, { timeout: 10000 });
     ...
     */
    async printWithPrintCommand(printerName, data) {
      ...
      `;
            const tempScriptPath = path.join(os.tmpdir(), `print-script-${Date.now()}.ps1`);
            try {
                fs.writeFileSync(tempScriptPath, printScript, 'utf8');
                const { stdout } = await execAsync(`powershell -NoProfile -ExecutionPolicy Bypass -File "${tempScriptPath}"`, { timeout: 10000 });
     ...
     */
    async runPrinterDiagnostics(printerName) {
        try {
            // Get all available printers
            const printersResponse = await this.getPrinters({});
            const availablePrinters = printersResponse.success && printersResponse.data
            ...
            try {
                const { stdout } = await execAsync(`powershell -command "Get-WmiObject -Query \\"SELECT * FROM Win32_Printer WHERE Name='${printerName.replace(/'/g, "''")}'\\""`, { timeout: 5000 });
                if (stdout.includes('PrinterStatus')) {
                    const statusMatch = stdout.match(/PrinterStatus\s+:\s+(\d+)/);
                    const workOfflineMatch = stdout.match(/WorkOffline\s+:\s+(\w+)/);
                    const status = statusMatch && statusMatch[1] ? parseInt(statusMatch[1], 10) : 0;
                    const workOffline = workOfflineMatch && workOfflineMatch[1] === 'True';
                    let statusMessage = 'Unknown';
                    ...
                    if (!isAccessible) {
                        result.errorDetails = statusMessage;
                    }
                    return result;
                }
            }
    ...
    async resetPrintSpooler() {
        ...
        `;
                const { stdout: clearOutput } = await execAsync(`powershell -NoProfile -ExecutionPolicy Bypass -Command "${clearQueueScript.replace(/"/g, '\\"').replace(/\n/g, ' ')}"`, { timeout: 10000 });
                ...
                this.logger.warn('Non-admin queue clear failed, trying admin method...');
            }
      ...
      `;
            const tempScriptPath = path.join(os.tmpdir(), `spooler-reset-${Date.now()}.ps1`);
            try {
                fs.writeFileSync(tempScriptPath, resetScript, 'utf8');
                const { stdout } = await execAsync(`powershell -NoProfile -ExecutionPolicy Bypass -File "${tempScriptPath}"`, { timeout: 30000 } // 30 second timeout for service operations
                );
            ...
            this.logger.error(`❌ Print spooler reset failed: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    /**
     ...
     */
    async performRawPrint(printerName, data) {
        const methods = [
        ...
        for (const method of methods) {
            try {
                ...
                this.logger.warn(`❌ ${method.name} failed: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        return {
            success: false,
     ...
     */
    async performRawPrintBypass(printerName, data) {
        try {
            ...
            for (const method of methods) {
                try {
                    this.logger.info(`🔧 Trying ${method.name} bypass for ${printerName}...`);
                    const success = await method.fn();
                    if (success) {
                        this.logger.info(`✅ ${method.name} bypass successful! FORCING CUTTING NOW...`);
                        // DISABLED: Aggressive cutting sequence that was causing early cutting
                        this.logger.info(`🚫 NUCLEAR CUTTING SEQUENCE DISABLED TO PREVENT EARLY CUTTING`);
                        this.logger.info(`⏳ Allowing sufficient time for complete printing before any cutting attempts...`);
                    ...
                    this.logger.warn(`❌ ${method.name} bypass failed: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            return {
                success: false,
     ...
     */
    async directTestPrint(printerName) {
        try {
            this.logger.info(`🚀 Direct test print to ${printerName} (bypassing all checks)...`);
            // Left-aligned text - no manual centering to avoid DIP switch width conflicts
...
--------------------------------`;
            ...
            if (result.success) {
                this.logger.info(`✅ Direct test print successful using ${result.method_used}`);
                return true;
            }
            else {
            ...
            this.logger.error(`❌ Direct test print error: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    /**
     ...
     */
    async diagnosticTestPrint(printerName) {
        try {
            this.logger.info(`🔍 Running diagnostic test for ${printerName}...`);
...
END OF TEST`;
            ...
            const success = await this.printWithPowerShell(printerName, testContent, true);
            if (success) {
                this.logger.info(`✅ Diagnostic test successful using PowerShell RAW Printing`);
                return true;
            }
            else {
                this.logger.warn(`❌ Diagnostic test failed with PowerShell RAW method`);
                return false;
            }
        }
        catch (error) {
            this.logger.error(`❌ Diagnostic test error: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    /**
     ...
     */
    async thermalLibraryTestPrint(printerName) {
        try {
            this.logger.info(`🔍 Testing with thermal library for ${printerName}...`);
            const success = await this.printWithPowerShell(printerName, `THERMAL LIBRARY TEST PRINT\n\nDate: ${new Date().toLocaleString()}\nPrinter: ${printerName}\n--------------------------------\nPRINTING SUCCESSFUL!\n\nUsing electron-pos-printer library\nFormatting and layout test\n--------------------------------`, true);
            if (success) {
                this.logger.info(`✅ Thermal library test successful for ${printerName}`);
                return true;
            }
            else {
            ...
            this.logger.error(`❌ Thermal library test error: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    /**
     * Test using hybrid approach: electron-pos-printer + ESC/POS cutting
     */
    async hybridThermalTestPrint(printerName) {
        try {
            this.logger.info(`🔍 Testing hybrid thermal printing for ${printerName}...`);
            // Basic test content
...
--------------------------------`;
            ...
            const success = await this.printWithPowerShell(printerName, testContent, // NO EXTRA LINES = NO WASTED PAPER!
            true);
            if (success) {
                this.logger.info(`✅ Hybrid thermal test successful for ${printerName}`);
                return true;
            }
            else {
            ...
            this.logger.error(`❌ Hybrid thermal test error: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    /**
     ...
     */
    async testThermalPrinting(printerName) {
        try {
            ...
            this.logger.error(`Thermal test error: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    /**
     ...
     */
    async sendEscPosDirectly(printerName, commands) {
        try {
            ...
            return new Promise(resolve => {
                ...
                process.on('exit', (code) => {
                    ...
                    this.logger.info(`Direct ESC/POS command completed with code: ${code}`);
                    resolve(success);
                });
            ...
            this.logger.error(`Direct ESC/POS method failed: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    /**
     ...
     */
    async analyzePrinterDriverForSilentPrinting(printerName) {
        try {
            this.logger.info(`🔍 Analyzing driver for silent printing: ${printerName}`);
            ...
            else if (driverName.includes('thermal') ||
            ...
            else if (driverName.includes('text') ||
            ...
            this.logger.info(`📊 Driver analysis: ${driverType}, silent: ${silentCapable}, method: ${recommendedMethod}`);
            return {
                silentCapable,
                driverType,
                recommendedMethod,
            ...
            this.logger.error(`❌ Driver analysis failed: ${error instanceof Error ? error.message : String(error)}`);
            return {
                silentCapable: false,
                driverType: 'generic',
                recommendedMethod: 'raw-printing',
     ...
     */
    async ultimateThermalTestPrintWithDetails(printerName) {
        try {
            ...
            const directPrintResult = await this.directTextPrintWithFormatting(printerName);
            ...
            this.logger.error(`Printing error: ${error instanceof Error ? error.message : String(error)}`);
            return {
                success: false,
     ...
     */
    async fixPrinterStatus(printerName) {
        try {
            ...
            this.logger.error(`❌ Printer status fix error: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    /**
     ...
     */
    async directTextPrintWithFormatting(printerName, customContent) {
        try {
            ...
            return new Promise(resolve => {
                PosPrinter.print(printData, printOptions)
                    ...
                    await new Promise(delayResolve => setTimeout(delayResolve, printFinalizationDelay));
            ...
            this.logger.error(`Direct text printing error: ${error instanceof Error ? error.message : String(error)}`);
            return {
                success: false,
     ...
     */
    async sendRawCommand(printerName, command) {
        try {
     ...
     */
    async analyzeDriverCompatibility(printerName) {
        try {
            this.logger.info(`🔍 Analyzing driver compatibility for ${printerName}...`);
            ...
            this.logger.info(`📊 Analysis Result: Direct USB: ${analysis.supportsDirectUSB ? 'YES' : 'NO'}, ` +
            ...
            this.logger.warn(`⚠️ Driver analysis failed: ${error instanceof Error ? error.message : String(error)}`);
     ...
     */
    async testPrintDiagnostic(_event, params) {
        try {
     ...
     */
    categorizeDriver(driverName, portName) {
