...
export class RetryUtility {
    ...
    static async executeWithRetry(operation, config = {}, useLogging = true, operationName) {
        ...
        for (let attempt = 1; attempt <= finalConfig.maxAttempts; attempt++) {
            ...
            try {
                ...
                if (useLogging && operationName) {
                    AdvancedLogger.info(`Retry operation succeeded on attempt ${attempt}: ${operationName}`);
                }
                return {
                    success: true,
                    data,
                ...
                if (attempt === finalConfig.maxAttempts) {
                    if (useLogging && operationName) {
                        AdvancedLogger.error(`Retry operation failed after ${attempt} attempts: ${operationName}`, error);
                    }
                    return {
                        success: false,
                ...
                if (!this.isRetryableError(error, finalConfig.retryableErrors)) {
                    if (useLogging && operationName) {
                        AdvancedLogger.warn(`Non-retryable error encountered, stopping retry: ${operationName}`, error);
                    }
                    return {
                        success: false,
                        finalError: error instanceof Error ? error : new Error(String(error)),
                ...
                if (useLogging && operationName) {
                    AdvancedLogger.warn(`Retry operation failed on attempt ${attempt}, retrying in ${delay}ms: ${operationName}`, error);
                }
     ...
     */
    static calculateDelay(attemptNumber, config) {
     ...
     */
    static createPrinterConfig(operation) {
