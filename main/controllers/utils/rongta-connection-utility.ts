...
export class RONGTAConnectionUtility {
  ...
  private static readonly ESCPOS_COMMANDS = {
    // Basic commands
    INITIALIZE: {
      name: 'Initialize',
      command: [0x1b, 0x40],
  ...
  private static readonly MODEL_TEST_SUITES: Record<string, ESCPOSTestSuite> = {
    RP58: {
      deviceModel: 'RP58',
      commands: [
        'INITIALIZE' as ESCPOSCommandName,
        'STATUS_CHECK' as ESCPOSCommandName,
        'PRINT_TEST' as ESCPOSCommandName,
        'LINE_FEED' as ESCPOSCommandName,
        'DRAWER_OPEN' as ESCPOSCommandName,
        'PAPER_CUT' as ESCPOSCommandName,
      ],
      connectionTest: true,
      printTest: true,
      drawerTest: true,
    ...
    RP80: {
      ...
      commands: [
        ...
        'DRAWER_OPEN' as ESCPOSCommandName,
        'PAPER_CUT' as ESCPOSCommandName,
        'BARCODE_TEST' as ESCPOSCommandName,
      ],
      connectionTest: true,
      printTest: true,
      drawerTest: true,
    ...
    RP326: {
      ...
      commands: [
        ...
        'PAPER_CUT' as ESCPOSCommandName,
        'BARCODE_TEST' as ESCPOSCommandName,
        'QR_CODE_TEST' as ESCPOSCommandName,
      ],
      connectionTest: true,
      printTest: true,
      drawerTest: true,
   ...
   */
  static async testUSBConnection(
    device: RONGTADevice
    ...
    try {
      ...
      if (testResult.accessible) {
        ...
        result.errors.push(
          'USB device not accessible for direct communication'
        );
      }
    } catch (error) {
      result.status = 'FAILED';
      result.errors.push(
        `USB test failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
   ...
   */
  static async testSpoolerConnection(
    device: RONGTADevice
  ): Promise<ConnectionTestResult> {
    const startTime = Date.now();
    const result: ConnectionTestResult = {
    ...
    try {
      ...
      result.status = 'FAILED';
      result.errors.push(
        `Spooler test failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
   ...
   */
  static async testESCPOSCommands(
    device: RONGTADevice
  ): Promise<ConnectionTestResult> {
    const startTime = Date.now();
    const result: ConnectionTestResult = {
    ...
    try {
      ...
      result.status = 'FAILED';
      result.errors.push(
        `ESC/POS test failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
   ...
   */
  private static async performUSBAccessibilityTest(
    device: RONGTADevice
  ): Promise<{
    accessible: boolean;
    reliability: number;
    ...
    try {
      ...
      if (usbValidation.deviceFound) {
        ...
        if (usbValidation.driverPresent) reliability += 25; // Driver availability
        if (usbValidation.portAccessible) reliability += 25; // Port accessibility
        if (usbValidation.responseValid) reliability += 20; // Communication test
      }
      ...
      return {
        ...
        escPosCapable:
          accessibility && device.capabilities.model.includes('RP'),
      };
    } catch (error) {
   ...
   */
  private static async validateUSBDeviceAccess(device: RONGTADevice): Promise<{
    deviceFound: boolean;
   ...
   */
  private static async performSpoolerTest(printerName: string): Promise<{
    accessible: boolean;
    reliability: number;
    ...
    try {
      ...
      if (spoolerValidation.printerExists) {
        ...
        if (spoolerValidation.driverWorking) reliability += 30;
        if (spoolerValidation.spoolerReady) reliability += 20;
        if (spoolerValidation.canAcceptJobs) reliability += 10;
        ...
        result.supportedPaperSizes = this.determinePaperSizes(
          printerName,
          spoolerValidation.printerCapabilities
        );
   ...
   */
  private static async validateSpoolerAccess(printerName: string): Promise<{
    printerExists: boolean;
    driverWorking: boolean;
    spoolerReady: boolean;
    canAcceptJobs: boolean;
    printerCapabilities: any;
  }> {
    const result = {
      printerExists: false,
      driverWorking: false,
   ...
   */
  private static determinePaperSizes(
    printerName: string,
    capabilities: any
  ): string[] {
    const sizes: string[] = [];
    ...
    if (capabilities?.paperSizes) {
      ...
      if (paperSize.includes('a4')) sizes.push('A4');
      if (paperSize.includes('letter')) sizes.push('Letter');
    }
   ...
   */
  private static determineSpoolerFeatures(validation: any): string[] {
    ...
    if (
      validation.printerCapabilities?.duplex &&
   ...
   */
  private static async performESCPOSSimulation(
    device: RONGTADevice,
    testSuite: ESCPOSTestSuite
  ): Promise<{
    successful: boolean;
    reliability: number;
    ...
    try {
      ...
      const escPosValidation = await this.validateESCPOSCapabilities(
        device,
        testSuite
      );
   ...
   */
  private static async validateESCPOSCapabilities(
    device: RONGTADevice,
    testSuite: ESCPOSTestSuite
  ): Promise<{
    basicCommandsWork: boolean;
    hardwareCommandsWork: boolean;
    advancedCommandsWork: boolean;
    cutterResponsive: boolean;
    ...
    try {
      ...
      if (device.printerName && device.isInstalled) {
        const communicationTest = await this.testPrinterCommunication(
          device.printerName
        );
   ...
   */
  private static async testPrinterCommunication(printerName: string): Promise<{
    responsive: boolean;
    hardwareFeaturesWork: boolean;
    ...
    try {
      ...
      try {
        ...
        if (stdout.trim()) {
          ...
          let quality = 0;
          if (result.responsive) quality += 40;
          if (commInfo.queueWorking) quality += 30;
          if (commInfo.driverPresent) quality += 20;
          if (commInfo.portAccessible) quality += 10;
   ...
   */
  static rankConnectionMethods(testResults: ConnectionTestResult[]): string {
    const weights = {
