...
export class RONGTADetectionUtility {
  ...
  private static readonly RONGTA_MODEL_PATTERNS = {
  ...
  private static readonly RONGTA_NETWORK_PORTS = [80, 8080, 9100, 515];

  /**
   * Check if a USB device is a RONGTA device based on vendor/product IDs
   */
  static isRONGTAUSBDevice(vendorId: string, productId: string): boolean {
    ...
    const validProducts =
      this.RONGTA_USB_IDS.products[
        normalizedVendorId as keyof typeof this.RONGTA_USB_IDS.products
      ];
   ...
   */
  static determineRONGTACapabilities(modelName: string): RONGTACapabilities {
    const defaultCapabilities: RONGTACapabilities = {
   ...
   */
  static extractModelName(description: string, manufacturer?: string): string {
   ...
   */
  static getRONGTANetworkPorts(): number[] {
   ...
   */
  static isRONGTADevice(deviceName: string, description?: string): boolean {
   ...
   */
  static getModelSpecs(modelName: string): typeof RONGTADetectionUtility.RONGTA_MODEL_PATTERNS[keyof typeof RONGTADetectionUtility.RONGTA_MODEL_PATTERNS] | null {
    return this.RONGTA_MODEL_PATTERNS[modelName as keyof typeof this.RONGTA_MODEL_PATTERNS] || null;
  }
} 