...
export class DashboardController extends BaseController {
    ...
    registerHandlers() {
        ...
        this.registerHandler(DASHBOARD_CHANNELS.GET_DATA, this.getDashboardData.bind(this));
        enhancedLogger.info('DashboardController initialized with data aggregation capabilities', LogCategory.SYSTEM, 'DashboardController');
    }
    /**
     * Get consolidated dashboard data from multiple services
     */
    async getDashboardData(_event, dateRange) {
        try {
            enhancedLogger.info('Fetching dashboard data', LogCategory.BUSINESS, 'DashboardController');
            ...
            const [orderStatsResponse, menuStatsResponse, tablesResponse] = await Promise.all([
            ...
            if (!orderStatsResponse.success) {
                throw new Error(orderStatsResponse.error || 'Failed to fetch order stats');
            }
            if (!menuStatsResponse.success) {
                throw new Error(menuStatsResponse.error || 'Failed to fetch menu stats');
            }
            ...
            const topMenuItems = orderStats.topItems?.slice(0, 5).map((item, index) => {
            ...
            enhancedLogger.info(`Dashboard data compiled successfully: ${totalOrders} orders, $${totalRevenue.toFixed(2)} revenue`, LogCategory.BUSINESS, 'DashboardController');
            return {
                success: true,
                data: dashboardData,
            ...
            enhancedLogger.error(`Dashboard data fetch error: ${errorMessage}`, LogCategory.SYSTEM, 'DashboardController', {
