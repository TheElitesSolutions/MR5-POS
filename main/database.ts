...
class DatabaseManager {
  ...
  private postgresPath: string;
  private port: number = 5432; // Different port to avoid conflicts
   ...
   */
  private getPostgresPath(): string {
      ...
      initProcess.on('close', code => {
        if (code === 0) {
          ...
          const error = new Error(
            `Database initialization failed with code ${code}: ${errorOutput}`
          );
   ...
   */
  private createPasswordFile(): string {
  ...
  private async startPostgresServer(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      logDebug(
        `Starting PostgreSQL: ${postgresExe} ${args.join(' ')}`,
        'DatabaseManager'
      );
      ...
      this.postgresProcess.stdout?.on('data', data => {
        ...
        if (output.includes('database system is ready to accept connections')) {
          serverReady = true;
          logInfo(
            `PostgreSQL server started successfully on port ${this.port}`,
            'DatabaseManager'
          );
          resolve();
        }
      ...
      this.postgresProcess.stderr?.on('data', data => {
        ...
        if (
          output.includes('could not bind') ||
      ...
      this.postgresProcess.on('close', (code, signal) => {
        logInfo(
          `PostgreSQL process exited with code ${code} and signal ${signal}`,
          'DatabaseManager'
        );
        ...
        if (!serverReady && code !== 0) {
          const error = new Error(
            `PostgreSQL failed to start (exit code: ${code})\n${errorOutput}`
          );
          logError(error, 'DatabaseManager');
          reject(error);
        }
      ...
      setTimeout(() => {
        if (!serverReady) {
          logError(
            new Error('PostgreSQL startup timed out'),
            'DatabaseManager'
          );
          if (this.postgresProcess) {
   ...
   */
  private async stopPostgresServer(): Promise<void> {
    if (!this.postgresProcess) {
  ...
  private async waitForConnection(): Promise<void> {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        logDebug(
          `Connection attempt ${attempt} failed: ${error}`,
          'DatabaseManager'
        );
  ...
  public async initialize(): Promise<void> {
    try {
      ...
      if (getIsDev()) {
        logInfo(
          'Development mode: Using external PostgreSQL connection',
          'DatabaseManager'
        );
      ...
      logInfo(
        'Production mode: Starting embedded PostgreSQL',
        'DatabaseManager'
      );
      ...
      logError(error as Error, 'DatabaseManager');
      throw error;
    }
  }
  ...
  public async shutdown(): Promise<void> {
    try {
      ...
      if (getIsDev()) {
        logInfo(
          'Development mode: External PostgreSQL connection closed',
          'DatabaseManager'
        );
        this.isConnected = false;
        return;
