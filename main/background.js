...
import { app, ipcMain, dialog, Menu, protocol } from 'electron';
...
(async () => {
    ...
    try {
        ...
        if (!integrityOk) {
            logError(new Error('Post-update integrity check failed - attempting recovery'), 'Main');
            // Attempt recovery from backup
            const recoveryResult = await updateSafety.handleUpdateFailure();
            if (!recoveryResult.success) {
                dialog.showErrorBox('Update Recovery Failed', `The application failed to recover from an update error.\n\n${recoveryResult.error}\n\nPlease contact support.`);
                app.quit();
                return;
            }
        ...
        logError(error, 'Main - Update Safety Check');
    ...
    try {
        ServiceFactory.initializeServices(prisma);
        console.log('[Main] All services initialized successfully');
    }
    catch (error) {
        console.error('[Main] Service initialization failed:', error);
        dialog.showErrorBox('Service Initialization Error', `Failed to initialize services:\n\n${error instanceof Error ? error.message : String(error)}\n\nStack:\n${error instanceof Error ? error.stack : ''}`);
    }
    ...
    try {
        ...
        dialog.showErrorBox('Controller Initialization Error', `Failed to initialize controllers:\n\n${error instanceof Error ? error.message : String(error)}\n\nStack:\n${error instanceof Error ? error.stack : ''}`);
    }
    // Create main window
    ...
    if (isProd) {
        try {
            ...
            logError(error, 'Main - Auto-update initialization');
      ...
      `);
            if (result.isAuthenticated) {
                ...
                if (response.response === 1) {
                    ...
                    enhancedLogger.info('Force quit requested - clearing authentication and closing', LogCategory.SECURITY, 'Main', { user: result.user, action: 'force-quit' });
                    // Clear localStorage
                    await mainWindow.webContents.executeJavaScript(`
            localStorage.removeItem('auth-storage');
          `);
                    enhancedLogger.info('Authentication cleared - closing application', LogCategory.SECURITY, 'Main', { user: result.user });
                    ...
                    enhancedLogger.info('Close cancelled - user still logged in', LogCategory.SECURITY, 'Main', { user: result.user });
                }
            }
            else {
                // Not authenticated - allow close - use destroy to avoid re-triggering close event
                enhancedLogger.info('Closing app - user not authenticated', LogCategory.SECURITY, 'Main');
                // Reset crash count before closing (this is a normal close, not a crash)
                await getUpdateSafety().recordSuccessfulStartup();
                isForceClosing = true;
                mainWindow.destroy();
            ...
            enhancedLogger.error('Error checking authentication status - allowing close', LogCategory.SECURITY, 'Main', { error: error.message }, error);
            // Reset crash count before closing (this is a normal close, not a crash)
            await getUpdateSafety().recordSuccessfulStartup();
            isForceClosing = true;
            mainWindow.destroy();
...
ipcMain.handle('app:toggle-devtools', async () => {
    try {
        ...
        logError(error, 'Main - Toggle DevTools Handler');
...
ipcMain.handle('app:quit', async () => {
    try {
        ...
        logError(error, 'Main - App Quit Handler');
        return { success: false, error: error.message };
    }
});
