...
export class TableModel {
    ...
    mapPrismaTable(table) {
        ...
        if (table.orderId) {
            ...
            activeOrder = {
                ...
                updatedAt: table.orderUpdatedAt ||
    ...
    validateTableStatus(status) {
        ...
        switch (status) {
            ...
            default:
                ...
                logger.warn(`Invalid table status: ${status}, defaulting to AVAILABLE`, 'TableModel');
                return TableStatus.AVAILABLE;
        }
    }
    /**
    ...
    async getAllTables() {
      ...
      `;
            ...
            logger.error(`Failed to get all tables: ${error instanceof Error ? error.message : error}`, 'TableModel');
            return {
                success: false,
    ...
    async getTableById(id) {
        try {
            ...
            logger.error(`Failed to get table by ID ${id}: ${error instanceof Error ? error.message : error}`, 'TableModel');
            return {
                success: false,
                error: error instanceof Error ? error.message : 'Failed to retrieve table',
                timestamp: getCurrentLocalDateTime(),
            };
        }
    }
    ...
    async createTable(tableData) {
        try {
            ...
            logger.info('Table created successfully', `tableId: ${table.id}, tableName: ${table.name}`);
            return {
                success: true,
            ...
            logger.error(`Failed to create table ${tableData.name}: ${error instanceof Error ? error.message : error}`, 'TableModel');
            return {
                success: false,
    ...
    async updateTableStatus(id, status) {
        try {
            ...
            logger.info('Table status updated', `tableId: ${id}, newStatus: ${status}`);
            return {
                success: true,
                data: this.mapPrismaTable(table),
                timestamp: getCurrentLocalDateTime(),
            ...
            logger.error(`Failed to update table status for ${id} to ${status}: ${error instanceof Error ? error.message : error}`, 'TableModel');
            return {
                success: false,
    ...
    async deleteTable(id) {
        try {
            ...
            await this.prisma.$transaction(async (tx) => {
                ...
                // Log the operations
                logger.info(`Table ${id} deleted: Preserved ${preservedOrders.count} completed/cancelled orders, deleted ${deletedOrders.count} active orders`, 'TableModel');
            });
            return {
                success: true,
                data: true,
            ...
            logger.error(`Failed to delete table ${id}: ${error instanceof Error ? error.message : error}`, 'TableModel');
            return {
                success: false,
    ...
    async getAvailableTables() {
        try {
            ...
            logger.error(`Failed to get available tables: ${error instanceof Error ? error.message : error}`, 'TableModel');
            return {
                success: false,
                error: error instanceof Error
                    ? error.message
    ...
    async getTableStats() {
        try {
            const [total, available, occupied, outOfOrder, reserved] = await Promise.all([
            ...
            logger.error(`Failed to get table statistics: ${error instanceof Error ? error.message : error}`, 'TableModel');
            return {
                success: false,
                error: error instanceof Error
                    ? error.message
    ...
    async togglePayLater(id) {
        try {
            ...
            logger.info('Table pay later status toggled', `tableId: ${id}, newValue: ${newPayLaterValue}`);
            return {
                success: true,
                data: this.mapPrismaTable(table),
                timestamp: getCurrentLocalDateTime(),
            ...
            logger.error(`Failed to toggle pay later status for table ${id}: ${error instanceof Error ? error.message : error}`, 'TableModel');
            return {
                success: false,
                error: error instanceof Error
                    ? error.message
