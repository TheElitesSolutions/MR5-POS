...
import { OrderItemStatus, OrderStatus, OrderType, PaymentMethod, } from '../types';
import { addDecimals, decimalToNumber, multiplyDecimals, validateCurrencyAmount, toDecimal, } from '../utils/decimal';
...
function toISOString(date) {
    ...
    if (date instanceof Date)
        return dateToLocalDateTime(date); // FIXED: Use local time, not UTC
    return getCurrentLocalDateTime();
}
/**
 * Maps application OrderStatus to Prisma OrderStatus
 */
function mapAppOrderStatusToPrismaOrderStatus(status) {
 ...
 */
function mapPrismaOrderStatusToAppOrderStatus(status) {
    ...
    switch (status) {
        ...
        default:
            logger.warn(`Invalid order status: ${status}, defaulting to PENDING`, 'OrderModel');
            return OrderStatus.PENDING;
    }
}
/**
 ...
 */
function mapAppOrderItemStatusToPrismaOrderItemStatus(status) {
    // This handles the conversion between application enum and Prisma enum
    switch (status) {
 ...
 */
function mapPrismaOrderItemStatusToAppOrderItemStatus(status) {
    // This handles the conversion between Prisma enum and application enum
    switch (status) {
        case 'PENDING':
        ...
        default:
            logger.warn(`Invalid order item status: ${status}, defaulting to PENDING`, 'OrderModel');
            return OrderItemStatus.PENDING;
    }
}
/**
 ...
 */
function mapAppPaymentMethodToPrismaPaymentMethod(method) {
    if (!method)
        return null;
    // This handles the conversion between application enum and Prisma enum
    switch (method) {
 ...
 */
function mapPrismaPaymentMethodToAppPaymentMethod(method) {
    if (!method)
        return null;
    // This handles the conversion between Prisma enum and application enum
    switch (method) {
 ...
 */
function mapAppStatusToPrismaStatus(status) {
    switch (status) {
        case OrderStatus.DRAFT:
            return 'DRAFT';
        case OrderStatus.PENDING:
...
export class OrderModel {
    constructor(prisma) {
        ...
        this.pendingAddItemRequests = new Map();
     ...
     */
    addonsAreEqual(addons1, addons2) {
     ...
     */
    async fetchOrderItemsWithRelations(orderId) {
        ...
        const itemsWithRelations = await Promise.all(items.map(async (item) => {
            ...
            try {
                ...
                addonsWithDetails = await Promise.all(addons.map(async (addon) => {
                    try {
                        ...
                        logger.error(`Failed to fetch addon details for addon ${addon.id} on item ${item.id}: ${addonError}`, 'OrderModel');
                ...
                logger.error(`Failed to fetch addons for order item ${item.id}: ${error}`, 'OrderModel');
     ...
     */
    mapPrismaOrderStatusToAppStatus(status) {
        // Map Prisma OrderStatus to application OrderStatus
        switch (status) {
            case 'DRAFT':
                return OrderStatus.DRAFT;
     ...
     * This handles the differences between Prisma schema enums and application enums
     */
    mapPrismaOrderItemStatusToAppStatus(status) {
        // Map Prisma OrderItemStatus to application OrderItemStatus
        switch (status) {
            case 'PENDING':
                return OrderItemStatus.PENDING;
    ...
    validateOrderType(type) {
        switch (type) {
            ...
            default:
                logger.warn(`Invalid order type: ${type}, defaulting to DINE_IN`, 'OrderModel');
                return OrderType.DINE_IN;
        }
    }
    /**
    ...
    mapPrismaOrderItem(item) {
        try {
            ...
            const mappedStatus = this.mapPrismaOrderItemStatusToAppStatus(item.status);
            const mapped = {
                id: item.id,
            ...
            console.log(`🔍 OrderModel.mapPrismaOrderItem: Mapped result for ${item.id}:`, {
                itemId: mapped.id,
                hasAddonsInMapped: !!mapped.addons,
                addonsCountInMapped: (mapped.addons?.length || 0),
            });
            ...
            if (mapped.addons.length === 0 && expectedAddonPrice > 0.01) {
                logger.warn(`Item ${item.id} has empty addons array but totalPrice suggests addons exist - potential data loss`, 'OrderModel', {
                    itemId: item.id,
                    menuItemId: item.menuItemId,
                    basePrice,
            ...
            logger.error(`Failed to map order item ${item?.id || 'unknown'}: ${error}`, 'OrderModel');
        ...
        try {
            ...
            const mapped = {
                ...
                ...(order.deliveryFee
                    ...
                    : {}),
                items: order.items
                    ? order.items.map((item) => this.mapPrismaOrderItem(item))
                    : [],
            };
            // Bulletproof serialization: strip any remaining non-serializable properties
            ...
            logger.error(`Failed to map order ${order?.id || 'unknown'}: ${error}`, 'OrderModel');
            // Return a safe fallback object
            return {
                id: order?.id || 'unknown',
                orderNumber: order?.orderNumber || `ORD-${order?.id?.slice(-8) || 'unknown'}`,
    ...
    async findById(id) {
        try {
            ...
            console.log(`🔍 OrderModel.findById: Fetched order ${id}:`, {
                ...
                items: itemsWithRelations.map((i) => ({
                    id: i.id,
            ...
            logger.error(`Failed to get order by ID ${id}: ${error instanceof Error ? error.message : error}`, 'OrderModel');
            return {
                success: false,
    ...
    async create(orderData) {
        try {
            ...
            if (!validateCurrencyAmount(orderData.subtotal) ||
            ...
            if (orderData.userId === 'current-user' || orderData.userId === 'owner') {
                ...
                if (!userExists) {
                    ...
                    if (fallbackUser) {
                        validUserId = fallbackUser.id;
                        logger.warn(`Invalid userId provided (${orderData.userId}), using fallback user: ${fallbackUser.username}`);
                    }
                    else {
                        throw new AppError('Invalid user ID and no fallback user available', true);
                    }
                }
            }
            ...
            logger.error(`Failed to create order: ${error instanceof Error ? error.message : error}`, 'OrderModel');
            return {
                success: false,
    ...
    async update(id, updateData) {
        try {
            ...
            if (updateData.deliveryFee !== undefined) {
                logger.info(`🚨 [DELIVERY FEE UPDATE] Updating deliveryFee for order ${id}, will recalculate total`);
                ...
                logger.info(`✅ [DELIVERY FEE UPDATE] Recalculated total for order ${id}: ` +
            ...
            logger.error(`Failed to update order ${id}: ${error instanceof Error ? error.message : error}`, 'OrderModel');
            return {
                success: false,
    ...
    async cancel(id, reason) {
        try {
            ...
            const order = await this.prisma.$transaction(async (tx) => {
                ...
                logger.info(`Order cancelled successfully with stock restoration`, `orderId: ${id}, itemsRestored: ${existingOrder.items.length}`);
            ...
            logger.error(`Failed to cancel order ${id}: ${error instanceof Error ? error.message : error}`, 'OrderModel');
            return {
                success: false,
    ...
    async complete(id) {
        try {
            ...
            logger.error(`Failed to complete order ${id}: ${error instanceof Error ? error.message : error}`, 'OrderModel');
            return {
                success: false,
    ...
    async findByTable(tableId) {
        try {
            ...
            logger.error(`Failed to get orders by table ${tableId}: ${error instanceof Error ? error.message : error}`, 'OrderModel');
            return {
                success: false,
    ...
    async findByType(type) {
        try {
            ...
            logger.error(`Failed to get orders by type ${type}: ${error instanceof Error ? error.message : error}`, 'OrderModel');
            return {
                success: false,
                error: error instanceof Error
                    ? error.message
    ...
    async findByStatus(status) {
        try {
            ...
            logger.error(`Failed to get orders by status ${status}: ${error instanceof Error ? error.message : error}`, 'OrderModel');
            return {
                success: false,
                error: error instanceof Error
                    ? error.message
    ...
    async updateStatus(id, status) {
        try {
            ...
            logger.error(`Failed to update order status for ${id} to ${status}: ${error instanceof Error ? error.message : error}`, 'OrderModel');
            return {
                success: false,
                error: error instanceof Error
                    ? error.message
        ...
        const requestPromise = (async () => {
            try {
                ...
                const result = await this.prisma.$transaction(async (tx) => {
                    ...
                    const inventoryItems = await tx.menuItemInventory.findMany({
                    ...
                    const resolvedUnitPrice = item.unitPrice && validateCurrencyAmount(item.unitPrice)
                        ? item.unitPrice
                    ...
                    if (inventoryItems && inventoryItems.length > 0) {
                        logger.info(`🔍 STOCK DEDUCTION: Processing for ${menuItem.name} (quantity: ${item.quantity})`);
                        ...
                        for (const inventoryLink of inventoryItems) {
                            ...
                            const requiredQuantity = Number(inventoryLink.quantity) * item.quantity;
                        ...
                        for (const [inventoryId, requiredQuantity,] of Array.from(inventoryUpdates.entries())) {
                            ...
                            if (!inventoryItem) {
                                throw new AppError(`Inventory item ${inventoryId} not found`, true);
                            }
                            const currentStock = Number(inventoryItem.currentStock);
                            const newStock = currentStock - requiredQuantity;
                            if (newStock < 0) {
                                throw new AppError(`Insufficient stock for ${inventoryItem.itemName}. Available: ${currentStock}, Required: ${requiredQuantity}`, true);
                            }
                            ...
                            logger.info(`✅ STOCK DEDUCTED: ${inventoryItem.itemName} (${currentStock} → ${newStock} ${inventoryItem.unit})`);
                        }
                    }
                    let resultOrderItem;
                    if (existingItem) {
                        ...
                        const newTotalPrice = multiplyDecimals(resolvedUnitPrice, new DecimalJS(newQuantity));
                        ...
                        const totalPrice = multiplyDecimals(resolvedUnitPrice, new DecimalJS(item.quantity));
                        const orderItemData = {
                            orderId,
                            menuItemId: item.menuItemId,
                            quantity: item.quantity,
                    ...
                    logger.info(`✅ [TRANSACTION] Item added and totals updated atomically for order ${orderId}`);
                ...
                try {
                    ...
                    if (validationOrder) {
                        ...
                        if (hasItems && totalIsZero) {
                            logger.error(`🚨 CRITICAL: Order ${orderId} has ${validationOrder.items.length} items ` +
                                `but total is $0! Attempting recovery...`);
                            // Attempt recovery using the external recalculation method
                            await this.recalculateOrderTotals(orderId);
                        }
                        else {
                            logger.info(`✅ [VALIDATION] Order ${orderId}: ${validationOrder.items.length} items, ` +
                ...
                logger.error(`Failed to add item to order: ${error instanceof Error ? error.message : 'Unknown error'}`);
                return {
                    success: false,
                    error: error instanceof Error
                        ? error.message
    ...
    async removeItem(orderItemId) {
        try {
            ...
            const result = await this.prisma.$transaction(async (tx) => {
                ...
                if (inventoryItems && inventoryItems.length > 0) {
                    logger.info(`🔄 STOCK RESTORATION: Processing for ${orderItem.menuItem.name} (quantity: ${orderItem.quantity})`);
                    const inventoryUpdates = new Map();
                    // Calculate total quantities to restore per inventory item
                    for (const inventoryLink of inventoryItems) {
                        const inventoryId = inventoryLink.inventoryId;
                        const restoreQuantity = Number(inventoryLink.quantity) * orderItem.quantity;
                        const currentTotal = inventoryUpdates.get(inventoryId) || 0;
                    ...
                    for (const [inventoryId, restoreQuantity,] of Array.from(inventoryUpdates.entries())) {
                        ...
                        if (!inventoryItem) {
                            logger.warn(`Inventory item ${inventoryId} not found during restoration`);
                            continue;
                        }
                        const currentStock = Number(inventoryItem.currentStock);
                        ...
                        logger.info(`✅ STOCK RESTORED: ${inventoryItem.itemName} (${currentStock} → ${newStock} ${inventoryItem.unit})`);
                    }
                }
                ...
                logger.info(`✅ [TRANSACTION] Item removed and totals updated atomically for order ${orderItem.orderId}`);
            ...
            try {
                ...
                if (validationOrder) {
                    ...
                    const totalIsZero = new DecimalJS(validationOrder.total).equals(0);
                    if (hasItems && totalIsZero) {
                        logger.error(`🚨 CRITICAL: Order ${result} has ${validationOrder.items.length} items ` +
                            `but total is $0! Attempting recovery...`);
                        // Attempt recovery using the external recalculation method
                        await this.recalculateOrderTotals(result);
                    }
                    else {
                        logger.info(`✅ [VALIDATION] Order ${result}: ${validationOrder.items.length} items, ` +
                            `Total: $${validationOrder.total} - OK`);
                    }
                }
            }
            ...
            logger.error(`Failed to remove item from order: ${error instanceof Error ? error.message : 'Unknown error'}`);
            return {
                success: false,
                error: error instanceof Error
                    ? error.message
     ...
     */
    async updateItemQuantity(orderItemId, newQuantity) {
        try {
            if (newQuantity <= 0) {
                return {
                    success: false,
            ...
            const result = await this.prisma.$transaction(async (tx) => {
                ...
                if (quantityDifference !== 0 &&
                    ...
                    logger.info(`🔄 STOCK ADJUSTMENT: Processing for ${orderItem.menuItem.name} (${currentQuantity} → ${newQuantity})`);
                    // Calculate stock adjustments needed per inventory item
                    for (const inventoryLink of inventoryItems) {
                        const inventoryId = inventoryLink.inventoryId;
                        const adjustmentQuantity = Number(inventoryLink.quantity) * Math.abs(quantityDifference);
                        const currentTotal = inventoryUpdates.get(inventoryId) || 0;
                        inventoryUpdates.set(inventoryId, currentTotal + adjustmentQuantity);
                    }
                    // Apply stock adjustments atomically within the same transaction
                    for (const [inventoryId, adjustmentQuantity,] of Array.from(inventoryUpdates.entries())) {
                        const inventoryItem = await tx.inventory.findUnique({
                            where: { id: inventoryId },
                        });
                        if (!inventoryItem) {
                            logger.warn(`Inventory item ${inventoryId} not found during quantity update`);
                            continue;
                        }
                        const currentStock = Number(inventoryItem.currentStock);
                        let newStock;
                        ...
                        if (quantityDifference > 0) {
                            ...
                            if (newStock < 0) {
                                throw new AppError(`Insufficient stock for ${inventoryItem.itemName}. Available: ${currentStock}, Required: ${adjustmentQuantity}`, true);
                            }
                        }
                        else {
                        ...
                        logger.info(`${actionEmoji} STOCK ADJUSTED: ${inventoryItem.itemName} (${currentStock} → ${newStock} ${inventoryItem.unit})`);
                    }
                }
                ...
                if (quantityDifference !== 0) {
                    ...
                    if (orderItemAddons && orderItemAddons.length > 0) {
                        logger.info(`🔄 ADDON STOCK ADJUSTMENT: Processing ${orderItemAddons.length} addons for order item quantity change (${currentQuantity} → ${newQuantity})`);
                                ...
                                for (const addonInvItem of addon.inventoryItems) {
                                    if (addonInvItem.inventory && addonInvItem.quantity > 0) {
                                        ...
                                        if (quantityDifference > 0) {
                                            ...
                                            if (newStock < 0) {
                                                throw new AppError(`Insufficient addon stock for ${addon.name}. Available: ${currentStock}, Required: ${totalAdjustment}`, true);
                                            }
                                        }
                                        else {
                                        ...
                                        const actionEmoji = quantityDifference > 0 ? '📉' : '📈';
                                        logger.info(`${actionEmoji} ADDON STOCK ADJUSTED: ${addon.name} → ${addonInvItem.inventory.itemName} (${currentStock} → ${newStock})`);
                                    }
                                }
                            }
                        }
                ...
                logger.debug(`📦 ADDON CALCULATION: Per-item cost: ${addonTotalPerItem}, Item qty: ${newQuantity}, Total: ${addonTotalScaled}`);
                logger.info(`💰 PRICE CALCULATION: Item ${orderItem.menuItem?.name || 'Unknown'} - Base: ${unitPrice} × ${newQuantity} = ${new DecimalJS(newQuantity).mul(unitPrice || 0)}, Addons: ${addonTotalPerItem} × ${newQuantity} = ${addonTotalScaled}, Total: ${new DecimalJS(newQuantity).mul(unitPrice || 0).add(addonTotalScaled)}`);
            ...
            logger.error(`Failed to update order item quantity: ${error instanceof Error ? error.message : 'Unknown error'}`);
            return {
                success: false,
                error: error instanceof Error
                    ? error.message
     ...
     */
    async calculateAndUpdateTotalsInTransaction(tx, orderId) {
 ...
 */
export function mapPrismaOrderItemToDTO(prismaOrderItem) {
    // Use type assertion to handle properties that might not exist in the Prisma model
    const orderItem = prismaOrderItem;
    return {
        id: orderItem.id,
        orderId: orderItem.orderId,
        menuItemId: orderItem.menuItemId,
 ...
 */
export function mapPrismaOrderToDTO(prismaOrder) {
    // Use type assertion to handle properties that might not exist in the Prisma model
    const order = prismaOrder;
    return {
        id: order.id,
        orderNumber: order.orderNumber || `ORD-${order.id.slice(-8)}`,
        tableId: order.tableId,
