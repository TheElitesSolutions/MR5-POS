...
export class OptimizedPrintingService {
    ...
    constructor() {
        ...
        this.PRINTER_CACHE_DURATION = 10 * 60 * 1000; // 10 minutes - stable printer configurations
        this.ORDER_CACHE_DURATION = 5 * 60 * 1000; // 5 minutes - active order sessions
        // Performance optimizations
        this.OPTIMIZED_PRINT_DELAY = 100; // 100ms instead of 15 seconds
        this.MAX_QUEUE_SIZE = 100;
        this.QUEUE_PROCESS_INTERVAL = 50; // Process queue every 50ms
    ...
    registerIPCHandlers(ipcMain) {
        ...
        try {
            ...
            ipcMain.handle('print-receipt-optimized', this.handleOptimizedPrintRequest.bind(this));
            // Queue status check
     ...
     */
    async handleOptimizedPrintRequest(event, request) {
        try {
            this.logger.info(`🚀 INSTANT PRINT: Queuing ${request.isInvoice ? 'invoice' : request.isKitchenOrder ? 'kitchen' : 'receipt'} for order ${request.orderId}`);
            // Validate request
            ...
            if (this.printQueue.length >= this.MAX_QUEUE_SIZE) {
                ...
                const nonPriorityIndex = this.printQueue.findIndex(job => job.priority > 1);
                if (nonPriorityIndex !== -1) {
                    this.printQueue.splice(nonPriorityIndex, 1);
                    this.logger.warn(`⚠️ Print queue full, removed oldest non-priority job`);
                }
            }
            this.printQueue.push(printJob);
            this.sortQueue(); // Sort by priority
            this.logger.info(`⚡ INSTANT RESPONSE: Print job ${jobId} queued successfully (${this.printQueue.length} jobs in queue)`);
    ...
    startQueueProcessor() {
        ...
        setInterval(async () => {
            ...
            if (this.printQueue.length > 0 || this.isProcessingQueue) {
                this.logger.info(`🔥 [Queue Processor] Checking queue... (length: ${this.printQueue.length}, isProcessing: ${this.isProcessingQueue})`);
            }
            ...
            try {
                ...
                this.logger.info(`🔥 [Queue Processor] PICKED UP JOB: ${job.id} (type: ${job.type}, orderId: ${job.orderId}, priority: ${job.priority})`);
                this.logger.info(`🖨️ BACKGROUND PROCESSING: Starting ${job.type} print job ${job.id}`);
                job.status = 'printing';
                // Process the print job using optimized method
                const success = await this.processOptimizedPrintJob(job);
                job.status = success ? 'completed' : 'failed';
                this.logger.info(`${success ? '✅' : '❌'} Print job ${job.id} ${job.status}`);
            }
            catch (error) {
     ...
     */
    async processOptimizedPrintJob(job) {
        try {
            // ✅ DIAGNOSTIC: Log entry
            this.logger.info(`🔥 [processOptimizedPrintJob] CALLED for job ${job.id} (type: ${job.type}, orderId: ${job.orderId})`);
            ...
            this.logger.info(`🔥 [processOptimizedPrintJob] Printer found: ${printer.name}`);
            ...
            this.logger.info(`🔥 [processOptimizedPrintJob] Order data retrieved: ${job.orderId} (${orderData.order.items?.length || 0} items)`);
            this.logger.info(`🔍 Print processing using ${bypassCache ? 'FRESH' : 'CACHED'} order data for ${job.orderId}`);
            // Step 3: Use optimized print method with exact format preservation
            // ✅ DIAGNOSTIC: Log before calling optimizedDirectPrint
            this.logger.info(`🔥 [processOptimizedPrintJob] Calling optimizedDirectPrint for ${job.type}...`);
            const success = await this.optimizedDirectPrint(job, orderData, printer);
            // ✅ DIAGNOSTIC: Log result
            this.logger.info(`🔥 [processOptimizedPrintJob] optimizedDirectPrint returned: ${success}`);
            return success;
        }
        catch (error) {
     ...
     */
    async optimizedDirectPrint(job, orderData, printer) {
        try {
            // ✅ DIAGNOSTIC: Log entry with job details
            this.logger.info(`🔥 [optimizedDirectPrint] ENTRY - Job type: ${job.type}, OrderId: ${orderData.order.id || orderData.order.orderNumber}, Items: ${orderData.order.items?.length || 0}`);
            this.logger.info(`🖨️ OPTIMIZED PRINT: Using electron-pos-printer with ${this.OPTIMIZED_PRINT_DELAY}ms delay`);
            ...
            if (job.type === 'invoice') {
                ...
                this.logger.info(`🔥 [optimizedDirectPrint] Generating KITCHEN TICKET content...`);
                this.logger.info(`🔥 [optimizedDirectPrint] Parameters: cancelledItems=${job.cancelledItems?.length || 0}, updatedItemIds=${job.updatedItemIds?.length || 0}, itemChanges=${job.itemChanges?.length || 0}`);
                this.logger.info(`🔥 [optimizedDirectPrint] About to call generateKitchenContent() NOW...`);
                printContent = await this.generateKitchenContent(orderData.order, job.cancelledItems, job.updatedItemIds, job.itemChanges // Pass the enhanced change tracking information
                );
                // ✅ DIAGNOSTIC: Log after generateKitchenContent returns
                this.logger.info(`🔥 [optimizedDirectPrint] Kitchen content generated successfully! Length: ${printContent.length} chars`);
            }
            else {
            ...
            if (!printData || printData.length === 0) {
                ...
                if (mainWindow) {
                    mainWindow.webContents.send('print-error', {
                        jobId: job.id,
                        orderId: orderData.order.id || orderData.order.orderNumber,
            ...
            return new Promise(resolve => {
                PosPrinter.print(printData, printOptions)
                    ...
                    this.logger.info(`⚡ SPEED OPTIMIZED: Quick finalization delay ${this.OPTIMIZED_PRINT_DELAY}ms for instant printing...`);
                    await new Promise(delayResolve => setTimeout(delayResolve, this.OPTIMIZED_PRINT_DELAY));
                    this.logger.info(`✅ Print finalization completed - 150x faster than original!`);
                    resolve(true);
                })
                    .catch((error) => {
                    enhancedLogger.error(`❌ electron-pos-printer failed`, LogCategory.BUSINESS, 'OptimizedPrintingService', {
                        error,
            ...
            enhancedLogger.error(`❌ Optimized print error`, LogCategory.BUSINESS, 'OptimizedPrintingService', { error, errorString: String(error), stack: error instanceof Error ? error.stack : undefined });
            return false;
        }
    }
    /**
     ...
     */
    async getCachedPrinter(printerName) {
        const now = Date.now();
     ...
     */
    async getCachedOrder(orderId, bypassCache = false) {
        const now = Date.now();
        ...
        // Log cache bypass
        if (bypassCache) {
            this.logger.info(`🔄 CACHE BYPASS: Forcing fresh order fetch for ${orderId}`);
        }
        try {
     ...
     */
    async fetchOrderQuickly(orderId) {
        try {
            ...
            if (orderResult.success) {
                this.logger.info(`📋 Order ${orderId} fetched successfully for optimized printing`);
     ...
     */
    async generateInvoiceContent(order) {
        try {
            ...
            const invoiceContent = await addonInvoiceGenerator.generateEnhancedThermalInvoiceWithAddons(order, businessInfo, getResourcesPath);
            ...
            <p style="font-size: 16px; font-weight: bold;">Total: $${order.totalAmount || order.total}</p>
     ...
     */
    async generateKitchenContent(order, cancelledItems, updatedItemIds, itemChanges // Enhanced change tracking information
    ) {
        ...
        try {
            ...
            const result = await generateEnhancedKitchenTicket(order, false, // Not just unprinted items
     ...
     */
    async generateReceiptContent(order) {
        // Import the original PrinterController class
        const { PrinterController } = require('../controllers/printerController');
        const { LebanesReceiptGenerator } = require('../utils/receiptGenerator');
        const businessInfo = LebanesReceiptGenerator.getDefaultBusinessInfo();
        ...
        // Wrap in try-catch to handle any potential errors
        try {
            return await printerController.generateEnhancedThermalReceipt(order, businessInfo);
        }
        catch (error) {
