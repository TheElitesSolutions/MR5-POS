...
export class AddonCacheService {
    constructor(addonService, config = {}) {
     ...
     */
    async getCategoryAddons(categoryId, useCache = true) {
        ...
        try {
            ...
            if (useCache) {
                ...
                await this.setCache(cacheKey, cacheData, this.config.ttl.categoryAddons);
            }
            return result;
        }
        catch (error) {
     ...
     */
    async getAddonGroups(filters, useCache = true) {
        ...
        try {
            ...
            // Cache the result
            if (useCache) {
                await this.setCache(cacheKey, result.data, this.config.ttl.addonGroups);
            }
            return result;
        }
        catch (error) {
     ...
     */
    async invalidateAddonGroupCache(groupId) {
        if (groupId) {
     ...
     */
    async warmUpCache(categoryIds) {
        console.log(`🔄 Warming up cache for ${categoryIds.length} categories...`);
        const promises = categoryIds.map(categoryId => this.getCategoryAddons(categoryId, true).catch(error => console.warn(`Failed to warm cache for category ${categoryId}:`, error)));
