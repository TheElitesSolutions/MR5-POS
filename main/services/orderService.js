 ...
 */
function mapPrismaOrderStatusToAppOrderStatus(prismaStatus) {
    switch (prismaStatus) {
        case 'DRAFT':
 ...
 */
function mapAppOrderStatusToPrismaOrderStatus(appStatus) {
    const status = typeof appStatus === 'string' ? appStatus : appStatus;
    switch (status) {
 ...
 */
function mapPrismaOrderItemStatusToAppOrderItemStatus(prismaStatus) {
    switch (prismaStatus) {
        case 'PENDING':
 ...
 */
function mapAppOrderItemStatusToPrismaOrderItemStatus(appStatus) {
    switch (appStatus) {
 ...
 */
function mapPrismaPaymentMethodToAppPaymentMethod(prismaMethod) {
    if (!prismaMethod)
        return undefined;
    switch (prismaMethod) {
        case 'CASH':
 ...
 */
function mapAppPaymentMethodToPrismaPaymentMethod(appMethod) {
    if (!appMethod)
        return null;
    // Convert string to PaymentMethod if needed
    const method = typeof appMethod === 'string' ? appMethod : appMethod;
    switch (method) {
 ...
 */
function mapPrismaOrderTypeToAppOrderType(prismaType) {
    switch (prismaType) {
        case 'DINE_IN':
 ...
 */
function mapAppOrderTypeToPrismaOrderType(appType) {
    const type = typeof appType === 'string' ? appType : appType;
    switch (type) {
...
export class OrderService extends BaseService {
    ...
    async findAll(filters) {
        return this.wrapMethod(async () => {
            ...
            console.log('OrderService: building parameterized query for filters:', filters);
      ...
      `;
            ...
            if (filters?.status) {
                const prismaStatus = mapAppOrderStatusToPrismaOrderStatus(filters.status);
                query += ` AND o.status = ?`;
            ...
            console.log('OrderService: executing raw SQL query:', query, 'with params:', queryParams);
            // Execute the raw query
            try {
                const orders = await this.prisma.$queryRawUnsafe(query, ...queryParams);
                // Cast the result to any[] to allow mapping
                const ordersArray = orders;
                console.log(`OrderService: raw query returned ${ordersArray.length} orders`);
                ...
                return ordersWithItems.map(order => this.mapPrismaOrder(order));
            }
            catch (error) {
    ...
    async findById(id) {
        return this.wrapMethod(async () => {
            ...
            return this.mapPrismaOrder(order);
        })();
    }
    /**
     * Create a new order
    ...
    async create(orderData) {
        return this.wrapMethod(async () => {
            ...
            const subtotal = hasItems
                ? orderItems.reduce((sum, item) => sum.add(item.subtotal), new Decimal(0))
                : new Decimal(0);
            ...
            if (hasItems) {
                try {
                    ...
                    const stockCheck = await inventoryService.checkStockAvailability(orderData.items.map(item => ({
                        menuItemId: item.menuItemId,
                        quantity: item.quantity,
                    })));
                    if (!stockCheck.success) {
                        throw new AppError(stockCheck.error || 'Failed to check stock availability');
                    }
                    if (stockCheck.data && !stockCheck.data.available) {
                        const unavailableItems = stockCheck.data.unavailableItems;
                        throw new AppError(`Insufficient stock for ${unavailableItems.length} item(s): ${unavailableItems
                            .map(i => `${i.name} (needs ${i.required} ${i.unit}, have ${i.available} ${i.unit})`)
                            .join(', ')}`, true);
                    }
                }
                catch (error) {
                    ...
                    console.warn('⚠️ WARNING: Could not check inventory availability:', error);
            ...
            const order = await this.executeTransaction(async (tx) => {
                ...
                if (hasItems && orderItems.length > 0) {
                    console.log('🔍 ATOMIC: Processing inventory within order transaction');
                    ...
                    for (const [inventoryId, requiredQuantity,] of Array.from(inventoryUpdates.entries())) {
                        ...
                        if (!inventoryItem) {
                            console.warn(`Inventory item ${inventoryId} not found during order processing`);
                            continue;
                        }
                    ...
                    console.log('✅ ATOMIC: Order and inventory operations completed atomically');
                }
                return newOrder;
            }).catch(error => {
            ...
            console.log('✅ ATOMIC FIX: Order and inventory operations completed atomically');
            return this.mapPrismaOrder(order);
        })();
    }
    /**
    ...
    async update(id, updateData) {
        return this.wrapMethod(async () => {
            ...
            if (['COMPLETED', 'CANCELLED'].includes(existingOrder.status)) {
                throw new AppError(`Cannot update a ${existingOrder.status.toLowerCase()} order`);
            }
            ...
            const tax = new Decimal(0);
            // Handle delivery fee for delivery orders
            const deliveryFee = updateData.updates.deliveryFee !== undefined
            ...
            const order = await this.executeTransaction(async (tx) => {
                ...
                if (prismaOrderStatus === 'COMPLETED' &&
                ...
                const updatedOrder = await tx.order.update({
                    ...
                    data: {
                        ...
                        deliveryFee,
                        total,
                        notes: updateData.updates.notes !== undefined
    ...
    async updateStatus(statusRequest) {
        ...
        return this.wrapMethod(async () => {
            ...
            const order = await this.executeTransaction(async (tx) => {
                ...
                console.log('🔍 TRANSACTION: Starting order status update transaction', {
                    orderId: id,
                ...
                if (prismaOrderStatus === 'COMPLETED' &&
                ...
                else if (prismaOrderStatus === 'CANCELLED' &&
                    existingOrder.status !== 'CANCELLED') {
                    // CRITICAL FIX: Restore stock for cancelled orders
                    console.log('🔄 STOCK RESTORATION: Processing cancelled order stock restoration', {
                        orderId: id,
                    ...
                    try {
                        for (const orderItem of orderItems) {
                            if (orderItem.menuItem?.inventoryItems &&
                                orderItem.menuItem.inventoryItems.length > 0) {
                                for (const ingredientLink of orderItem.menuItem
                                    .inventoryItems) {
                                    const restoreQuantity = Number(ingredientLink.quantity) * orderItem.quantity;
                                    // Enhanced error handling: Validate quantities before processing
                                    if (restoreQuantity <= 0) {
                                        console.warn('⚠️ STOCK RESTORATION: Invalid restore quantity', {
                                    ...
                                    if (!currentInventory) {
                                        const error = new Error(`Inventory item not found: ${ingredientLink.inventoryId}`);
                                        console.error('❌ STOCK RESTORATION: Critical error - inventory not found', {
                                            inventoryId: ingredientLink.inventoryId,
                                            orderId: id,
                                    ...
                                    const newStock = Number(currentInventory.currentStock) + restoreQuantity;
                                    // Enhanced error handling: Validate final stock levels
                                    if (newStock < 0) {
                                        const error = new Error(`Invalid stock calculation during restoration: ${ingredientLink.inventory.itemName}`);
                                        console.error('❌ STOCK RESTORATION: Invalid stock calculation', {
                                            inventoryId: ingredientLink.inventoryId,
                                    ...
                                    try {
                                        ...
                                        await tx.auditLog.create({
                                            data: {
                                                ...
                                                newValues: {
                                                    reason: 'Order cancelled - automatic stock restoration',
                                                    orderId: id,
                                                    orderItemId: orderItem.id,
                                        ...
                                        console.error('❌ STOCK RESTORATION: Failed to update inventory', {
                                            inventoryId: ingredientLink.inventoryId,
                                            itemName: ingredientLink.inventory.itemName,
                        ...
                        console.error('❌ STOCK RESTORATION: Transaction failed, automatic rollback initiated', {
                            orderId: id,
                            error: restorationError instanceof Error
                        ...
                        data: {
                            ...
                            newValues: {
                                ...
                                ...(prismaOrderStatus === 'CANCELLED' && {
                                    ...
                                    restoredItemsCount: (existingOrder.items || []).filter(item => item.menuItem?.inventoryItems?.length).length,
                                }),
                            },
                        },
                    });
            ...
            console.log('🎉 ORDER STATUS UPDATE: Transaction completed successfully', {
                orderId: id,
            ...
            if (!(error instanceof AppError)) {
                throw new AppError(`Failed to update order status: ${error instanceof Error ? error.message : 'Unknown error'}`, false);
            }
            throw error;
        });
    }
        ...
        return this.wrapMethod(async () => {
            ...
            try {
                ...
                if (error.message && error.message.includes('completedAt')) {
                    console.warn('[OrderService] Working around missing completedAt column:', error.message);
                    // Try to add the missing column automatically
                    try {
                        console.log('[OrderService] Attempting to add completedAt column...');
                        // Check if column exists first (SQLite-compatible way)
                        const tableInfo = await this.prisma.$queryRawUnsafe('PRAGMA table_info(orders)');
                        const hasCompletedAt = tableInfo.some((col) => col.name === 'completedAt');
                        if (!hasCompletedAt) {
                            await this.prisma.$executeRawUnsafe('ALTER TABLE orders ADD COLUMN completedAt TEXT');
                        ...
                        console.log('[OrderService] ✅ Successfully fetched orders after adding column');
                        return orders.map(order => this.mapPrismaOrder(order));
                    }
                    catch (migrationError) {
                        console.warn('[OrderService] Could not add column automatically:', migrationError.message);
     ...
     */
    async findByType(orderType) {
        return this.wrapMethod(async () => {
            const prismaOrderType = mapAppOrderTypeToPrismaOrderType(orderType);
    ...
    async search(params) {
        return this.wrapMethod(async () => {
            ...
            if (params.paymentMethod) {
                where.paymentMethod = mapAppPaymentMethodToPrismaPaymentMethod(params.paymentMethod);
            }
            const orders = await this.prisma.order.findMany({
                where,
                orderBy: { createdAt: 'desc' },
    ...
    async getStats(dateRange) {
        return this.wrapMethod(async () => {
            ...
            const statusCounts = await this.prisma.$queryRawUnsafe(statusCountsQuery, ...queryParams);
      ...
      `;
            ...
            const salesResults = await this.prisma.$queryRawUnsafe(salesQuery, ...salesParams);
            ...
            if (menuItemIds.length > 0) {
                ...
                const categoryIds = [...new Set(menuItems.map((mi) => mi.categoryId).filter(Boolean))];
            ...
            const totalValue = sum && typeof sum === 'object' && 'total' in sum
                ? sum.total
                : new Decimal(0);
            const subtotalValue = sum && typeof sum === 'object' && 'subtotal' in sum
                ? sum.subtotal
                : new Decimal(0);
            const taxValue = sum && typeof sum === 'object' && 'tax' in sum
                ? sum.tax
                : new Decimal(0);
            // Calculate total orders from ONLY completed and served statuses
            const completedOrdersCount = statusCounts
                .filter((s) => s.status === 'COMPLETED' || s.status === 'SERVED')
            ...
            return {
                ...
                statusCounts: statusCounts.reduce((obj, s) => {
                    ...
                    const appStatus = mapPrismaOrderStatusToAppOrderStatus(s.status).toLowerCase();
                    obj[appStatus] = Number(s.count || 0);
                    return obj;
                }, {}),
                sales: {
                    total: decimalToNumber(totalValue),
                    subtotal: decimalToNumber(subtotalValue),
                    tax: decimalToNumber(taxValue),
                    count: count,
