...
export class InventoryService extends BaseService {
   ...
   */
  async increaseStockForOrderItem(
    orderItemId: string
  ): Promise<IPCResponse<{ success: boolean }>> {
    try {
      AdvancedLogger.info(
        `Restoring inventory for removed order item: ${orderItemId}`
      );
      ...
      if (!ingredients || ingredients.length === 0) {
        AdvancedLogger.info(
          `Menu item ${orderItem.menuItem?.name || menuItemId} has no linked inventory items to restore`
        );
        return this.createSuccessResponse({ success: true });
      }

      // Calculate inventory to restore
      const inventoryUpdates: Map<string, Decimal> = new Map();

      // Process the ingredients
      for (const inventoryLink of ingredients) {
        const inventoryId = inventoryLink.inventoryId;
        const restoreQuantity = new Decimal(inventoryLink.quantity).mul(
          quantity
        );
        ...
        AdvancedLogger.info(
          `Calculated inventory restoration for removed item`,
          {
            inventoryId,
      ...
      await this.executeTransaction(async tx => {
        ...
        for (const [
          ...
          AdvancedLogger.info(
            `Restored inventory for ${inventoryItem.itemName}`,
            {
              inventoryId,
        ...
        await tx.auditLog.create({
          data: {
            ...
            newValues: {
              inventoryUpdates: JSON.stringify(
  ...
  async decreaseStockForOrder(
    ...
    orderId: string
  ): Promise<IPCResponse<{ success: boolean; lowStockItems: Inventory[] }>> {
    try {
      ...
      for (const {
        ...
        for (const inventoryLink of ingredients) {
          const inventoryId = inventoryLink.inventoryId;
          const requiredQuantity = new Decimal(inventoryLink.quantity).mul(
            quantity
          );

          // Add to existing quantity or create new entry
          const currentTotal =
            inventoryUpdates.get(inventoryId) || new Decimal(0);
      ...
      await this.executeTransaction(async tx => {
        ...
        for (const [
          ...
          AdvancedLogger.info(
            `Updated inventory for ${inventoryItem.itemName}`,
            {
              inventoryId,
              previousStock: inventoryItem.currentStock.toString(),
          ...
          if (newStock.lte(inventoryItem.minimumStock)) {
            ...
            AdvancedLogger.warn(
              `Low stock detected for ${inventoryItem.itemName}`,
              {
                inventoryId,
  ...
  > {
    try {
      ...
      // Calculate required inventory for each item
      const inventoryRequirements: Map<
        string,
        {
          required: Decimal;
          item: { id: string; name: string; unit: string; stock: Decimal };
        }
      > = new Map();
  ...
  async getLowStockItems(): Promise<IPCResponse<Inventory[]>> {
    try {
      ...
      const lowStockItems = await this.prisma.$queryRawUnsafe<Inventory[]>(
