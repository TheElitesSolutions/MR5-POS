...
export class ReportService extends BaseService {
    ...
    async getSalesReport(dateRange) {
        try {
            enhancedLogger.info(`Generating sales report from ${dateRange.startDate} to ${dateRange.endDate}`, LogCategory.BUSINESS, 'ReportService');
            ...
            enhancedLogger.info(`Sales report generated successfully. Total revenue: $${totalRevenueNum.toFixed(2)}, Orders: ${totalOrders}`, LogCategory.BUSINESS, 'ReportService');
            return this.createSuccessResponse(reportData);
        }
        catch (error) {
            enhancedLogger.error(`Failed to generate sales report: ${error}`, LogCategory.ERROR, 'ReportService');
    ...
    async getInventoryReport(dateRange) {
        try {
            enhancedLogger.info(`Generating inventory report from ${dateRange.startDate} to ${dateRange.endDate}`, LogCategory.BUSINESS, 'ReportService');
            ...
            if (filteredCount > 0) {
                enhancedLogger.info(`Filtered out ${filteredCount} placeholder items from inventory report`, LogCategory.BUSINESS, 'ReportService');
            }
            // Calculate summary KPIs
            ...
            orderItems.forEach(orderItem => {
                ...
                menuItemInventories.forEach(mii => {
                    ...
                    const isPlaceholder = invItem.itemName?.toLowerCase().includes('placeholder') ||
            ...
            enhancedLogger.info(`Inventory report generated successfully. Total items: ${inventoryItems.length}, Low stock: ${lowStockCount}`, LogCategory.BUSINESS, 'ReportService');
            return this.createSuccessResponse(reportData);
        }
        catch (error) {
            enhancedLogger.error(`Failed to generate inventory report: ${error}`, LogCategory.ERROR, 'ReportService');
    ...
    async getProfitReport(dateRange) {
        try {
            enhancedLogger.info(`Generating profit report from ${dateRange.startDate} to ${dateRange.endDate}`, LogCategory.BUSINESS, 'ReportService');
            ...
            enhancedLogger.info(`Profit report generated successfully. Revenue: $${totalRevenueNum.toFixed(2)}, Profit: $${grossProfit.toFixed(2)}, Margin: ${profitMargin.toFixed(2)}%`, LogCategory.BUSINESS, 'ReportService');
            return this.createSuccessResponse(reportData);
        }
        catch (error) {
            enhancedLogger.error(`Failed to generate profit report: ${error}`, LogCategory.ERROR, 'ReportService');
