...
export class EnhancedPrinterService extends EventEmitter {
     ...
     */
    async diagnosePrinter(printerName) {
        ...
        try {
            ...
            if (!result.driverInstalled) {
                result.recommendedActions.push('Install or reinstall RONGTA printer driver');
            }
            // Step 3: Test printer communication interfaces
            const communicationTest = await this.testPrinterCommunication(printerName);
            ...
            if (this.isRongtaPrinter(printerName)) {
                const rongtaChecks = await this.performRongtaSpecificChecks(printerName);
            ...
            result.recommendedActions.push('Check system logs for detailed error information');
     ...
     */
    async recoverPrinter(printerName, options = {}) {
        const attempts = this.recoveryAttempts.get(printerName) || 0;
        if (attempts >= this.MAX_RECOVERY_ATTEMPTS) {
            return {
                success: false,
                message: 'Maximum recovery attempts exceeded. Manual intervention required.',
                actionsPerformed: [],
            };
        }
        ...
        try {
            console.log(`🔧 Starting recovery for printer: ${printerName} (attempt ${attempts + 1})`);
     ...
     */
    async testPrintWithFallbacks(printerName) {
        ...
        return {
            ...
            message: 'All printing methods failed. Check printer connection and drivers.',
            diagnostics,
        };
    }
    /**
    ...
    async checkPrintSpoolerHealth() {
      ...
      `;
            const tempScript = path.join(os.tmpdir(), `spooler-check-${Date.now()}.ps1`);
    ...
    async restartPrintSpooler() {
      ...
      `;
            const tempScript = path.join(os.tmpdir(), `spooler-restart-${Date.now()}.ps1`);
            fs.writeFileSync(tempScript, script, 'utf8');
            try {
                await execAsync(`powershell -NoProfile -ExecutionPolicy Bypass -File "${tempScript}"`, { timeout: 15000 });
                return true;
            }
            finally {
                fs.unlinkSync(tempScript);
     ...
     */
    async checkPrinterDriver(printerName) {
        try {
            const script = `
        try {
      ...
      `;
            const tempScript = path.join(os.tmpdir(), `driver-check-${Date.now()}.ps1`);
            fs.writeFileSync(tempScript, script, 'utf8');
            try {
                const { stdout } = await execAsync(`powershell -NoProfile -ExecutionPolicy Bypass -File "${tempScript}"`, { timeout: 8000 });
     ...
     */
    async testPrinterCommunication(printerName) {
        try {
            const script = `
        try {
          $printer = Get-Printer -Name "${printerName.replace(/"/g, '""')}" -ErrorAction Stop
      ...
      `;
            ...
            fs.writeFileSync(tempScript, script, 'utf8');
            try {
                const { stdout } = await execAsync(`powershell -NoProfile -ExecutionPolicy Bypass -File "${tempScript}"`, { timeout: 6000 });
    ...
    async checkPrinterStatus(printerName) {
      ...
      `;
            const tempScript = path.join(os.tmpdir(), `status-check-${Date.now()}.ps1`);
            fs.writeFileSync(tempScript, script, 'utf8');
            try {
                const { stdout } = await execAsync(`powershell -NoProfile -ExecutionPolicy Bypass -File "${tempScript}"`, { timeout: 8000 });
     ...
     */
    async performRongtaSpecificChecks(printerName) {
        ...
        try {
            ...
            if (!driverCheck.installed) {
                recommendations.push('Download RONGTA driver from: https://www.rongtatech.com/category/downloads/1');
                recommendations.push('Run driver installer as Administrator');
                recommendations.push('Ensure USB cable is connected during driver installation');
            }
            ...
            recommendations.push('Verify RONGTA printer is powered on and paper loaded');
            recommendations.push('Try different USB port or cable');
            recommendations.push('Check Windows Device Manager for hardware conflicts');
     ...
     */
    async clearPrintQueue(printerName) {
        try {
            const script = `
        try {
      ...
      `;
            const tempScript = path.join(os.tmpdir(), `queue-clear-${Date.now()}.ps1`);
            fs.writeFileSync(tempScript, script, 'utf8');
            try {
                await execAsync(`powershell -NoProfile -ExecutionPolicy Bypass -File "${tempScript}"`, { timeout: 10000 });
                return true;
            }
            finally {
                fs.unlinkSync(tempScript);
     ...
     */
    async resetPrinterPort(printerName) {
        try {
     ...
     */
    async testBasicCommunication(printerName) {
     ...
     */
    async testNativePrint(printerName) {
        try {
            const testContent = `
=== MR5-POS TEST PAGE ===
Printer: ${printerName}
...
"@
      ...
      `;
            const tempScript = path.join(os.tmpdir(), `native-print-${Date.now()}.ps1`);
            fs.writeFileSync(tempScript, script, 'utf8');
            try {
                await execAsync(`powershell -NoProfile -ExecutionPolicy Bypass -File "${tempScript}"`, { timeout: 15000 });
                return { success: true };
     ...
     */
    async testDirectPortPrint(printerName) {
        try {
     ...
     */
    async testRawSocketPrint(printerName) {
        try {
     ...
     */
    async testESCPOSDirectPrint(printerName) {
        try {
     ...
     */
    getTroubleshootingInstructions(diagnostics) {
        const instructions = [];
        if (!diagnostics.spoolerHealthy) {
            instructions.push('1. Open Services (services.msc) and restart "Print Spooler" service');
            instructions.push('2. Set Print Spooler startup type to "Automatic"');
        }
        if (!diagnostics.driverInstalled) {
            instructions.push('3. Download the latest RONGTA driver from the official website');
            instructions.push('4. Run the driver installer as Administrator');
            instructions.push('5. Disconnect and reconnect the USB cable during installation');
        }
        if (!diagnostics.isOnline) {
            instructions.push('6. Check physical printer connections (USB/Network/Power)');
