...
try {
    ...
    enhancedLogger.warn('⚠️ Native printer library not available - will use PowerShell only', LogCategory.SYSTEM, 'NativePrinterDetection', { error });
}
/**
...
export class NativePrinterDetection {
    ...
    constructor() {
        ...
        enhancedLogger.info(`🔧 NativePrinterDetection initialized`, LogCategory.SYSTEM, 'NativePrinterDetection', {
     ...
     */
    async detectPrinters(forceRefresh = false) {
        const startTime = Date.now();
        try {
            // Check cache first (unless force refresh)
            if (!forceRefresh && this.printerCache && this.isCacheValid()) {
                const duration = Date.now() - startTime;
                enhancedLogger.info(`📦 Returning cached printer list (${this.printerCache.data.length} printers)`, LogCategory.SYSTEM, 'NativePrinterDetection', { cacheAge: Date.now() - this.printerCache.timestamp, method: this.printerCache.method });
                return {
            ...
            if (this.USE_NATIVE_DETECTION && electronPrinter) {
                try {
                    ...
                    enhancedLogger.info('🚀 Attempting native Win32 API printer detection', LogCategory.SYSTEM, 'NativePrinterDetection');
                    ...
                    enhancedLogger.info(`✅ Native detection successful: ${printers.length} printer(s) in ${duration}ms`, LogCategory.SYSTEM, 'NativePrinterDetection', { stats: this.getStats() });
                    return {
                        printers,
                        method: DetectionMethod.NATIVE,
                        duration,
                    ...
                    enhancedLogger.warn('⚠️ Native detection failed, falling back to PowerShell', LogCategory.SYSTEM, 'NativePrinterDetection', { error: nativeError });
                    // Fall through to PowerShell fallback
                }
            }
            // PowerShell fallback (always available)
            enhancedLogger.info('🔄 Using PowerShell printer detection', LogCategory.SYSTEM, 'NativePrinterDetection');
            ...
            enhancedLogger.info(`✅ PowerShell detection successful: ${printers.length} printer(s) in ${duration}ms`, LogCategory.SYSTEM, 'NativePrinterDetection');
            return {
                printers,
                method: DetectionMethod.POWERSHELL,
                duration,
            ...
            const duration = Date.now() - startTime;
            enhancedLogger.error('❌ Printer detection failed', LogCategory.SYSTEM, 'NativePrinterDetection', { error, duration }, error);
    ...
    async getNativePrinters() {
        try {
            ...
            enhancedLogger.info(`🔍 Native detection found ${printers.length} printer(s)`, LogCategory.SYSTEM, 'NativePrinterDetection');
            return printers;
        }
        catch (error) {
            enhancedLogger.error('❌ Native printer detection error', LogCategory.SYSTEM, 'NativePrinterDetection', { error }, error);
            throw error;
        }
    }
    /**
    ...
    async getPowerShellPrinters() {
      ...
      `;
            const tempScriptPath = path.join(os.tmpdir(), `printer-detect-native-${Date.now()}.ps1`);
            ...
            try {
                ...
                if (stderr && stderr.trim()) {
                    enhancedLogger.warn(`PowerShell warnings: ${stderr.trim()}`, LogCategory.SYSTEM, 'NativePrinterDetection');
                }
                // Validate output
                if (!stdout || stdout.trim() === '' || stdout.trim() === 'null') {
                    enhancedLogger.warn('No printer data returned from PowerShell', LogCategory.SYSTEM, 'NativePrinterDetection');
                    return [];
                }
                // Parse JSON output
                let printers;
                try {
                    ...
                    enhancedLogger.error('Failed to parse PowerShell JSON output', LogCategory.SYSTEM, 'NativePrinterDetection', { error: parseError, rawOutput: stdout.substring(0, 200) });
                ...
                enhancedLogger.info(`🔍 PowerShell detection found ${validPrinters.length} printer(s)`, LogCategory.SYSTEM, 'NativePrinterDetection');
            ...
            enhancedLogger.error('❌ PowerShell printer detection error', LogCategory.SYSTEM, 'NativePrinterDetection', { error }, error);
            throw error;
        }
    }
    /**
     ...
     */
    updateAverageDuration(method, duration) {
        if (method === 'native') {
    ...
    clearCache() {
        this.printerCache = null;
        enhancedLogger.info('🗑️ Printer cache cleared', LogCategory.SYSTEM, 'NativePrinterDetection');
    }
    /**
